...
\label{sec:2}
Wir betrachten nur reelle und komplexe Vektorbündel, sei also $K \in \set{\mathds{R},\mathds{C}}$.
...
\begin{itemize}
	...
	\item $\Hom \colon \VEKT_K \times \VEKT_K^\op \to \VEKT_K$, $(V,W) \mapsto \Hom_K(W,V)$ ist stetig $\Rightarrow$ Homomorphismenbündel\index{Homomorphismenbündel}.
\end{itemize}	
\end{beispiel}
...
Dieser hat die Form $h(x,v)= (x , g(x) \cdot v)$ für eine stetige Abbildung $g \colon X \to \GL_n(K)$. Sei jetzt 
\[
	w \colon X \times [c,b] \times K^n \longrightarrow X \times [c,b] \times K^n \quad , \quad (x,t,v') \longmapsto \enbrace*{x,t,g(x)\cdot v'}
\]
...
Abbildungen zu einem Isomorphismus von Vektorbündeln $X \times [a,b] \times K^n \to E$ zusammensetzen.	 \todo{Revision}
\end{beweis}
...
Sei nun $U := \bigcap_{i=1}^N U_i$. Nach Lemma \ref{trivialisierbarkeit_verkleben} ist $E|_{U \times I}$ trivialisierbar. Da $X \times I$ kompakt ist, wird es von endlichen 
vielen solcher Mengen überdeckt. 	
\end{beweis}
...
finden, so dass $E|_{U_i \times I}$ trivialisierbar ist. Seien $h_i \colon U_i \times I \times K^n \xrightarrow{\cong} E|_{U_i \times I}$ Trivialisierungen. Seien 
...
\]
da $h$ auf $A$ unabhängig ist von $t \in I$. Nach Konstruktion ist $E|_{A_j \times I}$ trivialisierbar. Nach Lemma \ref{schnitte_Hd_Raum} b) finden wir eine offene Umgebung 
