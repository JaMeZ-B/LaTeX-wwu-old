...
% $\id_{E'}$ ist ein Isomorphismus auf ganz $X$, also lässt sich $f \oplus \id$ zu einem Isomorphismus auf ganz $X$ ausdehnen.
...
$K^0(X,A)$ heißt auch \emph{$0$-te $K$-Gruppe} von $(X,A)$. Für $n>0$ definieren wir \index{K-Gruppe@$K$-Gruppe}
\begin{align*}
...
Wir wissen bisher nur, dass $K^0(X,A)$ ein Monoid ist bezüglich der Addition von Tripeln. Um zu sehen, dass dies tatsächlich eine Gruppenstruktur liefert, brauchen wir zwei 
Lemmata: 
% subsection 32 (end)


\begin{lemma}[label=sub:33,{name=[Existenz eines Bündelisomorphismus und einer Homotopie]}]
Sei $(X,A)$ ein Ko-Raumpaar.\marginnote{Morphismen entsprechen Schnitten von Morphismenbündeln, siehe Beweis von \autoref{schnitte_Hd_Raum}}
...
Abbildung, so dass $h_t$ ein Schnitt ist für alle $t \in I$ und $h_0=f'|_A$, sowie $h_1=f$ gilt.\todo[inline]{Ich sehe nicht ein, wie bei dieser Definition von $h$, $h_1=f$ bzw. $h_0=f'|_A$ gelten kann, da in deren Definitionen das Einheitsintervall doch gar nicht vorkommt \ldots}
...
\[
	...
	\end{tikzcd}
\]
Wir müssen $\tilde{h}$ zu einem Schnitt $X \times I \to \Iso(E'\times I, E \times I)$ ausdehnen, dass wir einen Schnitt bekommen. Betrachte \marginnote{$r$ aus Kofaserungseigenschaft}
\[
	\begin{tikzcd}
...
Wir können nur klären, was inverse Elemente in $K^0(X,A)$ sind. Kandidat für das inverse Element in $K^0(X,A)$ zu $(E_0,f,E_1)$ ist $(E_1, f ^{-1},E_0)$. Wir müssen
...
\begin{lemma}[label=sub:36,{name=[Homotopieinvarinaz]}]
...
\begin{lemma}[label=sub:37,{name=[Vektorbündel über einem Pushout]}]
...
\begin{lemma}[label=sub:38,{name=[Isomorphie eines Pullbackbündels für Ko-Raumpaare]}]
...
\begin{theorem}[label=sub:39,{name=[Isomorphismus in $K$-Theorie bei Pushout-Diagrammen]}]
\[
	\begin{tikzcd}[sep=large]
...
\begin{description}[wide=0cm]
	...
	\]
	zu einem Isomorphismus über $X_1$ fortsetzen lässt (siehe Bemerkung nach \ref{sub:31}). 
	Nach \autoref{sub:38} existieren Vektorbündel $q' \colon F' \to X$ und $\tilde{F} \to X_1$, sodass 
	$j_1^* F' \cong F \oplus \tilde{F}$ gilt. Nach Definition von stabiler Isomorphie gilt
	\[
		\benbrace[\big]{j_1^* E_0, j_1^* f , j_1^*E_1}_\st = \benbrace[\big]{j_1^* (E_0 \oplus F'),
		\Underbrace{j_1^*(f \oplus \id_{F'})}{\mathclap{\text{lässt sich zu Iso über $X_1$ ausdehnen}}}, j_1^*(E_1 \oplus F')}_\st
	\]
	...
	isomorph zur Verklebung von $j_1^* E_0 \to X_1$ und $j_2^* E_0 \to X_2$ mittels der Identität $i_2^* j_2^* E_0 \cong i_1^* j_1^* E_0$ wie in 
	\autoref{sub:37} beschrieben. Ähnliches gilt für $E_1 \to X$. Wir haben also Isomorphismen 
	\[
		j_1^* E_0 \xrightarrow[\cong]{\minwidthbox{j_1^* f }{2em}} j_1^* E_1 \quad \text{ und } \quad j_2^* E_0 = E_0\big|_{X_2} \xrightarrow[\cong]{\minwidthbox{f}{2em}}  
		E_1\big|_{X_2} = j_2^* E_1
	\]
	die kompatibel mit der Äquivalenzrelation in der Verklebung sind. Sie induzieren daher einen Isomorphismus $E_0 \cong E_1$ über $X$, der $f$ fortsetzt.
	Es folgt $\benbrace*{E_0,f,E_1}_\st=0$.
	\item[Surjektivität:] Sei $(E_0,f,E_1)$ ein Tripel über $(X_1,X_0)$. Nach \autoref{sub:38} existieren Vektorbündel $q \colon F \to X_1$ und $p_1' \colon E_1' \to X$
	mit einem Isomorphismus $\psi \colon E_1 \oplus  F \to j_1^* E_1'$. Sei $\alpha \colon (E_0 \oplus  F)|_{X_0} \to j_1^*E_1'|_{X_0}$ gegeben 
	durch
	\[
		...
		j_1^*E_1'\big|_{X_0}
	\]
	Definiere nun $p_0' \colon E_0' \to X$ durch Verkleben von $E_0 \oplus F \to X_1$ und $E_1'|_{X_2} \to X_2$ entlang $\alpha$ wie in \autoref{sub:37}. Sei 
	$f' \colon E_0'|_{X_2} \to E_1'|_{X_2}$ gegeben durch die Identität. Jetzt gilt
	\begin{align*}
	...
	\end{align*}
\end{description} 
Für den Fall $n>0$ lässt sich der Beweis auf den Fall $n=0$ zurückführen, denn es ist
\[
	(X_1,X_0) \times (I^n,\partial I^n) = \enbrace*{X_1 \times I^n, X_1 \times \partial I^n \cup X_0 \times I^n},
\]
und $X_0 \times I^n \cup X_1 \times \partial I^n \hookrightarrow X_1 \times I^n$ ist eine Kofaserung. Weiter ist das folgende Diagramm ein Pushout-Diagramm
\[
	\begin{tikzcd}
...
Ein Tripel $(X,B,A)$ von kompakten Hausdorff-Räumen heißt \Index{Ko-Raumtripel}, falls $A \subset B$ und $B \subset X$ Kofaserungen sind. 
\end{definition}
...
Projektion. Wir wollen nun sehen, dass $(1)$ und $(2)$ Isomorphismen sind:
\begin{enumerate}[(1)]
...
\end{beweis}
$\delta^{-n}$ für $n>1$ ist folgendermaßen definiert: Wir betrachten das Tripel
\[
...
\begin{itemize}
	...
	\[
		...
		\end{tikzcd}
	\]
	Dabei kommutiert \circled{1} nach Definition von $\delta ^{-1}$ und \circled{2}, da $\pr_x \circ i_3 \circ i_4 = i$ ist. $i_4^*$ ist ein Isomorphismus wegen 
	...
	\[
		...
		\end{tikzcd}
	\]
	\circled{4} kommutiert wieder nach Definition von $\delta ^{-1}$. Dass das Diagramm bei \circled{3} kommutiert, ist eine (mühsame!) Übungsaufgabe.
	...
	\[
		...
		\end{tikzcd}
	\]
	\circled{5} kommutiert nach Definition von $\delta^{-(n+1)}$. Die linke Spalte des Diagramms ist exakt, also auch die rechte Spalte. \qedhere \todo{Revision}
\end{itemize}
\end{beweis}
\newpage
...
\begin{definition}
\mbox{ }\marginnote{Ab hier weicht die Nummerierung aus Konsistenzgründen von den Vorlesungsnotizen ab}
\begin{itemize}
	\item Ein \Index{Kettenkomplex von $K$-Vektorbündeln} über einem Ko-Raumpaar $(X,A)$ ist gegeben durch ein Paar $(C_n,c_n)_{n \in \mathds{N}}$, wobei die $C_i$ 
	...
	\begin{enumerate}[a)]
		...
		exakt.
	\end{enumerate}
	\item Eine \Index{Kettenabbildung} zwischen $(C_*,c_*)$ und $(D_*,d_*)$ über $(X,A)$ ist eine Folge von Morphismen $f_k \colon C_k \to D_k$, so dass folgendes Diagramm 
	kommutiert
	\[
		\begin{tikzcd}
		...
		\end{tikzcd}
	\]
	\item Zwei Kettenkomplexe $(C_*^i, c_*^i)$ über $(X,A)$ für $i \in \set{0,1}$ heißen \Index{homotop}, falls es einen Kettenkomplex $(C_*,c_*)$ über 
...
\begin{lemma}[label=sub:lemma_3_1_4]
...
\begin{lemma}[label=sub:lemma_3_1_5]
...
\begin{definition}[label=sub:46,{name=[Hopf-Invariante]}]
