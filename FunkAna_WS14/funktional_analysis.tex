...
%-- Zweckentfremden für Titelbild
\publishers{\LARGE\emph{\enquote{Das folgt mit Hahn-Banach}}}
...
\begin{enumerate}[1)]
	...
	\item $d(x,z) \le d(x,y) + d(y,z) \quad \forall x,y,z \in X$
\end{enumerate}
% subsection 11 (end)
...
\begin{description}
	\item[$f$ ist Grenzwert:] Sei $\varepsilon>0$. Dann gibt es ein $N\in\mathds{N}$, sodass $d_{W,X}(f_k,f_l)<\varepsilon$ für alle $k,l\ge N$ gilt, da $(f_n)_n$ Cauchy ist.
...
\label{sub:19}
Sei $(X,d)$ ein vollständiger metrischer Raum und $\enbrace*{\overline{B}(x_n, \varepsilon_n)}_{n \in \mathds{N}}$ eine Folge mit 
...
\]
Wir schreiben $X^*:= \mathcal{L}(X,\mathds{K})$ für den \Index{Dualraum} von $X$. Die Elemente von $X^*$ heißen (stetige) \Index{Funktionale}.\footnote{Aus der linearen Algebra: $X^* = \Hom(X,\mathds{R}) = \set[T : X \to \mathds{R}]{T \text{ linear}}$. Die Elemente von $X^*$ heißen Funktionale.}
...
\begin{description}
	...
	\] 
	Angenommen, $\mu = \infty$. Dann existieren $x_n \in X$ mit $\frac{1}{\norm[X]{x_n}} \cdot \norm[Y]{T(x_n)} \ge n$. Betrachte 
...
\subsection[Proposition: $\mathcal{L}(X,Y)$ ist ein normierter Raum mit der Operatornorm]{Proposition} % (fold)
\label{sub:29}
...
\begin{enumerate}[a)]
	...
	$B(\overline{x},\varepsilon) \subseteq q \enbrace[\big]{ B(x,\varepsilon)} \subseteq q(V)$. Da $x \in V$ beliebig gewählt war, folgt $B(\overline{x},\varepsilon) \subseteq q(V)$ für jedes $\overline{x} \in q(V)$. \bewende
\end{enumerate}
% subsection 213 (end)
...
\minisec{Beweis}
Definiere $\tilde{\varphi} \colon X \to \mathds{R}$ durch $\tilde{\varphi}(x) := \inf_{y \in Y} \enbrace[\big]{ \varphi(x-y) + \psi(y)}$, $x \in X$. 
% Es gilt $0 = \varphi(x +y - x-y) \le \varphi(x-y) + \varphi(-x) + \varphi(y)$, womit folgt
Es gilt $0= \varphi(-y) - \varphi(-y) = \varphi(x-y-x) - \varphi(-y) \le \varphi(x-y) + \varphi(-x) - \varphi(-y)$, womit folgt
\[
...
\label{sub:34}
Seien $X,Y$ normierte Räume. Wir definieren eine Abbildung $.^{\tr} \colon \mathcal{L}(X,Y) \to \mathcal{L}(Y^*,X^*)$ durch $T \mapsto T^{\tr} = (\varphi \mapsto \varphi \circ T)$.
...
$L^1(\mu)$ ist ein topologischer Vektorraum (Topologie induziert durch $\norm[1]{\cdot}$) und $\mathcal{N}^1 \subset \mathcal{L}^1(\mu)$ ist ein abgeschlossener Untervektorraum. Definiere 
\[
...
Nullmengen sind, folgt, dass $\sum_{k=0}^{\infty} \abs*{f_{n_{k+1}}- f_{n_k}}$ fast überall auf $E$ konvergiert. Also konvergiert $\sum_{k=0}^{\infty} \abs*{f_{n_{k+1}}- f_{n_k}}$
...
\label{sub:519}
\begin{enumerate}[a)]
	\item Sei $X$ ein normierter Raum. Falls $X^*$ separabel ist, so ist $X$ separabel.\marginnote{bezüglich der Normtopologie}
...
\marginnote{Diese Aussage wird manchmal als Riesz\-scher Darstellungssatz bezeichnet}
\begin{enumerate}[(i)]
...
\subsection[Proposition: Die kompakten Operatoren bilden ein abgeschlossenes Ideal in $\mathcal{L}(X)$]{Proposition} % (fold)
\label{sub:89}
...
\subsection[Satz: Diagonalisierbarkeit selbstadjungierter, kompakter Operatoren (Spektralsatz)]{Satz (Spektralsatz)} % (fold)
\label{sub:810}
...
\subsection[Corollar: Für kompakte Operatoren $K \in \mathcal{K}(\mathcal{H})$ gilt für den Index $\idx(\id_\mathcal{H}- K)=0$]{Corollar} % (fold)
\label{sub:94}
...
\subsection[Beispiele für Fredholm-Operatoren]{Beispiele} % (fold)
\label{sub:95}
\begin{enumerate}[(i)]
	\item Sei $S \in \mathcal{L}\enbrace*{\ell^2(\mathds{N})}$ der unilaterale Shift\index{unilateraler Shift}, das heißt
