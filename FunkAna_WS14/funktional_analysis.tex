...
\begin{enumerate}[(i)]
	\item Es gilt das Parallelogrammgesetz $\norm{x+y}^2 + \norm{x-y}^2 = 2 \enbrace*{\norm{x}^2 + \norm{y}^2}$ für alle $x,y \in X$.
...
linear in der ersten), dann ist $q$ durch die Abbildung $n_q \colon X \to \mathds{C}$, $n_q(x) := q(x,x)$ eindeutig bestimmt durch die Polarisierungsidentität
\[
...
\begin{enumerate}[(i)]
	\item $\kappa$ ist konjugiert linear, d.h. $\kappa(\alpha x+ \alpha' x') = \overline{\alpha} \kappa(x) + \overline{\alpha'}\kappa(x')$ mit $x,x' \in \mathcal{H}$, 
...
\label{sub:714}
\begin{enumerate}[(i)]
	\item Für $x \bot y \in \mathcal{H}$ gilt der Satz des Pythagoras: $\norm{x+y}^2 = \norm{x}^2  + \norm{y}^2$.
	\item Wir werden sehen, dass jeder Hilbertraum eine Hilbertraumbasis besitzt.
	\item Im Allgemeinen ist eine Hilbertraumbasis \emph{keine} Vektorraumbasis (Hamelbasis).
\end{enumerate}
% subsection 714 (end)
% section 7 (end)
