...
\label{sub:11}
Ein \Index{metrischer Raum} ist ein Paar $(X,d)$, wobei $X$ eine Menge und $d : X \times X \to [0,\infty)$ ist, sodass
\begin{enumerate}[1)]
...
alle $x \in \mathcal{H}_1$, $y \in \mathcal{H}_2$. Folglich muss $T^{**} x = T x$ gelten.\marginnote{gleiche Werte auf allen Elementen, Hahn-Bannach} \bewende 
% subsection 725 (end)
...
\end{equation*}
Falls $\mathds{K}=\mathds{C}$, so gilt für $\rho, \sigma \in \mathds{R}_+$ geeignet \todo{das stimmt noch nicht so ganz \ldots}
\begin{align*}
...
d.h. $(T_j)_{j \in I}$ konvergiert gegen $T$ in der \bet{starken Operatortopologie}\index{starke Operatortopologie}.
\minisec{Beweis}
...
\label{sub:81}
Sei $X$ ein $\mathds{K}$-Banachraum und $T \in \mathcal{L}(X)$. Wir definieren das \Index{Spektrum} von $T$ als \todo{RevChap8}
\[
