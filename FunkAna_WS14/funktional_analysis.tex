...
\]
Ein stetiger Operator zwischen normierten Vektorräumen heißt deswegen aus \bet{beschränkt}\index{Operator!beschränkter Operator} 
\minisec{Beweis}
\begin{description}
...
\subsection[Definition: Normierte $K$-Algebra]{Definition} % (fold)
\label{sub:210}
Eine $K$-Algebra heißt \bet{normiert}\index{normierte $K$-Algebra}, falls $A$ mit einer Norm $\norm{.}$ versehen ist, so dass $(A,\norm{.} )$ ein normierter Vektorraum und
\[
...
\subsection{Proposition und Definition} % (fold)
\label{sub:213}
...
\begin{enumerate}[a)]
	...
	\end{itemize}
	\item Sei $0 \not= \lambda \in K$, $x \in X$. Dann gilt
	\begin{align*}
	...
	$B(\overline{x},\varepsilon) \subseteq q \enbrace*{ B(x,\varepsilon)} \subseteq q(V)$. \bewende
\end{enumerate}
% subsection 213 (end)
...
\label{sub:215}
$C(\Omega, K)$, wobei $\Omega$ ein kompakter Hausdorffraum ist, ist eine Banachalgebra.
% subsection 215 (end)
...
\minisec{Beweis}
Definiere $\tilde{\varphi} : X \to \mathds{R}$ durch $\tilde{\varphi}(x) := \inf_{y \in Y} \enbrace*{ \varphi(x-y) + \psi(y)}$, $x \in X$. Es gilt
\[
