...
\subsection[Proposition: $X$ vollständig $\Rightarrow$ $C_b(W,X)$ und $C(W,X)$ vollständig]{Proposition} % (fold)
\label{sub:18}
...
Aus $\tilde{\varphi}(y_0) = \inf_{y \in Y}\varphi(y_0-y) + \psi(y)$ folgt $\tilde{\varphi}|_{Y} \le \psi$ mittels $y=y_0$. Weiter folgt aus \ref{sub:220}, dass $\psi$ minimal in 
$\mathcal{S}(Y)$ ist. Also muss insgesamt gelten:
\[
...
\subsection[Definition und Proposition: Lineare Isometrie $\iota_X : X \to X^{**}$ (kanonische Inklusion)]{Definition und Proposition} % (fold)
\label{sub:33}
...
\subsection[Corollar über Beschränktheit einer Teilmenge von $\mathcal{X,Z}$]{Corollar} % (fold)
\label{sub:37}
...
Angenommen $\sigma(T)= \emptyset$. $R(T,\lambda)$ ist stetig auf $\set[\lambda \in \mathds{C}]{\abs*{z} \le 2 \cdot \norm{T}}$, also beschränkt. Weiter gilt für 
...
\label{sub:810}
Sei $T \in \mathcal{K}(X)$ ein selbstadjungierter, kompakter Operator auf dem Hilbertraum $\mathcal{H}$. Dann ist $T$ diagonalisierbar.
% subsection 810 (end)
% section 8 (end)
