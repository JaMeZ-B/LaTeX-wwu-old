...
\begin{align*}
	...
	\norm[X]{x'}      
\end{align*}
Daher ist $\cdot : \mathds{K} \times X \to X$ stetig, denn falls $(\lambda_i)_{i \in I}$ ein Netz in $\mathds{K}$, $(x_i)_{i \in I}$ ein Netz in $X$ mit $\lambda_i \to \lambda $, $x_i\to x$,
...
\]
Wir schreiben $X':= L(X,\mathds{K})$ für den \Index{Dualraum} von $X$. Die Elemente von $X'$ heißen (stetige) \Index{Funktionale}.\footnote{Aus der linearen Algebra: $X^* = \Hom(X,\mathds{R}) = \set[T : X \to Y]{T \text{ linear}}$. Die Elemente von $X^*$ heißen Funktionale}
Die Elemente von $L(X,X)$ heißen (stetige) \Index{Operatoren} auf $X$. 
% subsection 25 (end)

\subsection[Bemerkung: Vektorraumstruktur auf $L(X,Y)$, Algebrastruktur auf $L(X,X)$]{Bemerkung} % (fold)
\label{sub:26}
$L(X,Y)$ und $L(X,\mathds{K}) = X'$ sind Vektorräume, wobei die Vektorraumstruktur punktweise definiert ist. $L(X,X)$ ist sogar eine Algebra mittels Komposition.
% subsection 26 (end)

\subsection[Proposition: Stetigkeit linearer Abbildungen zwischen normierten Vektorräumen]{Proposition} % (fold)
\label{sub:27}
...
\label{sub:28}
Seien $X,Y$ normierte Vektorräume, $T \in L(X,Y)$. Die \bet{Norm} (oder \Index{Operatornorm}) von $T$ ist
\[
...
\subsection[Propostion: $L(X,Y)$ ist ein normierter Raum mit der Operatornorm]{Proposition} % (fold)
\label{sub:29}
Seien $X,Y$ normierte Räume, betrachte $L(X,Y)$ mit $\norm{.}$. Dann ist $L(X,Y)$ ein normierter Raum.
\minisec{Beweis}
\begin{itemize}
	\item $\norm{T} \ge 0 $ für alle $T \in L(X,Y)$.
	\item Es gilt
	\begin{align*}
		\norm{T}=0 \iff \forall x \in X, \norm[X]{x}=1 : \norm[Y]{Tx}=0 &\iff \forall x \in X, \norm[X]{x}=1 : Tx=0 \\ &\iff \forall x \in X : Tx=0 \\ &\iff T=0
	\end{align*}
	\item Sei $\lambda  \in \mathds{K}$, $T \in L(X,Y)$. Dann gilt
	\[
		\norm{\lambda \cdot T} = \sup_{\norm[X]{x}=1} \norm[Y]{\lambda  \cdot T(x)}  = \sup_{\norm[X]{x}=1 } \abs*{\lambda } \cdot \norm[Y]{Tx} =
		\abs*{\lambda } \cdot \sup_{\norm[X]{x}=1 } \norm[Y]{Tx} = \abs*{\lambda } \cdot \norm{T}      
	\]
	\item Seien $T_1, T_2 \in L(X,Y)$ und $x \in X$ mit $\norm[X]{x}=1 $. Dann gilt
	\[
...
\begin{enumerate}[(i)]
	...
	\item $L(X,X)$, für $X$ ein normierter Raum, mit $\norm{.}$. Für $S,T \in L(X,X)$ gilt
	\[
...
\begin{enumerate}[a)]
	...
	\begin{itemize}
		\item $\norm{\overline{x}} \ge 0 $ Klar für alle $\overline{x} \in \nicefrac{X}{Y}$
...
$\overline{\psi} : X \to \mathds{K}$ mit $\norm[L(X,\mathds{K})]{\overline{\psi}} = \norm[L(Y,\mathds{K})]{\psi}$.
\minisec{Beweis}
...
\subsection[Proposition: $Y$ vollständig $\Rightarrow $ $L(X,Y)$ ist vollständig]{Proposition} % (fold)
\label{sub:31}
Seien $X,Y$ normierte Vektorräume, $Y$ vollständig. Dann ist $L(X,Y)$ vollständig bezüglich $\norm[L(X,Y)]{.}$.
\minisec{Beweis}
Sei $(T_n)_{n \in \mathds{N}} \subseteq L(X,Y)$ eine Cauchy-Folge bezüglich $\norm[L(X,Y)]{.}$. Für $x \in X$ ist dann auch $(T_n x)_{n \in \mathds{N}} \subset Y$ Cauchy
...
$\norm{T_n} = \norm{T_n - T_{n_0}+ T_{n_0}} \le \norm{T_n- T_{n_0}} + \norm{T_{n_0}}$}. Dann existiert $C \ge 0$ mit $\norm[L(X,Y)]{T_n} \le C $, $n \in \mathds{N}$, also
gilt $\norm[Y]{T x} \le C \cdot \norm{x}$, $x \in X$, woraus $\norm[L(X,Y)]{T} \le C$ und die Stetigkeit von $T$ folgt. Also ist $T \in L(X,Y)$.

\noindent Falls $\varepsilon >0$, so existiert $n_0 \in \mathds{N}$ mit $\norm[L(X,Y)]{T_n - T_m}  < \varepsilon$ für $n,m \ge n_0$. Insbesondere: 
\begin{align*}
	\norm[Y]{(T_n- T_m)x} \le \norm[L(X,Y)]{T_n -T_m} \cdot \norm[X]{x} < \varepsilon \cdot \norm[X]{x}    
\end{align*}
...
$\norm[L(X,Y)]{T_n -T} \le \varepsilon$ für$n \ge n_0$, also $T_n \to T$ bezüglich $\norm[L(X,Y)]{.}$. \bewende
% subsection 31 (end)

\subsection[Corollar: Der Dualraum ist vollständig, $L(X,X)$ ist Banachalgebra, falls $X$ Banachraum ist]{Corollar} % (fold)
\label{sub:32}
\begin{enumerate}[(i)]
	\item Der Dualraum $X^*$ eines normierten $\mathds{K}$-Vektorraumes $X$ ist vollständig. 
	\item Falls $\set{0} \not= X$ ein Banachraum ist, so ist $L(X,X)$ eine Banachalgebra.
\end{enumerate}
\minisec{Beweis}
\begin{enumerate}[(i)]
	\item $X^*=L(X,\mathds{K})$ und $\mathds{K}$ ist vollständig. Wende \ref{sub:31} an.
	\item $L(X,X)$ ist ein Banachraum nach \ref{sub:31}. $L(X,X)$ ist eine normierte Algebra: 
	\begin{itemize}
		\item $\norm[L(X,X)]{\id_X}=1$ \hfill (Warum?)
		\item Für $S,T \in L(X,X)$, $x \in X$ gilt
		\begin{align*}
			\norm[X]{(S \circ T)x} \le \norm{S} \cdot \norm[X]{T x} \le \norm{S} \cdot \norm{T} \cdot \norm[X]{x}   
		\end{align*}
		Also ist $\norm{S \circ T} \le \norm{S} \cdot \norm{T}$. \bewende
...
\begin{description}
	...
	\item[$\iota_X$ ist linear:] Für $\alpha, \beta \in \mathds{K}$, $x,y \in X$, $\varphi \in L(X,\mathds{K})= X^*$ gilt
	\begin{align*}
...
\label{sub:34}
Seien $X,Y$ normierte Räume. Definiere eine Abbildung $.^{\tr} L(X,Y) \to L(Y^*,X^*)$ durch $T \mapsto T^{\tr} = (\varphi \mapsto \varphi \circ T)$.
...
\[
	...
	\end{tikzcd}
\]
kommutiert für jedes $T \in L(X,Y)$. Für $\iota_{X^*} : X^* \to X^{***}$ und $\iota_{X}^\tr : X^{***} \to X^*$ gilt $\iota_X^\tr \circ \iota_{X^*} = \id_{X^*}$.
\minisec{Beweis}
\begin{enumerate}[(i)]
	\item Für $T,S \in L(X,Y)$, $\alpha, \beta \in \mathds{K}$, $\varphi \in Y^*$ gilt
	\begin{align*}
	...
	ist $\norm[X^*]{T^\tr \psi} \le  \norm[Y^*]{\psi} \cdot  \norm[L(X,Y)]{T}$ und damit folgt 
	\[
		\norm[L(Y^*,X^*)]{T^\tr} \le \norm[L(X,Y)]{T}  
	\]
	Ebenso ist $\norm[L(X^{**},Y^{**})]{T^{\tr \, \tr}} \le \norm[L(Y^*,X^*)]{T^\tr} \le \norm[L(X,Y)]{T}$. Andererseits gilt 
	$\norm[l(X,Y)]{T} \le \norm[L(X^{**},Y^{**})]{T^{\tr\, \tr}}$ nach \ref{sub:33} und da das Diagramm kommutiert (Warum?). 
	\[
		\Rightarrow \norm[L(X,Y)]{T} \le \norm[L(Y^*,X^*)]{T^\tr}  \le  \norm[L(X,Y)]{T} 
	\]
...
\label{sub:35}
Sei $X$ ein Banachraum und $Y$ ein normierter Raum. Sei $M \subset L(X,Y)$, sodass die Menge 
\[
...
\end{align*}
$\Rightarrow \norm[L(X,Y)]{T} \le \frac{4 K}{\varepsilon} =: C$. \bewende
% subsection 35 (end)
...
$M = \iota_Z(N) \subset Z^{**}= L(Z^*,\mathds{K})$.
...
\subsection{Corollar} % (fold)
\label{sub:37}
Sei $X$ ein Banachraum und $Z$ ein normierter Raum. $M \subset L(X,Z)$ eine Teilmenge, so dass gilt: Für alle $\varphi \in Z^*$ und $x \in X$ existiert $C_{x,\varphi} \ge 0$
mit
\[
...
\label{sub:38}
Seien $X,Y$ Banachräume, $T \in L(X,Y)$ surjektiv. Dann ist $T$ offen, d.h. für eine offene Menge $U \subset X$ ist $T(U) \subset Y$ offen.
\minisec{Beweis}
\begin{enumerate}[(i)]
...
\label{sub:39}
Seien $X,Y$ Banachräume, $T \in L(X,Y)$ bijektiv. Dann gilt $T ^{-1} \in L(Y,X)$, d.h. $T$ ist ein Homöomorphismus\index{Homöomorphismus}.
\minisec{Beweis}
...
Tatsache für Übungen: $X,Y$ Banachräume, $T \in L(X,Y)$ surjektiv $\Rightarrow T$ ist offen.

Corollar: $T \in L(X,Y)$ bijektiv $\Rightarrow $ $T ^{-1}$ stetig.
