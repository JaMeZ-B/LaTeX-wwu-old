...
\label{sub:111}
\begin{enumerate}[(i)]
	\item Die Aussage gilt auch für lokalkompakte Hausdorffräume.\index{lokalkompakt} \hfill(Übung)
...
\minisec{Bemerkung}
Wenn $\norm{1_A} < 1$, dann $\norm{1_A} = \norm{1_A \cdot 1_A} \le \norm{1_A} \cdot \norm{1_A} \Rightarrow \norm{1_A} =0 $. Also $1_A=0$ und damit $A= \set{0}$.\todo{Rev Chap 2}
% subsection 210 (end)
...
\subsection[Corollar: Dualraum ist vollständig und $\mathcal{L}(X,X)$ ist Banachalgebra, falls $X$ Banachraum]{Corollar} % (fold)
\label{sub:32}
\begin{enumerate}[(i)]
...
\]
punktweise \Index{gleichmäßig beschränkt} ist, d.h. für $x \in X$ existiert $C_x \ge 0$ mit $\norm[Y]{T x} \le C_x$, $T \in M$. Dann existiert $C \ge 0$ mit $\norm[\mathcal{L}(X,Y)]{T} \le C$,
$T \in M$.
\minisec{Beweis}
...
\]
$(L^1(\mu),\norm[1]{\cdot})$ ist ein Banachraum (Ana \RM{3}, 9.23).
% subsection 41 (end)
...
\[
	\norm[1]{f \cdot g} \le \norm[p]{f} \cdot \norm[q]{g}   
\]
Die entsprechenden Aussagen gelten dann auch für $L^p(\mu)$, $L^q(\mu)$, $L^1(\mu)$.\index{Hölder-Ungleichung}
\minisec{Beweis}
...
\label{sub:46}
Für $1\le p < \infty$, $f,g \in \mathcal{L}^p(\mu)$ (bzw. $L^p(\mu)$) gilt die \Index{Minkowski-Ungleichung}
\[
	\norm[p]{f +g} \le \norm[p]{f} + \norm[p]{g}   
\]
\minisec{Beweis}
...
\label{sub:51}
Sei $X$ ein normierter $\mathds{K}$-Vektorraum. Die \Index{schwache Topologie} $\weakT{X}$ ist die gröbste\footnote{auch: "dollste"} Topologie auf $X$, sodass alle 
$\varphi \in X^*$ stetig sind. 
\todo{RevChap5}
% subsection 51 (end)

\subsection[Bemerkungen zu schwachen Topologie]{Bemerkung} % (fold)
\label{sub:52}
\begin{enumerate}[(i)]
...
Sei nun $(\varphi_\lambda)_{\lambda \in \Lambda} \subset \overline{B_{X^*}(0,1)}$ ein Netz und $\varphi \in \overline{B_{X^*}(0,1)}$. Es genügt zu zeigen
\[
...
\subsection*{Nachtrag (zum Beweis von \ref{sub:512})} % (fold)
...
gerichtet bezüglich "$\subset$". Zu jedem $V \in \Lambda$ wähle \marginnote{Auswahlaxiom nötig!} $x_V \in (X \setminus U) \cap V$. Dann gilt 
$(x_v)_{v \in \Lambda} \subset X \setminus U$ und $x_v \to x$.

% subsection nachtrag_512 (end)

\subsection[Definition: Reflexiver normierter Raum]{Definition} % (fold)
\label{sub:513}
Der normierte Raum $X$ heißt \Index{reflexiv}, falls die kanonische Abbildung $\iota_X : X \to X^{**}$ ein Isomorphismus ist. 
...
\end{align*}
Für $y \in \overline{B_X(0,1)}$ gilt $\re \sum_{i=1}^{n} \enbrace*{\varphi_i(y)- \gamma_i} \cdot \overline{\delta _i} \le 0$.
...
\label{sub:61}
Ein normierter Raum $X$ heißt \Index{gleichmäßig konvex}, falls eine der folgenden äquivalenten Bedingungen erfüllt ist:\todo{RevChap6}
\begin{enumerate}[(i)]
...
\begin{figure}[H]
	...
	\end{tikzpicture}
	\caption[Einheitskugel für $p=2$, ${\color{DarkGoldenrod2}p=1}$ und ${\color{Firebrick3}p=\infty}$]{Einheitskugel für $p=2$, ${\color{DarkGoldenrod2}p=1}$ und 
	${\color{Firebrick3}p=\infty}$. Nur für $p \not\in \set{1,\infty}$ ist (\ref{61:enum:2}) aus 
	\ref{sub:61} erfüllt. Anschaulich gesprochen bedeutet dies, dass nur in gleichmäßig konvexen Räumen Bälle auch tatsächlich \enquote{rund} sind.}}
\end{figure}
% subsection 62 (end)

\subsection[Definition und Proposition: Strikt konvexer Raum]{Definition und Proposition} % (fold)
\label{sub:63}
...
\minisec{Beweis}
Seien $x,y$ wie angegeben; \oE $\norm{x}=1$. Setze $x' := \frac{x+y}{\norm{x+y}}$, $a := \frac{x+x'}{2}$, $\alpha := \norm{a}$, $\beta := \norm{x+y-a}$
\begin{figure}{H}
	\centering{
	\begin{tikzpicture}[scale=3, my brace/.style={decorate, decoration={brace, amplitude=5pt, mirror, raise=3pt}, gray}]
		...
		\draw[thick] (0,0)  -- (x+y);
		\draw[my brace] (0) -- (x') node[midway, yshift=-0.6cm]{$\norm{x'}=1$};
		\draw[my brace] (x') --(x+y) node[midway, yshift=-0.6cm]{$\norm{y}$};
		
		\draw[thick] (0) -- (x);
		\draw[my brace] (x) -- (0) node[midway, above left, yshift=0.16cm]{$\norm{x}=1$};
		
		\draw[thick] (x) -- (x');
		
		\draw[thick] (x+y) -- (x);
		\draw[my brace] (x+y) -- (x) node[midway, above right, xshift=0.1cm]{$\norm{y}$};
		
		\draw[thick,dashed] (0) -- (a) -- (x+y);
		\draw[my brace] (0) -- (a) node[midway, below,yshift=-0.2cm,xshift=0.17cm]{$\alpha$};
		\draw[my brace] (a) -- (x+y) node[midway, below,yshift=-0.2cm,xshift=-0.22cm]{$\beta$};
		
		\foreach \t/\pos in {0/left,x/above,x'/below,x+y/right,a/above right}{
			\draw[fill=DodgerBlue2,thick] (\t) circle (0.02) node[\pos]{$\t$};
		}
	\end{tikzpicture}
	\caption[Zeichnung zum Beweis von \ref{sub:63}]{Die Tatsache, dass $\norm{x}$ und $\norm{x'}$ in dieser Zeichnung nicht gleich sind, weißt schon darauf hin, dass diese 
	Zeichnung in Wahrheit \enquote{degeneriert}.}}
\end{figure}
Es gilt 
\[
	\norm{x+y-x'} = \enbrace*{1- \frac{1}{\norm{x+y}}} \cdot \norm{x+y} = \norm{x+y}-1 = 1 + \norm{y} -1 = \norm{y}      
\]
also $\beta = \norm{x+y-a} \le \doppelstrich[\big]{\frac{x+y-x}{2}} + \doppelstrich[\big]{\frac{x+y-x'}{2}}\le \frac{\norm{y}}{2} + \frac{\norm{y}}{2} = \norm{y}$. Weiter gilt
$\alpha = \norm{a} \le \frac{\doppelstrich{x}}{2} + \frac{\doppelstrich{x'}}{2} =1$ und
\begin{align*}
	1 + \norm{y} = \norm{x+y} \le \norm{a} + \norm{x+y-a} = \alpha+ \beta\le 1 + \norm{y} 
\end{align*}
also $1 \le \alpha \le 1$. Wegen $\norm{x}=1$, $\norm{x'}=1$ und $\norm{\frac{1}{2} (x+x')}= \norm{a}=\alpha=1 > 1 -\delta$ für jedes $\delta >0$ erhalten wir aus 
gleichmäßiger Konvexität $\norm{x-x'} \le \varepsilon$ für jedes $\varepsilon>0 \Rightarrow x=x'$ und $y=\norm{y} \cdot x$. \bewende
% subsection 63 (end)
...
\begin{enumerate}[(i)]
	...
	\item Wähle eine Folge $(x_n)_{\mathds{N}} \subset W$ mit \marginnote{\oE wieder $a=0$}
	\[
...
\label{sub:66}
Für $\alpha,\beta \ge 0$, $p \ge r >0$ gilt $(\alpha^p + \beta^p)^{\frac{1}{p}} \le \enbrace*{\alpha^r + \beta^r}^{\frac{1}{r}}$.\index{Jensen-Ungleichung}
\minisec{Beweis}
...
\subsection[Definition: Hermitesche Form]{Definition} % (fold)
\label{sub:71}
Sei $X$ ein $\mathds{K}$-Vektorraum. Eine \Index{hermitesche Form} auf $X$ ist eine Abbildung $\skal*{\cdot}{\cdot} \colon X \times X \to \mathds{K}$ mit\todo{RevChap7}
\begin{align*}
...
\end{align*}
$\skal*{\cdot}{\cdot } $ heißt \bet{positiv semidefinit}\index{hermitesche Form!positiv semidefinit}, falls $\skal*{x}{x}\ge 0$ für $x \in X$.  $\skal*{\cdot }{\cdot } $ 
heißt \bet{positiv definit}\index{hermitesche Form!positiv definit}, falls 
...
\subsection[Proposition über Zusammenhang von Norm und Skalarprodukt mit dem Parallelogrammgesetz]{Proposition} % (fold)
\label{sub:77}
Für einen normierten Raum $X$ sind äquivalent: \todo{eventuell Bild von Parallelogramm hinzufügen}
\begin{enumerate}[(i)]
...
\begin{enumerate}[(i)]
	\item $\kappa$ ist \Index{konjugiert linear}, d.h. $\kappa(\alpha x+ \alpha' x') = \overline{\alpha} \kappa(x) + \overline{\alpha'}\kappa(x')$ mit $x,x' \in \mathcal{H}$, 
...
\begin{enumerate}[(i)]
	...
	\end{align*}
	nach l'Hospital. Im Detail: \todo{stimmt noch nicht so ganz, aber passt schon …}
	\begin{align*}
...
\begin{enumerate}[(i)]
	\item $x,y \in \mathcal{H}$ heißen \Index{orthogonal}, $x\bot y$, falls $\skal*{x}{y}=0$.
	\item $A,B  \subset \mathcal{H}$ heißen \Index{orthogonal}, falls $x \bot y$ für alle $x \in A$, $y \in B$. 
	...
	\item $\set{x_i}_{I} \subset \mathcal{H}$ heißt \Index{Orthonormalsystem}, falls $\skal*{x_i}{x_j}= \delta_{ij}$, $i,j \in I$.
...
\label{sub:714}
\begin{enumerate}[(i)]
	\item Für $x \bot y \in \mathcal{H}$ gilt der \Index{Satz des Pythagoras}: $\norm{x+y}^2 = \norm{x}^2  + \norm{y}^2$.
...
\subsection[Proposition über Schnitt und Summe eines Unterraumes mit seinem orthogonalen Komplement]{Proposition} % (fold)
\label{sub:715}
...
\subsection[Definition: Summierbar]{Definition} % (fold)
\label{sub:716}
...
\begin{enumerate}[(i)]
	\item Es gilt $\sum_{i \in I} \abs*{\skal*{x_i}{x}}^2 \le \norm{x}^2$, (\Index{Besselsche Ungleichung}).
	\item Es gilt die \Index{Parcelvalsche Gleichung} $\sum_{i \in I} \abs*{\skal*{x_i}{x}}^2 = \norm{x}^2$  g.d.w. $x= \sum_{i \in I} \skal*{x_i}{x} \cdot x_i$.
\end{enumerate}
\minisec{Beweis}
\begin{enumerate}[(i)]
	\item Für $J \Subset I$ endlich gilt
	\begin{align*}
	...
	\[
		\sum_{i \in J} \abs*{\skal*{x_i}{x}}^2 \xrightarrow{J \Subset I} \norm{x}^2  \iff \sum_{i \in J} \skal*{x_i}{x} \cdot x_i \xrightarrow{J \Subset I}  x \bewende
	\]
\end{enumerate}
% subsection 720 (end)
