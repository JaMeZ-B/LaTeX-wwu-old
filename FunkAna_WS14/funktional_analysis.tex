...
\label{sub:51}
Sei $X$ ein normierter $\mathds{K}$-Vektorraum. Die \Index{schwache Topologie} $\schwachT{X}$ ist die gröbste\footnote{auch: "dollste"} Topologie auf $X$, sodass alle 
...
\begin{enumerate}[(i)]
	...
	$\schwachT{X} \subset \mathcal{T}_X^{\norm{\cdot}}$. Mit anderen Worten: Die Abbildung 
	$\id_X: \enbrace*{X,\mathcal{T}_X^{\norm{\cdot}}}\to\enbrace[\Big]{X,\schwachT{X}}$ ist stetig.
	\item Mengen der Form $\varphi ^{-1}(U)$ für $U \subset K$ offen, $\varphi \in X^*$ bilden eine Subbasis für $\schwachT{X}$. Mengen der Form 
	\[
		\bigcap_{i=1}^n \varphi_i ^{-1} (U_i) \qquad, \qquad U_i \subset \mathds{K} \text{ offen}, \varphi_i \in X^*
	\]
	bilden eine Basis für $\schwachT{X}$.
\end{enumerate}
% subsection 52 (end)

\subsection[Proposition: Die schwache Topologie ist Hausdorffsch]{Proposition} % (fold)
\label{sub:53}
$\enbrace[\big]{X,\schwachT{X}}$ ist Hausdorffsch.
\minisec{Beweis}
Seien $x \not= y \in X$. Nach \hyperref[sub:221]{Hahn-Banach} existiert $\varphi \in X^*$ mit $\varphi(x) \not= \varphi(y)$. Setze 
...
\begin{enumerate}[(i)]
	\item $x_\lambda \xrightarrow{\w}  x$, d.h. $x_\lambda \to x$ in $\schwachT{X}$
...
\begin{description}
	...
	$\varphi$ stetig bezüglich $\schwachT{X}$
	\[
