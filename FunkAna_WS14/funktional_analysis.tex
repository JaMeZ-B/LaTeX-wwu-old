...
\]
also ist $\norm[X^*]{\varphi} \le \re \sum_{i=1}^{n} \gamma_i \cdot \overline{\delta_i}$. Andererseits gilt aber\todo[noline]{Das verstehe ich noch nicht \ldots}
\[
...
\subsection[Satt: Äquivalenzen zu: $X$ ist reflexiv]{Satz} % (fold)
\label{sub:518}
...
\]
Wir schreiben $\Fred(\mathcal{H}) := \set[T \in \mathcal{L}(\mathcal{H})]{T \text{ ist Fredholm}}$. \todo{RevChap9}
% subsection 91 (end)
