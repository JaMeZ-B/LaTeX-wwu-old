...
\label{sub:318}
Für eine $K$-Algebra $A$ heißt $A^e = A \otimes_K A^\mathrm{op}$ die \Index{einhüllende Algebra} von $A$. Sei $A \subseteq B$ eine Unteralgebra. Dann wird $B$ in 
...
\label{sub:321}
Ist $A$ eine endlich dimensionale zentral einfache $K$-Algebra, dann ist $A^e = A \otimes A^\mathrm{op} \simeq M_n(K)$ mit $n= \dim A$.
\minisec{Beweis}
...
\begin{enumerate}[(i)]
	...
	\item Ist $E$ Zerfällungskörper für $A$ ,dann auch für $A^\mathrm{op}$: 
	\[
		A \otimes E \otimes E \otimes A^\mathrm{op} = M_n(E) = M_r(E) \otimes (E \otimes A^\mathrm{op}) = M_r(E \otimes A^\mathrm{op})
	\]
	Daher ist $E \otimes A^\mathrm{op} \simeq M_s(E)$ mit $r \cdot s = n$.
...
\minisec{Beweis}
\begin{description}
	\item["$\Leftarrow$":] Sei $E \subseteq M_r(D) = A$ mit $C_A(E)=E$. Dann ist $A = \End(V)$, wobei $V$ ein $r$-dimensionaler $D^\mathrm{op}$-Vektorraum ist. Dann ist
	$V$ auch $D^\mathrm{op} \otimes _K E$-Modul, da 
	\[
		(d \otimes e)x = d \cdot e \cdot x = e \cdot d \cdot x 
	\]
	für $d \in D^\mathrm{op}$, $ e \in E \subseteq A$, $x \in V$. Da $D^\mathrm{op} \otimes E$ einfach ist, ist die Darstellung von $D^\mathrm{op} \otimes E$ auf $V$ treu,
	\marginnote{treu, da Kern sonst Unteralgebra} d.h. wir können $D^\mathrm{op} \otimes E$ mit dem entsprechenden Endomorphismenring identifizieren. Da 
	$D^\mathrm{op} \otimes E$ endlich dimensional und einfach über $K$ ist, ist nach den Sätzen von Wedderburn $V$ als $D^\mathrm{op} \otimes E$ als Modul halbeinfach und
	daher ist der Dichtheitssatz anwendbar:
	
	Es ist $A = \End_{D^\mathrm{op}}(V)$, also ist $\End_{D^\mathrm{op} \otimes E} (V) = C_A(E) = E$ \marginnote{zusätzlich auch mit $E$ vertauschen}. 
	Also können wir $V$ auch als endlich dimensionalen $E$-Vektorraum 
	betrachten. Nach dem Dichtheitssatz ist $D^\mathrm{op} \otimes E = \End_E(V) \simeq M_n(E)$ für $n = \dim_E V$. Daher ist $E$ Zerfällungskörper von $D^\mathrm{op}$ und
	damit von $D$.
	\item["$\Rightarrow$":] Ist umgekehrt $D \otimes E \simeq M_n(E)$, dann auch $D^\mathrm{op} \otimes E \simeq M_n(E)$. Ist $V$ ein irreduzibler 
	$D^\mathrm{op} \otimes E$-Modul, dann auch ein $M_n(E)$-Modul, d.h. $V$ ist $n$-dimensionaler $E$-Vektorraum und $\End_E(V) = D^\mathrm{op} \otimes E$. Aber $V$ ist 
	auch ein endlich dimensionaler $D^\mathrm{op}$-Vektorraum also etwa $r = \dim_{D^\mathrm{op}}V$. Dann ist $E \subseteq \End_{D^\mathrm{op}}(V)$. Daher ist 
	\[
		E \subseteq C_{\End_{D^\mathrm{op}}(V)}(E) \subseteq C_{\End_E(V)} \enbrace*{D^\mathrm{op} \otimes E} = E.
	\]
...
\minisec{Beweis}
$E= A \otimes_K B^\mathrm{op}$ ist einfach und endlich dimensional. Nach Wedderburn ist jeder $E$-Modul halbeinfach und alle einfachen $E$-Moduln sind isomorph. Daher sind
...
Zentrum $E$ und $B'' = C_A(B') = C_A(C_A(B)) = B$.
% subsection 336 (end)
