...
\section{Moduln: Halbeinfache Moduln, freie, projektive und injektive Moduln, Tensorprodukte} % (fold)
\label{sec:2}
...
\label{sub:228}
Ist $(R,+,\cdot ,1)$ ein Ring, dann ist $R^{\mathrm{op}} := R(+, *, 1)$ ein Ring mit $r * s = s \cdot r$, der \bet{entgegengesetzte Ring}\index{entgegengesetzter Ring}.
...
\label{sub:234}
Für eine beliebige Menge $I$ heißt der (Links-)$R$-Modul $\mathcal{F}_I = \bigoplus_{I} R$ der freie $R$-Modul mit Basis der Mächtigkeit $\abs*{I}$. 
\bigskip \\
...
\subsection{Satz} % (fold)
\label{sub:242}
Sei $D$ ein Hauptidealring, $L \subseteq D^m$ ein Untermodul, dann $L \simeq D^n$ für ein $n \ge m$, d.h. Untermoduln freier Moduln sind frei.
\minisec{Beweis}
...
\section{Algebren: Einfache Algebren, Tensorprodukte, äußere Algebra} % (fold)
\label{sec:3}
