...
\begin{enumerate}[(i)]
	\item\begin{enumerate}[(a)]
		...
		\]
		Gruppenhomomorphismus: $\rho_{g h} (x) = x \cdot g \cdot h = \rho_h \circ \rho_g (x)$.\marginnote{Beachte: Die Addition in $\Sym(X)$ ist die \emph{linksseitige} 
...
$k \le m$, also ist $[G : Z_G(x)] = p^{m-k}$. Wegen $p \mid \abs*{G} $ und $\abs*{Z(G)} \ge 1 $ folgt $p \mid \abs*{Z(G)} $. \bewende
% subsection 16 (end)
