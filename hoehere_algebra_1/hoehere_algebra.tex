...
\end{itemize}
\minisec{Alternative Definition}
...
\begin{enumerate}[(i)]
	...
	\]
	Dann ist $G_x = \set[g \in G]{g \cdot x \cdot g ^{-1}= x} =\set[g \in G]{g \cdot x =x\cdot g} = Z_G(x) $ der \Index{Zentralisator} von $x$ in $G$. Der Kern der Wirkung
	$Z(G) = \set[g \in G]{x \cdot g = g \cdot x \text{ für alle } x \in G}$ ist das \Index{Zentrum}  von $G$ .
	...
	\bet{Bemerkung:} Die rechtsseitige Gruppenwirkung  $\tau : G  \to \Aut(G)$, $g \mapsto \tau_g$ mit $\tau_g(x) = g^{-1}  \cdot x \cdot  g$ 
...
\bet{Spezialfall:} Wirkung von $G$ durch Konjugation auf sich selbst. $\kappa_g(x) = g  \cdot  x \cdot g^{-1}$.
% subsection 14 (end)
