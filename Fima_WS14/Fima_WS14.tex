...
\uline{"$\Rightarrow$":} etwas schwierig, math. Argument ist der Trennungssatz von Minkowski. \marginnote{vgl. Funktional A1nalysis}
%sub end
...
$\G^*:=\{(H\cdot S^*)(N)~|~H\in\mc{H} \}$ bezeichnet die Menge der möglichen Gewinne, notiert in Anteile des Numeriare Assets, die beim Handel entsprechend einer selbstfinanziernden Handelsstrategie erzielt werden können.
...
\begin{aligned}
	...
	&= \E^*(\sprod{H(k)}{\Delta S^*(k)}~|~\F_{k-1}) = \E^*(\sum_{j=1}^{d}H_j(k)\Delta S_j^*(k)~|~\F_{k-1})\\
