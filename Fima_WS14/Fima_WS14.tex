...
Im Markt seu $C$ die Auszahlung eines Claims zum Zeitpunkt $N$.
...
\begin{enumerate}[(i)]
	...
	\[
	M_n=\E M_0 + (H\cdot S^*)(n) ~\forall ^\le n\le N
	\]
...
Es gilt:
\[
S(n)=S(0)u^{Z_n}d^{n-Z_n},~Z_n=\sum_{k=1}^{n}K_k
\]
...
\[
v^*(N,k)=\enbrace{\frac{1}{1+\rho}}^N\cdot g(S(0)u^kd^{N-k}),~k=0\dt{,}N
\]
'Initialisierung':\\
...
$v^*(n-1,k)$ ist der Preisin Einheiten des Numeraire Assets und $v^*(n,k+X_n)$ zum Zeitpunkt $n+1$ zu hedgen.
...
\[
H(n)=\sum_{k=0}^{n-1}h(n,k)\mathbb{1}_{\penbrace{Z(n-1)=k}}1
\]
%sub end
...
\[
\Delta S^*(1)=S^*(1)-S^*(0)=(\frac{uS_0}{1+\rho}-S_0,\frac{mS_0}{1+\rho}-S_0,\frac{dS_0}{1+\rho}-S_0)=S_0(\frac{uS_0}{1+\rho}-1,\frac{mS_0}{1+\rho}-1,\frac{dS_0}{1+\rho}-1)=S_0\cdot R
\]
Ein Claim $C$ entspricht einem Vektor $C=(c_1,c_2,c_3)$
\[
...
\]
$\penbrace{(V_0,H)~|~V_0+HS_0R_i\ge c_i^*,~i=1,2,3}$ ist der Durschnitt von 3 Halbräumen im $\R^2$
\begin{center}
