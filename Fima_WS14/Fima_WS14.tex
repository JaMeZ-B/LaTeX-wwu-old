...
\[
V_0\le 0,~ V^*(N)=\frac{V(N)}{S_0(N)}\le 0 \text{ und } \Pw(V^*(N)-V_0 > 0) > 0
\]
...
\[
V_0+(H\cdot S^*)(N)\le 0 \text{ und } \Pw(H\cdot S^*(N)>0)>0
\]
...
\[
V^*_N=(H\cdot S^*)(N)\ge 0 \text{ und } \Pw(H\cdot Sq*(N)>0)>0
\]
...
\begin{equation*}
\begin{aligned}
	0\le H(n)\frac{\Delta S^*(n)}{S^*(n-1)}&=H(n)(\frac{S^*}{S^*(n-1)}-1)\\
	&= H(n)(\frac{1}{1+\rho}u^{X_n}d^{1-X_n}-1)
\end{aligned}
\end{equation*}
Daher folgt:
\begin{equation*}
...
\uline{"$\Rightarrow$":} etwas schwierig, math. Argument ist der Trennungssatz von Minkowski. %\marginnote{vgl. Funkanalysis}
%sub end
...
$\G^*:=\{(H\cdot S^*)(N)~|~H\in\mc{H} \}$ bezeichnet die Menge der möglichen Gewinne, notiert in Anteile des Numeriare Assets, die beim Handel entsprechend einer selbstfinaziernden Handelsstrategie erzielt werden können.
