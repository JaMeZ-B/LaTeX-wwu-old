...
\[
	\begin{tikzcd}
		H_n \enbrace*{X^{(n)}, X^{(n-1)};R} \rar["\partial_n"] & H_{n-1}(X^{(n-1)};R) \dar[equal] \rar & \cdots\\
		& H_{n-1}\enbrace*{X^{n-1};R} \rar["\enbrace*{j^{(n-1)}}_*"] & H_{n-1} \enbrace*{X^{(n-1)}, X^{(n-2)};R} \rar["\partial_{n-1}"] & H_{n-2} \enbrace*{X^{(n-2)};R} 
		\rar & \cdots \\ 
		& & & H_{n-2} \enbrace*{X^{(n-2)};R}  \rar["\enbrace*{j^{(n-2)}}_*"] & H_{n-2} \enbrace*{X^{(n-2)}, X^{(n-3)}} 
	\end{tikzcd}
\]
...
\begin{enumerate}[a)]
	\item 
	\[
		\enbrace*{\coprod_{i \in I^{(n)}} Q_i^{(n)}, \coprod_{i \in I^{(n)}} q_i^{(n)}} \colon \enbrace*{\coprod_i D^n, \coprod_i S^{n-1}}  \longrightarrow 
		\enbrace*{X^{(n)}, X^{(n-1)}} 
	\]
