...
\begin{enumerate}[(i)]
	...
	alle $n$ offen ist.
\end{enumerate}
Eine Abbildung $f \colon X \to Y$ zwischen \CW-Komplexen heißt \Index{zellulär}, falls $f\enbrace*{X^{(n)}}\subseteq Y^{(n)}$ für alle $n \in \mathds{N}$ gilt. 
% subsection 113 (end)
...
\begin{enumerate}[(i)]
	...
	gefordert --- sie ist nicht notwendig eindeutig. Hat man $q_i^{(n)}$ und $Q_i^{(n)}$ \emph{gewählt}, so heißt $q_i^{(n)}$ die \Index{anklebende Abbildung} der $i$-ten
	...
	\bet{abgeschlossenen Zellen}\index{abgeschlossene Zelle} und sind als Abschluss von offenen Zellen unabhängig von der Wahl der $Q_i^{(n)}$.
...
\begin{enumerate}[(i)]
	...
	\item Der $n$-dimensionale reelle projektive Raum $\mathds{R}P^n$ kann definiert werden durch\index{reell projektiver Raum} 
	\[
...
\]
\minisec{Beweis}
Wir schreiben die Paarsequenz für $\enbrace*{X^{(n)},X^{(n-1)}}$ dreimal übereinander und erhalten:
\[
	\tikzexternaldisable
	\begin{tikzpicture}[baseline= (a).base]
	\node[scale=0.81] (a) at (0,0){
	\begin{tikzcd}[font=\normalsize,column sep=2.0em,arrows=thick]
		H_n \enbrace*{X^{(n)}, X^{(n-1)}} \rar["\partial_n"] \arrow[rrd, bend right,"\partial_n^{\cell}"',Firebrick3]& H_{n-1}(X^{(n-1)}) \dar[equal] \rar & \cdots\\
		& H_{n-1}\enbrace*{X^{n-1}} \rar["j^{(n-1)}_*"] & H_{n-1} \enbrace*{X^{(n-1)}, X^{(n-2)}} \rar["\partial_{n-1}"] 
		\arrow[rrd, bend right,"\partial_{n-1}^{\cell}"',Firebrick3] & H_{n-2} \enbrace*{X^{(n-2)}} \dar[equal] \rar & \cdots \\ 
...
\subsection[Lemma über $\enbrace{\coprod Q_i^{(n)}, \coprod q_i^{(n)}}_* $ und $H_k \enbrace{\coprod_i D^n, \coprod_i S^{n-1};R}$]{Lemma} % (fold)
\label{sub:125}
Sei $X$ ein \CW-Komplex. Wähle $q_i^{(n)}$ und $Q_i^{(n)}$ für die Zellstruktur auf $X$. Dann gilt
\begin{enumerate}[a)]
	\item 
	\[
		\enbrace*{{\textstyle\coprod\limits_{i \in I^{(n)}}} Q_i^{(n)}, {\textstyle\coprod\limits_{i \in I^{(n)}}} q_i^{(n)}} 
...
\begin{enumerate}[a)]
	...
	\]
	Dann sind 
	$X^{(n-1)} \hookrightarrow X_+^{(n-1)} \hookrightarrow X_{++}^{(n-1)}$ Homotopieäquivalenzen, da $\coprod_{i \in I^{(n)}} Q^{(n)}_i$ ein Homöomorphismus auf den offenen 
...
Die $\enbrace{Q_i^{\scriptscriptstyle(n-1)}, q_i^{\scriptscriptstyle(n-1)}}$ induzieren einen Homöomorphismus
$\nicefrac{\coprod_i D^{n-1}}{\coprod_i S^{n-2}} \cong \nicefrac{X^{(n)}}{X^{(n-1)}}$. Durch Komposition mit der Projektion auf die $j$-te Zelle erhalten wir die folgenden 
Abbildung
\[
...
$\tau_C \colon F(C) \to G(C)$ für alle Objekte $C$ von $\mathcal{C}$ ein Isomorphismus in $\mathcal{D}$ ist. \todo{RevChap12}
% subsection 127 (end)
...
\begin{enumerate}[a)]
	...
	\item Für $k <n$ induziert die Inklusion $l_n\colon X^{(n)} \hookrightarrow X$ einen Isomorphismus $(l_n)_* \colon H_k(X^{(n)};R) \to H_k(X;R)$.
\end{enumerate}
\minisec{Beweis}
\begin{enumerate}[a)]
	...
	\[
		\begin{tikzcd}[column sep=small]
			\underset{\textstyle\stackrel{\mbox{{\scriptsize\ref{sub:125}}}}{=} 0}{H_{k+1}\enbrace*{X^{(n)}, X^{(n-1)};R}} \rar &  H_k(X^{(n-1)};R) \rar["\cong"] & 
			H_k(X^{(n)};R) \rar 
			& H_k \underset{\textstyle\stackrel{\mbox{\scriptsize\ref{sub:125}}}{=}0}{\enbrace*{X^{(n)}, X^{(n-1)};R} }
		\end{tikzcd}
	\]
	...
	\[
		\begin{tikzcd}[column sep=small]
			\underset{\textstyle\stackrel{\mbox{\scriptsize\ref{sub:125}}}{=}0}{H_{k+1} \enbrace*{X^{(n+1)},X^{(n)};R}} \rar & H_k \enbrace*{X^{(n)};R} \rar["\cong"] 
			& H_k \enbrace*{X^{(n+1)};R} \rar 
			& \underset{\textstyle\stackrel{\mbox{\scriptsize\ref{sub:125}}}{=}0}{H_k \enbrace*{X^{(n+1)},X^{(n)};R}   }
		\end{tikzcd}
	\]
	Für $m \ge n >k$ induziert die Inklusion $X^{\scriptscriptstyle(n)} \hookrightarrow X^{\ssbrace{m}}$ daher einen Isomorphismus $H_k(X^{(n)};R) \to H_k(X^{(m)};R)$. Ist $\dim X$ endlich, so folgt
	die Behauptung. 
...
\subsection[Bemerkung: $n$-te Bettizahl]{Bemerkung} % (fold)
\label{sub:1211}
Ist $X$ ein \CW-Komplex mit endlich vielen $n$-Zellen, so ist $H_n(X)$ ein endliche erzeugter $\mathds{Z}$-Modul. Man definiert dann die \bet{$\mathbf{n}$-te Bettizahl}\index{n-te Bettizahl@$n$-te Bettizahl} von $X$ als $b_n := \Rg H_n(X)$. ist $X$ ein endlicher \CW-Komplex, so definiert man die Eulercharakteristik von $X$ als
\[
...
\[
	...
	\end{pmatrix}
\]
Es folgt, dass $y_+$ ein Erzeuger von $H_k(D^k,S^{k-1})$ ist und $y_- = (-1)^{k+1} y_+$ ist. In \eqref{eq:1213} wird $y_+$ auf $\enbrace*{Q^{(k)},q^{(k)}}_*(y_+)$
...
\label{sub:1314}
Seien $v_0, \ldots ,v_p \in \abs*{\Delta^n}$. Dann bezeichnen wir mit $[v_0, \ldots ,v_p]$ den singulärer $p$-Simplex mit $\sum_{i=0}^{p} t_i e_i \to \sum_{i=1}^{p} t_i v_i$.
...
\label{sub:1316}
Sei $[v_0, \ldots , v_p]$ ein affiner singulärer Simplex in $\abs*{\Delta^n}$. Dann gilt:
\begin{enumerate}[(i)]
	\item $\diam \enbrace*{[v_0, \ldots ,v_p]}= \max \set[\norm{v_i - v_j} ]{0 \le i,j\le p}$
	\item Es ist 
	\[
		(B_n)_{\abs*{\Delta^n}} \enbrace*{[v_0, \ldots ,v_p]} = \sum_{\sigma \in \sum_{\set{v_0,\ldots ,v_p}}} \sgn(\sigma)  
