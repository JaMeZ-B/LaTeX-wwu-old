...
\begin{enumerate}[(i)]
	...
	alle $n$ offen ist.
\end{enumerate}
Eine Abbildung $f \colon X \to Y$ zwischen \CW-Komplexen heißt \Index{zellulär}, falls $f\enbrace*{X^\ssbrace{n}}\subseteq Y^{(n)}$ für alle $n \in \mathds{N}$ gilt. 
% subsection 113 (end)
...
\]
Also ist $x=0$ in $H_* \enbrace*{C_*^\mathcal{U}(X;R)}$. \bewende \todo{RevChap13}
% subsection 1312 (end)
...
\[
	\enbrace*{s_{n-1} \circ \partial_n + \partial_{n+1}' \circ s_n} \circ \partial_{n+1} = \partial_{n+1}' \circ  \enbrace*{s_n \circ \partial_{n+1} + \partial_{n+2}' \circ s_{n+1}}  
\]
...
Singuläre Simplizes dieser Form heißen \Index{affin}. 
% subsection 1314 (end)

\subsection[Definition: Durchmesser eines Simplizes]{Definition} % (fold)
\label{sub:1315}
