...
Wir zeigen $d(R_0)= -1$ durch Induktion nach $n$.\todo{RevChap10}
\begin{description}
	\item[Induktionsanfang:] Sei $i_- \colon \set{\mathrm{pt}} \to S^0$ die Abbildung mit $\im i_- = \set{-1}$ und $i_+ \colon\set{\mathrm{pt}} \to S^0 $ mit $\im i_+ = \set{+1}$. Dann ist
	\begin{align*}
		H_0(S^0) &= \set[(i_-)_*(a)+ (i_+)_*(b)]{a,b \in H_0(\set{\mathrm{pt}})\rule{0cm}{0.8em}} \qquad \text{und}\\
		\tilde{H}_0(S^0) &= \set[(i_-)_*(a)- (i_+)_*(a)]{a \in H_0(\set{\mathrm{pt}})\rule{0cm}{0.8em}}
	\end{align*}
	Nun ist
	\begin{align*}
	...
	Isomorphismus ist. Da die Randabbildung natürlich ist, erhalten wir ein kommutatives Diagramm\todo{fertig stellen}
	\[
...
\missingfigure{$S^2$ mit Vektorfeld} 
% subsection 107 (end)
...
\subsection[Bemerkung: Funktorialität des zellulären Kettenkomplexes und der zellulären Homologie]{Bemerkung} % (fold)
\label{sub:124}
...
\subsection[Lemma über die Homologie des $n$-Gerüsts und die von $X^\ssbrace{n} \hookrightarrow X$ induzierte Abbildung]{Lemma} % (fold)
\label{sub:129}
