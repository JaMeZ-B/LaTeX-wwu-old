...
\begin{enumerate}[(i)]
	...
	$q^\ssbrace{n} \colon S^{n-1} \to \mathds{R}P^{n-1}$ die Projektion, so erhalten wir einen Pushout
	\[
		\begin{tikzcd}
...
\[
	...
	\begin{tikzcd}[font=\normalsize,column sep=2.4em,arrows=thick]
...
\begin{enumerate}[a)]
	...
	Damit folgt \begin{enumerate}[(1)]
		...
		trivial ist.
	\end{enumerate}
	Da für $m \ge n >k$ die Abbildung $H_k(X^\ssbrace{n};R) \to H_k(X^\ssbrace{m};R)$ ein Isomorphismus ist, ergibt sich damit die Behauptung. \bewende \todo{RevChap12}
\end{enumerate}
% subsection 129 (end)
...
\[
	\begin{tikzcd}[column sep=huge, row sep=large]
		...
		H_{k+1}(D^{k+1},S^k) \uar["\enbrace*{Q^\ssbrace{k+1},q^\ssbrace{k+1}}_*","\cong"'] \rar["\partial^{D^{k+1},S^k}","\cong"'] & \tilde{H}_k(S^k) \uar["q^\ssbrace{k+1}_*"] \rar["j_*"] 
