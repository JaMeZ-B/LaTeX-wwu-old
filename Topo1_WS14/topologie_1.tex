...
\end{align*}
Also ist $(f_*)^{-1}=(f ^{-1})_*$. Insbesondere ist $f_*$ ein Isomorphismus. \bewende \todo{Rev Chap 7}
% subsection 71 (end)

\subsection[Definition: Homotopieäquivalenz]{Definition} % (fold)
\label{sub:72}
...
\minisec{Beweis}
\emph{siehe \ldots } \todo{hinzufügen} \bewende
% subsection 73 (end)
...
\[
	H_n(X;R) \cong H_n(\set{x_0};R) = \begin{cases}
		R, &\text{ falls }n=0\\
		0, &\text{ sonst}
	\end{cases} \bewende
\]
...
\subsection[Proposition: Die induzierten Abbildungen von kettenhomotopen Kettenabbildungen sind gleich]{Proposition} % (fold)
\label{sub:77}
Seien $f_*,g_* : (C_*,d_*) \to (C_*',d_*')$ Kettenabbildungen. Sind $f_*$ und $g_*$ kettenhomotop, so gilt $H_n(f_*) = H_n(g_*)$ für alle $n$.
\minisec{Beweis}
...
$h_n : C_n(X;R) \to C_{n+1}(X \times [0,1];R)$ mit 
\begin{equation*}
