...
\begin{description}
	...
	\begin{description}
		...
		\[
			i_{n-1} \enbrace[\big]{d_n(a)} = d_n' \enbrace[\big]{i_n(a)} = d_n' \enbrace*{v'- d_{n+1}(a')}  = d_n'(v') = 0 
		\]
...
Also
\begin{align*}
	\partial_n \enbrace*{\enbrace*{f,f|_A}_*(x'') } = \benbrace*{\partial_n^Y \enbrace*{f_*(v')} } = \benbrace*{f_* \circ \partial_n^X(v')} = f_* \partial_n(X'') \bewende  
\end{align*}
% subsection 813 (end)
