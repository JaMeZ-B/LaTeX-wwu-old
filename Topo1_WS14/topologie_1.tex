...
\label{sub:51}
Sei $X$ ein topologischer Raum. Sei $S_n(X)$ die Menge aller stetigen Abbildungen $\sigma : \abs*{\Delta^n} \to X$. Elemente von $S_n(X)$ heißen \Index{singuläre Simplizes} 
...
\]
für alle $n\ge 1$ ist. Also kommutiert folgendes Diagramm\todo{RevChap6}
\[
	\begin{tikzcd}
...
\label{sub:710}
Für $j=0, \ldots ,n$ seien $k_{n,j} : \abs{\Delta^{n+1}} \to \abs{\Delta^n} \times [0,1]$ und $\iota_{n,j} : \abs*{\Delta^{n-1}} \to \abs*{\Delta^n}$\marginnote{
...
\begin{align*}
	...
	+ \underbrace{(\sigma\times \id) \circ i_1}_{= i_0 \circ \sigma}  - \underbrace{(\sigma\times \id) \circ i_0}_{= i_1 \circ \sigma} \\
...
\label{sub:814}
Für eine Abbildung $(f,f|_A) : (X,A) \to (Y,B)$ erhalten wir ein kommutierendes Diagramm:
...
\label{sub:816}
Sei $f : (X,A) \to (Y,B)$ eine Abbildung von Paaren. Seien von den drei Abbildungen
\begin{enumerate}[(i)]
	\item $(f|_A)_* :  H_n(A;R) \to H_n(B;R)$
	\item $f_* : H_n(X;R) \to H_n(Y;R)$
	\item $(f,f|_A)_* : H_n(X,A;R) \to H_n(Y,B;R)$
\end{enumerate}
...
$\enbrace*{X \setminus L, A \setminus L} \xrightarrow{j} (X,A)$ einen Isomorphismus $j_* : H_n(X \setminus L, A \setminus L;R) \to H_n(X,A;R)$.
\todo{RevChap9}
% subsection 91 (end)

\subsection{Satz (Mayer-Vietoris Folge)} % (fold)
\label{sub:92}
Seien $A,B \subseteq X$ offen und $X=A \cup B$. Seien $i_A : A \hookrightarrow X$, $i_B : B \hookrightarrow X$, $j_A : A \cap B \hookrightarrow A$ und 
$j_B : A \cap B \hookrightarrow B$ die Inklusionen. Dann gibt es eine Randabbildung $\partial_n = \rand{X=A\cup B}_n : H_n(X;R) \to H_{n-1}(A \cap B;R)$, sodass
...
\[
	...
	\end{tikzcd}
\]
dabei sind $l_B : (B,\emptyset) \to (B,A \cap B)$ und $l_X : (X, \emptyset) \to (X,A)$ die Inklusionen von Paaren.
Definiere nun $\partial_n : H_n(X;R) \to H_{n-1}(A\cap B;R)$ durch
\[
...
\begin{itemize}
	...
	\begin{description}
		...
		\begin{align*}
			(j_B)_*(z) & =(j_B)_*(x) = b \\
			(j_A)_*(z) &= (j_A)_*(x) + (j_A)_* \enbrace*{\rand{B,A \cap B}(y)}_n = (j_A)_*(x) + \rand{X,A}_n \circ  \enbrace*{i_B, j_A}_*(y) = (j_A)_*(x) + a -
			(j_A)_*(x) = a  
		\end{align*}
		...
		\begin{align*}
			\enbrace*{(i_A)_* - (i_B)_*} \begin{pmatrix}
				(j_A)_* \\ (j_B)_*
			\end{pmatrix}  (x) = (i_A)_* \circ (j_A)_* (x) - (i_B)_* \circ (j_B)_*(x) = (i_A \circ j_A)_* (x) - (i_B \circ j_B)_* (x) = 0
		\end{align*}
...
\label{sub:95}
Sei $X$ ein nichtleerer topologischer Raum. Sei $p : X  \to \set{\mathrm{pt}}$ die Projektion auf den Ein-Punkt-Raum. die \Index{reduzierte singuläre Homologie} von $X$ ist definiert
durch 
\[
...
\minisec{Bemerkung}
\begin{itemize}
	\item Es gilt $\tilde{H}_k(X;R) =H_k(X;R)$ für $k>0$ und $H_0(X;R) = R \oplus \tilde{H}_0(X;R)$
...
\label{sub:102}
Jede stetige Abbildung $f : D^n \to D^n$ besitzt einen Fixpunkt.
\minisec{Beweis}
...
\subsection[Definition: Abbildungsgrad]{Definition} % (fold)
\label{sub:103}
Sei $f : S^n \to S^n$ eine stetige Abbildung. Da $\tilde{H}_n(S^n) \cong Z$ ist, gibt es eine ganze Zahl $d(f)$, so dass $f_*(x)= d(f) \cdot x$ für alle 
...
\label{sub:105}
Sei $i \in \set{0, \ldots ,n}$ und $R_i : S^n \to S^n$ die Spiegelung $R_i(x_0, \ldots , x_n) = (x_0, \ldots , - x_i, \ldots , x_n)$. Dann ist $d(R_i) = -1$.
\minisec{Beweis}
...
\begin{description}
	\item[Induktionsanfang:] Sei $i_- : \set{\mathrm{pt}} \to S^0$ mit $\im i_- = \set{-1}$ und $i_+ : \set{\mathrm{pt}} \to S^0 $ mit $\im i_+ = \set{+1}$. Dann ist
	\begin{align*}
	...
	\[
		...
		\end{tikzcd}
	\]
	dabei ist $i: S^{n-1} \to D^n_- \cap D_+^n$ die Homotopieäquivalenz $i(x_0, \ldots , x_{n-1}) = (x_0, \ldots , x_{n-1},0)$.
...
\label{sub:106}
Der Grad der Punktspiegelung $R : S^n \to S^n$ $x  \mapsto -x$ ist $(-1)^{n+1}$.
\minisec{Beweis}
...
\label{sub:107}
Sei $M$ eine $C^\infty$-Mannigfaltigkeit. Ein \Index{Vektorfeld} auf $M$ ist eine stetige Abbildung $v : M \to \Tmap M$ mit $v(x) \in \Tmap_x M$ für alle $x \in M$.
Für $M=S^n$ entspricht ein Vektorfeld genau einer stetigen Abbildung $v : S^n \to \mathds{R}^{n+1}$ mit $\sprod*{v(x)}{x} =0$ für alle $x \in S^n$.
...
$\abs*{z_1}^2 + \ldots + \abs*{z_k}^2 = 1$. Ein Vektorfeld entspricht dann einer stetigen Abbildung $v : S^n \to \mathds{C}^k$, so dass für das kanonische komplexe 
Skalarprodukt $\sprod*{\cdot }{\cdot}_\mathds{C} $ auf $\mathds{C}^k$ gilt
\[
	\sprod*{z}{v(z)}_\mathds{C} \in i \mathds{R} 
\]
Definiere nun $v : S^n \to \mathds{C}^k$ durch $v(z) := i z$. Dann $i z\not= 0$ für alle $z \in S^n$ und $\sprod*{z}{i z}= -i \norm[2]{z} \in i \mathds{R}$. Sei umgekehrt
$v : S^n \to \mathds{R}^{n+1}$ ein Vektorfeld ohne Nullstellen. Zu zeigen: $d( \text{Punktspiegelung }R)=1$ (Da $d(R)=(-1)^{n+1}$ folgt dann die Behauptung).
Zeige dazu: $R$ ist homotop zu $\id$. Seien $H, K : S^n \times [0,1] \to S^n$ definiert durch 
\[
...
\begin{enumerate}[a)]
	...
	\[
		\begin{tikzcd}[column sep=1.3em]
			...
			& \bigoplus\limits_{\textstyle\tilde{H}_i \enbrace*{S^n \setminus f \enbrace[\big]{[\nicefrac{1}{2},1] \times [0,1]^k} } }
...
\label{sub:111}
Ein kommutatives Diagramm von topologischen Räumen der Form \todo{RevChap11}
\begin{equation*}
	\begin{tikzcd}
...
\noindent\begin{minipage}{0.65\textwidth}
	...
	\centering{$\forall \hat{f} \colon X \to \hat{Z}$, $\hat{g}: Y \to \hat{Z}$ mit 
...
Betrachte dazu den Raum $X \cup_A Y := \nicefrac{X \coprod Y}{f(a) \sim g(a) \forall a \in A}$ mit der Quotiententopologie. Sind $\overline{f} : Y \to X \cup_A Y $  und 
$\overline{g} : X \to X \cup_A Y$ die von den Inklusionen $X \hookrightarrow X \coprod Y$ und $Y \hookrightarrow X \coprod Y$ induzierten Abbildungen. So ist 
\[
	\begin{tikzcd}
		A \rar["f"] \dar["g"'] & X \dar["\overline{g}"] \\
...
aus der universellen Eigenschaft können benutzt werden um zu zeigen, dass der Pushout eindeutig bis auf kanonischen Homöomorphismus ist.
% subsection 111 (end)

\subsection[Definition: \CW-Komplex]{Definition} % (fold)
\label{sub:112}
Ein \bet{\CW-Komplex}\index{CW-Komplex@\CW-Komplex} ist ein topologischer Raum $X$ zusammen mit einer Filtrierung der Unterräume von $X$: 
\[
...
\begin{enumerate}[(i)]
	\item \Index{Zellstruktur}: Zu jedem $n \in N$ gibt einen Pushout von topologischen Räumen 
	\[
	...
	\item \Index{Schwache Topologie}: Es ist $X= \bigcup_{n \in \mathds{N}} X^{(n)}$ und $U \subseteq X$ ist genau dann offen, wenn $U \cap X^{(n)} \subseteq X^{(n)}$ für 
	alle $n$ offen ist.
\end{enumerate}
Eine Abbildung $f \colon X \to Y$ zwischen \CW-Komplexen heißt \Index{zellulär}, falls $f \enbrace*{X^{(n)}} \to Y^{(n)}$ für alle $n \in \mathds{N}$. 
% subsection 112 (end)
...
\begin{enumerate}[(i)]
	...
	gefordert --- sie ist nicht notwendig eindeutig. Hat man $q_i^{(n)}$ und $Q_i^{(n)}$ \emph{gewählt}, so heißen $q_i^{(n)}$ die \Index{anklebende Abbildung} der $i$-ten
	$n$-Zelle und $Q_i^{(n)}$ die \Index{charakteristische Abbildung} der $i$-ten $n$-Zelle.
	...
	$X^{(n)} \setminus X^{(n-1)}$ identifizieren. Die Wegzusammenhangskomponenten heißen die \bet{offenen Zellen}\index{offene Zelle} von $X$. Damit ist jeder \CW-Komplex 
	...
	\bet{abgeschlossenen Zellen}\index{abgeschlossene Zelle} und sind als Abschluss von offenen Zellen unabhängig von der Wahl der $q_i^{(n)}$, $Q_i^{(n)}$.
...
\begin{enumerate}[(i)]
	...
	\[
		\begin{tikzcd}
			S^{n-1} \rar \dar & \enbrace*{S^n}^{(n-1)}=\set{(1,0,\ldots ,0)}  \dar \\
			D^n \rar & S^n  
		\end{tikzcd}
	\]
	...
	\[
		f \enbrace*{x_1, \ldots ,x_n} = \benbrace*{x_1 : \ldots : x_n : \sqrt{1- \sum\nolimits_{i=1}^{n} x_i^2}} 
	\]
	...
	\[
		f \enbrace*{z_1, \ldots ,z_n} = \benbrace*{z_1 : \ldots : z_n : \sqrt{1- \sum\nolimits_{i=1}^{n} \abs{z_i}^2}} 
	\]
...
\]
der \bet{$\mathbf{n}$-te zelluläre Kettenmodul}\index{n-ter zellulärer Kettenmodul@$n$-ter zelluläre Kettenmodul} von X mit Koeffizienten in $R$. Die $n$-te Randabbildung
...
\[
	\begin{tikzcd}
		...
		& H_{n-1}\enbrace*{X^{n-1}} \rar["\enbrace*{j^{(n-1)}}_*"] & H_{n-1} \enbrace*{X^{(n-1)}, X^{(n-2)}} \rar["\partial_{n-1}"] & H_{n-2} \enbrace*{X^{(n-2)}} 
		\rar & \cdots \\ 
		& & & H_{n-2} \enbrace*{X^{(n-2)}}  \rar["\enbrace*{j^{(n-2)}}_*"] & H_{n-2} \enbrace*{X^{(n-2)}, X^{(n-3)}} 
	\end{tikzcd}
\]
...
\subsection{Definition} % (fold)
\label{sub:123}
$\enbrace*{C_*^{\cell}(X;R),\partial_*^{\cell}}$ heißt der \Index{zelluläre Kettenkomplex} von $X$ über $R$. Seine Homologie $H_*^{\cell}(X;R)$ heißt die \Index{zelluläre 
Homologie} von $X$.
% subsection 123 (end)
...
\begin{enumerate}[a)]
	\item 
	\[
		\enbrace*{\coprod_{i \in I^{(n)}} Q_i^{(n)}, \coprod_{i \in I^{(n)}} q_i^{(n)}}_* \colon \enbrace*{\coprod_i D^n, \coprod_i S^{n-1};R}  \longrightarrow 
		\enbrace*{X^{(n)}, X^{(n-1)};R} 
	\]
...
\begin{enumerate}[a)]
	...
	\[
		X^{(n-1)}_{++} := X^{(n)} \setminus \bigcup_{i \in I^{(n)}} Q_i^{(n)}(\set{0}) \enspace\text{ und } \enspace
		X^{(n-1)}_+ := X^{(n)} \setminus \bigcup_{i \in I^{(n)}} Q_i^{(n)}\enbrace*{\frac{1}{2} D^n}
	\]
	Dann sind 
...
\subsection{Bemerkung} % (fold)
\label{sub:126}
\missingfigure{Diagramm mit 3 Zeilen}
...
\label{sub:127}
Eine natürliche Transformation $\tau$ zwischen Funktoren $F,G\colon \mathcal{C} \to \mathcal{D}$ heißt ein \Index{natürlicher Isomorphismus}, wenn 
...
Betrachte folgendes Diagramm \marginnote{"$R$" aus Platzgründen weggelassen}
\[
	\begin{tikzcd}[font=\small, row sep=large]
		0=H_n \enbrace*{X^{(n-1)}} \arrow[dr] & & H_n \enbrace*{X^{(n+1)}} \rar["\cong", "\text{Lemma b)}"'] & H_n(X)\\  
		& h_n \enbrace*{X^{(n)}} \arrow[ur, two heads] \\
		H_{n+1} \enbrace*{X^{(n+1)},X^{(n)}} \arrow[rr,"\partial_{n+1}^{\cell}"] & & H_n \enbrace*{X^{(n)},X^{(n-1)}} \arrow[rr,"\partial_n^{\cell}"] & &
		H_{n-1}  \enbrace*{X^{(n-1)},X^{(n-2)}}  
	\end{tikzcd}
\]
\todo{Diagramm vervollständigen}
