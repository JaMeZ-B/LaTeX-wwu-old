...
\begin{figure}[H]
	\centering{
	\scalebox{0.7}{\begin{tikzpicture}[scale=2.0]
		\coordinate (1) at (0,1);
		\coordinate (2) at (0,0);
...
\end{figure}
Die Zerlegung in $3$-Simplizes ist für $\Delta^2 \times [0,1]$ gegeben durch
\begin{figure}[H]
	\centering{
	\[
		\vcenter{\hbox{
...
\end{figure}
Diese verallgemeinern wir jetzt für beliebiges $n \in \mathds{N}$:
% subsection 79 (end)

\subsection[Definition: Affin lineare Abbildung {$\abs{\Delta^{n+1}} \to \abs{\Delta^n} \times [0,1]$}]{Definition} % (fold)
\label{sub:710}
Für $j=0, \ldots ,n$ sei $k_{n,j} : \abs{\Delta^{n+1}} \to \abs{\Delta^n} \times [0,1]$ die eindeutige affin lineare Abbildung für die gilt:
\begin{align*}
	k_{n,j}(e_0)&=(e_0,0) \\
	k_{n,j}(e_1)&= (e_1,0) \enspace\ldots \\
	k_{n,j}(e_j)&= (e_j,0) \\
	k_{n,j}(e_{j+1})&= (e_j,1) \enspace\ldots \\
	k_{n,j}(e_{n+1})&= (e_n,1)
\end{align*}
\todo{RevChap7}
% subsection 710 (end)
...
\subsection[Beweis von \textbf{[\#\#]}]{Beweis von \protect\eqref{eq:79}} % (fold)
\label{sub:713}
