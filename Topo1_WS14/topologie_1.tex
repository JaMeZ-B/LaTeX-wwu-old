...
\label{sub:156}
Seien $\omega,\omega' \colon [0,1] \to X$ Pfade in $X$ mit $\omega(1)=\omega'(0)$,\marginnote{\enquote{$\,*$} bezeichnet die Verkettung von Pfaden} 
dann ist $\omega * \omega' - \omega- \omega' \in C_1(X;\mathds{Z})$ der Rand eines 
$2$-Simplex.
\minisec{Beweis}
...
\]
Somit (nach \hyperref[sub:1511]{Lemma 4}) induziert $\overline{\rho}$ einen Homomorphismus: $\rho \colon H_1(X;\mathds{Z}) \to \pi_1(X,x_0)^\ab$. Es gilt:
\begin{align*}
...
Mit \hyperref[sub:1512]{Lemma 5} folgt, dass $ \theta^\ab$ ein Isomorphismus ist für wegzusammenhängende Räume $X$. \bewende
% subsection 1510 (end)
...
\end{align*}
\missingfigure{Pfade am Bild eines Simplex und Verbindungen zu $x_0$}
% subsection 1511 (end)

\subsection{Lemma 5} % (fold)
\label{sub:1512}
Sei $\omega \colon \Delta^1 \to X$ ein $1$-Simplex, dann gilt $\theta \circ \overline{\rho}(\omega) = \homologieklasse{\omega - \lambda_{\partial \omega}} $
\minisec{Beweis}
Es gilt
\begin{align*}
...
\label{sub:1514}
Sei $X$ ein topologischer Raum mit Basispunkt $x_0 \in X$, sei $I=[0,1]$. Sei $n \in \mathds{N}_0$. Die \bet{$\mathbf{n}$-te absolute Homotopiegruppe}\index{Homotopiegruppe}
