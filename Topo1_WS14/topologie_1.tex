...
\label{sub:27}
\begin{itemize}
	\item Sei $\partial \Delta^n := \enbrace[\big]{\set{0, \ldots ,n}, \mathcal{P} \enbrace*{\set{0,\ldots ,n}} \setminus \set{\emptyset, \set{0, \ldots , n}}} = $ 
...
\[
	\varepsilon_\sigma^\tau := \begin{cases}
		(-1)î, &\text{ falls }\tau = \set{v_0, \ldots , v_{i-1}, v_{i+1}, \ldots , v_n} \\
		0 , &\text{ sonst}
	\end{cases}
\]
\minisec{Bemerkung}
...
\minisec{Beispiel}
\[
	\text{Hier fehlt noch was } \varepsilon_{\set{0,1,2}}^{\set{0,1}} = 1 , \varepsilon_{\set{0,1,2}}^{\set{0,2}}=-1 , \varepsilon_{\set{0,1,2}}^{\set{1,2}} =1    
\]
\todo{2-Simplex mit Orientierung hinzufügen}
% subsection 35 (end)

\subsection[Definition: $n$-te Randabbildung]{Definition} % (fold)
\label{sub:36}
Sei $K=(V,\Sigma)$ ein geordneter simplizialer Komplex. Die \bet{$n$-te Randabbildung}\index{nte Randabbildung@$n$-te Randabbildung} 
\[
	\partial_n : \underset{=R[\Sigma_n]}{C_n(K;R)} \to \underset{=R[\Sigma_{n-1}]}{C_{n-1}(K;R)}
\]
ist definiert durch
\[
	\partial_n(\sigma) = \sum_{\tau \in \Sigma_{n-1}} \varepsilon_\sigma^\tau \cdot \tau \qquad \text{ für } \tau \in \Sigma_n
\]
% subsection 36 (end)
...
Sei $\sigma$ ein $n$-Simplex von $K$. 
\begin{align*}
	\partial_{n-1} \enbrace*{\partial_n (\sigma)} = \partial_{n-1} \enbrace*{ \sum_{\tau \in \Sigma_{n-1}} \varepsilon_\sigma^\tau \cdot \tau} = \sum_{\tau \in \Sigma_{n-1}} \varepsilon_{\sigma}^\tau \partial_{n-1} (\tau) 
	 &= \sum_{\tau \in \Sigma_{n-1}} \varepsilon_\sigma^\tau \enbrace*{ \sum_{\eta \in \Sigma_{n-2}} \varepsilon_\tau^\eta \cdot \eta} \\
	 &= \sum_{\eta \in \Sigma_{n-2}} \enbrace*{ \sum_{\tau \in \Sigma_{n-1}, \eta \subseteq \tau \subseteq \sigma} \varepsilon_\tau^\eta \varepsilon_\sigma^\tau}\eta 
	 \stackrel{\text{Lemma \ref{sub:38}}}{=} 0 \bewende
\end{align*}
% subsection 37 (end)
...
\[
	\varepsilon_\tau^\eta \varepsilon^\tau_\sigma = - \varepsilon_{\tau'}^\eta \varepsilon^{\tau'}_\sigma.
\]
\minisec{Beweis}
...
\]
Dann ist $\partial_n(\sigma) = \sum_{j=0}^{n} (-1)^j \delta^j \sigma$.
% subsection 39 (end)

\subsection{Bemerkung} % (fold)
\label{sub:310}
$\partial_{n-1} \circ \partial_n = 0 \iff \im \partial_n \subseteq \ker \partial_{n-1}$
% subsection 310 (end)

\subsection[Definition: $n$-ter Homologiemodul]{Definition} % (fold)
\label{sub:311}
Sei $K$ ein geordneter simplizialer Komplex. Der \bet{$n$-te Homologiemodul}\index{nter Homologiemodul@$n$-ter Homologiemodul} von $K$ über $R$ ist definiert als
\[
