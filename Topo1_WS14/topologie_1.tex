...
In der algebraischen Topologie werden topologische Fragen, wie zum Beispiel wann $\mathds{R}^n$ und $\mathds{R}^m$ homöomorph sind, in algebraische Fragen übersetzt. 
...
\minisec{Bemerkung}
\begin{itemize}
	\item In Aufgabe 4 von Blatt 1 haben wir gezeigt, dass $\abs*{K}$ homöomorph zu einem Unterraum $X$ von
...
\[
	a_n := \# \set[\sigma \in \Sigma]{\sigma \text{ ist ein $n$-Simplex von $K$}}
\] 
Dann heißt $\chi(K) := \sum_{n \in \mathds{N}} (-1)^n \cdot a_n $ die \Index{Euler-Charakteristik} von $K$. 
\minisec{Beispiel}
\begin{align*}
...
\[
	\Rg A = \max \set[m]{\exists C \le A, C \cong \mathds{Z}^m}.
\]
\minisec{Beweis}
...
\label{sub:61}
Seien $(C_*,d_*)$ und $(C_*',d_*')$ zwei $R$-Kettenkomplexe. Dann ist eine \bet{$\mathbf{R}$-Kettenabbildung}\index{R-Kettenabbildung@$R$-Kettenabbildung}
$f_* \colon (C_*,d_*) \to (C_*',d_*')$ eine Folge von $R$-linearen Abbildungen $f_n \colon C_n \to C_n'$, sodass
\[
...
\]
bezeichnen wir die \bet{Summe}\index{Summe!von $R$-Moduln} (auch das \bet{Koprodukt}\index{Koprodukt!von $R$-Moduln}) der $V_i$. Elemente von $V$ sind $I$-Folgen $(v_i)_{i \in I}$ mit $v_i \in V_i$ und $v_i=0$ für
...
\label{sub:76}
Seien $(C_*,d_*)$ und $(C_*',d_*')$ $R$-Kettenkomplexe. Seien $f_*,g_* \colon (C_*,d_*) \to (C_*',d_*')$ $R$-Kettenabbildungen. \marginnote{Kettenhomotopie definiert eine Äquivalenzrelation wie in Aufgabe 2 von Blatt 5 gezeigt.}
Eine \Index{Kettenhomotopie} von $f_*$ nach $g_*$ ist eine Folge von $R$-linearen Abbildungen $h_n \colon C_n \to C_{n+1}'$, $n \in \mathds{N}$, sodass für alle $n$ gilt:
\[
...
\label{sub:84}
Via $X \mapsto (X,\emptyset)$ können wir jeden Raum auch als Paar auffassen. Es gilt dann offensichtlich $H_n(X,\emptyset;R) = H_n(X;R)$.
% subsection 84 (end)
...
\begin{enumerate}[a)]
	...
	$(\iota_a)_*(x)=0$. 
	Nach Proposition \ref{sub:1011}(2) muss $x$ dann aber schon für ein $i$ in der reduzierten Homologie $\tilde{H}_i \enbrace*{S^n \setminus f \enbrace*{I_a \times [0,1]^k} }$ trivial sein. \light
...
\begin{enumerate}[a)]
	...
	\[
		...
		\end{tikzcd}
	\]
	Für $m \ge n >k$ induziert die Inklusion $X^{\scriptscriptstyle(n)} \hookrightarrow X^{\ssbrace{m}}$ daher einen Isomorphismus in Homologie $H_k(X^\ssbrace{n};R) \to H_k(X^{(m)};R)$. 
...
$(B_n)_X$} $(B_n)^{k}_X(\alpha)\in C^\mathcal{U}_n(X;R)$ für alle $k\ge k_0$.
...
\label{sub:151}
Sei $G$ eine Gruppe. Dann heißt die Untergruppe von $G$, die von Elementen der Form $[g,h]:= g h g^{-1} h ^{-1}$ erzeugt wird, \Index{Kommutatorgruppe} von $G$. Wir 
...
\begin{figure}[th]
	...
	\begin{tikzpicture}[scale=2, my brace/.style={decorate, decoration={brace, amplitude=5pt, mirror, raise=3pt}, gray},baseline=-\heightof{$\to$}/2]
...
Nach \hyperref[sub:1511]{Lemma 4} induziert $\overline{\rho}$ einen Gruppenhomomorphismus $\rho \colon H_1(X;\mathds{Z}) \to \pi_1(X,x_0)^\ab$. Für eine Schleife $\omega$ in $x_0$ 
gilt
\begin{align*}
...
\subsection[Korollar: Erste Homologiegruppe des Produkts wegzusammenhängender Räume]{Corollar} % (fold)
\label{sub:1513}
...
\subsection{Theorem (Hurewicz)} % (fold)
\label{sub:1523}
Seien $(X,A)$ mit $A \subset X$ beide wegzusammenhängend. Es gelte weiter $\pi_1(A,x_0)=1$ und $\pi_k(X,A,x_0)=0$ für $1 \le k \le n-1$, $n \ge 2$. Dann ist 
\[
...
\subsection[Korollar: Isomorphie von Homotopie-- und Homologiegruppen für $(n-1)$-zsmhg. Räume]{Korollar} % (fold)
\label{sub:1524}
...
\subsection[Bemerkung zur Homologie von Mannigfaltigkeiten]{Bemerkung} % (fold)
\label{sub:162}
Sei $M$ ein $n$-Mannigfaltigkeit und $x \in M$. Da $x$ in $M$ eine Umgebung hat,\marginnote{Ausgeschnitten wird das Komplement dieser Umgebung!} die homöomorph zu $\mathds{R}^n$ ist, folgt mit dem Ausschneidungsisomorphismus und der 
Paarfolge
\[
...
\subsection[Definition: Homologische Orientierung in einem Punkt]{Definition} % (fold)
\label{sub:163}
...
\subsection[Notation: Einschränkung eines Elementes $v\in H_n(X,X\setminus B;R)$]{Notation} % (fold)
\label{sub:164}
...
\subsection[Definition: Homologische Orientierung der gesamten Mannigfaltigkeit]{Definition} % (fold)
\label{sub:165}
Sei $M$ eine $n$-Mannigfaltigkeit. Eine (homologische) \bet{Orientierung}\index{homologische Orientierun!global} von $M$ über $R$ ist eine Zuordnung 
...
Gibt es eine solche Orientierung, so heißt $M$ $R$-\bet{orientierbar}\index{orientierbare Mannigfaltigkeit}. Eine Mannigfaltigkeit zusammen mit einer Orientierung heißt eine 
$R$-\bet{orientierte Mannigfaltigkeit}\index{orientierte Mannigfaltigkeit}. Lassen wir $R$ in den Bezeichnungen weg, so ist immer $R=\mathds{Z}$ gemeint.
% subsection 165 (end)

\subsection[Beispiel: Orientierung von $\mathds{R}^n$]{Beispiel} % (fold)
\label{sub:166}
...
\subsection[Satz über $R$-Orientierbarkeit]{Satz} % (fold)
\label{sub:169}
...
\subsection[Definition: Disjunkte Vereinigung der $H_n(M,M\setminus \set{x};R)$ und $\Gamma(M,\mathcal{H})$]{Definition} % (fold)
\label{sub:1610}
...
\subsection[Bemerkung: Eigenschaften von $\mathcal{H}$ und $\mathcal{H} \xrightarrow{\pi} M$]{Bemerkungen} % (fold)
\label{sub:1611}
\begin{enumerate}[1)]
...
\subsection[Beispiel: Für $R=\mathds{F}_2$ ist $\pi\colon \mathcal{H} \to M$ eine $2$-fache Überlagerung]{Beispiel} % (fold)
\label{sub:1613}
...
\subsection[Lemma über kompakten Teilraum einer $n$-Mannigfaltigkeit]{Lemma} % (fold)
\label{sub:1614}
...
\begin{enumerate}[label=\textbf{Schritt \arabic*:},wide=0cm]
	...
	\begin{enumerate*}[(1),itemjoin={,\hspace{1em}},itemjoin*={{\hspace{1ex}und\hspace{1ex}}}]
		...
		\item $\partial z \in C_{d-1}\enbrace[\big]{\mathds{R}^n \setminus (B_1 \cup \ldots \cup B_n);R}$
	\end{enumerate*}
	
	Es folgt, dass $z$ auch eine Homologieklasse $v' \in H_d \enbrace[\big]{\mathds{R}^n,\mathds{R}^n \setminus (B_1 \cup \ldots \cup B_n);R}$ repräsentiert mit $v'|_A=v$.
	...
	$H_n(\mathds{R}^n, \mathds{R}^n \setminus B_i;R) \to H_n(\mathds{R}^n,\mathds{R}^n \setminus \set{x_i};R )$ definiert. Daher gilt $v'|_y=0$ für alle 
	...
	Ausschneidungsisomorphismen $H_*(U_i,U_i\setminus A;R) \to H_*(M,M \setminus A;R)$ erhalten und $U_i \cong \mathds{R}^n$ ist, gilt die Behauptung nach Schritt 3 für 
	jedes $A_i$. Mit Schritt 1 folgt die Behauptung auch für $A$. \bewende
\end{enumerate}

% subsection 1614 (end)

\subsection[Bemerkung: Existenz einer Mayer-Vietoris-Folge für $A,B \subseteq X$ abgeschlossen]{Bemerkung} % (fold)
\label{sub:1615}
\begin{enumerate}[1)]
