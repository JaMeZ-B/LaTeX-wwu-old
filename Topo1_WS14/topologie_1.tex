...
\subsection[Lemma: Hilflemma für Proposition \ref{sub:55}]{Lemma} % (fold)
\label{sub:56}
Für $0 \le j \le k \le n-1$ und $\sigma \in S_n(X)$ ist $\delta_k \delta_j(\sigma) = \delta_j \delta_{k+1}(\sigma)$.
\minisec{Beweis}
Es ist
\begin{align*}
	\delta_k \enbrace[\big]{\delta_j(\sigma)} \enbrace*{x, \ldots , x_{n-2}} &= \delta_j(\sigma) \enbrace[\big]{x_0, \ldots , x_{k-1}, 0, x_k , \ldots ,x_{n-2}}  \\
...
\label{sub:63}
\begin{enumerate}[a)]
	\item $R$-Kettenkomplexe mit Kettenabbildungen bilden die Kategorie $R\text{-}\mathrm{KETTEN}$.
	\item Homologie definiert nun einen Funktor: $H_n : R\text{-}\mathrm{KETTEN} \to R\text{-}\MOD$.
	...
	\[
		H_* : R\text{-}\mathrm{KETTEN} \longrightarrow \mathrm{GR}\text{-}R\text{-}\MOD \quad , \qquad (C_*,d_*) \longmapsto \enbrace[\big]{H_n(C_*,d_*)}_n 
	\]
	zusammensetzen.
\end{enumerate}
% subsection 63 (end)
...
\begin{enumerate}[(i)]
	...
	\begin{align*}
		...
		r_\sigma (\delta_j \sigma)} \\
		&= \sum_{\sigma \in S_n(X)} \sum_{j=0}^{n} (-1)^j \cdot r_\sigma \enbrace*{f_{n-1} \circ (\delta_j \sigma)} \\
		&= \sum_{\sigma \in S_n(X)} \sum_{j=0}^{n} (-1)^j \cdot r_\sigma \enbrace*{f_{n-1} \circ (\sigma \circ \iota_{n,j})} \\
		&= \sum_{\sigma \in S_n(X)} \sum_{j=0}^{n} (-1)^j \cdot r_\sigma \enbrace*{(f_{n-1} \circ \sigma) \circ \iota_{n,j}} \\
		&= \sum_{j=0}^{n} (-1)^j \enbrace*{\sum_{\sigma \in S_n(X)} r_\sigma \underbrace{\enbrace*{f_{n-1} \circ \sigma} \circ \iota_{n,j}}_{\delta_j (f \circ \sigma)} } \\
