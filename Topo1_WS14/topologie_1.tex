...
\]
Weiter gibt es dann $n$ mit $A \cong \mathds{Z}^n \oplus TA$. $\Rg A := n$ ist der Rang von $A$\index{Rang einer abelschen Gruppe}. \todo{RevChap4}
% subsection 41 (end)
...
\]
für alle $n\ge 1$ ist. \todo{RevChap6}
\[
	\begin{tikzcd}
...
\noindent Die Kategorie von Paaren von topologischen Räumen bezeichnen wir mit $\TOP^2$. \todo{RevChap8}
% subsection 81 (end)
...
\[
	i_{n-2} \enbrace*{d_{n-1}(v)} = d'_{n-1} \enbrace*{i_{n-1}(v)} = d_{n-1}' \enbrace*{d_n'(v')} = 0   
\]
...
\begin{align*}
	i_{n-1} \enbrace*{-d_n(a)- (v-w)} & = -i_{n-1} \enbrace*{d_n(a)} - i_{n-1}(v-w) \\ &= - d_n' \enbrace*{i_n(a)} - \enbrace*{d_n'(v')- d_n'(w')}  \\  
...
\label{sub:815}
Seien die Zeilen in folgendem kommutativen Diagramm von $\mathds{R}$-Moduln exakt.
\[
	\begin{tikzcd}
		A_4 \rar["\alpha_4"] \dar["f_4"] & A_3 \rar["\alpha_3"] \dar["f_3"] & A_2 \rar["\alpha_2"]  \dar["f_2"] & A_1 \rar["\alpha_1"] \dar["f_1"] & A_0  \dar["f_0"]\\
...
\minisec{Beweis}
Da die Randabbildungen in der Paarfolge eine natürliche Transformation ist, erhalten wir ein kommutatives Leiterdiagramm:
\[
	\begin{tikzcd}[column sep=1.2em, font=\scriptsize]
		H_{n+1}(A;R) \rar \dar["{(f|_A)_*}"] & H_{n+1}(X;R) \rar \dar["f_*"] & H_{n+1}(X,A;R) \rar["\partial_{n+1}"] \dar["{(f,f|_A)_*}"] & H_n(A;R) \rar \dar["(f|_A)_*"] & H_n(X;R) \rar \dar["f_*"] & H_n(X,A;R) \rar["\partial_n"] \dar["{(f,f|_A)_*}"] & H_{n-1}(A;R) \dar["{(f|_A)_*}"] \\
		H_{n+1}(B;R) \rar & H_{n+1}(Y;R) \rar & H_{n+1}(Y,B;R) \rar["\partial_{n+1}"] & H_n(B;R) \rar & H_n(Y;R) \rar & H_n(Y,B;R) \rar["\partial_n"] & H_{n-1}(B;R) \\
	\end{tikzcd}
\]
...
$j_B : A \cap B \hookrightarrow B$ die Inklusionen. Dann gibt es eine Randabbildung $\partial_n = \partial_n^{X=A\cup B} : H_n(X;R) \to H_{n-1}(A \cap B;R)$, sodass
\[
	\begin{tikzcd}[column sep=3.5em]
		\cdots \rar["\partial_{n+1}"] & H_n(A \cap B;R) \rar["{\begin{psmallmatrix}(j_A)_* \\ (j_B)_* \end{psmallmatrix}}"] 
		&  \bigoplus\limits_{H_n(B;R)}^{H_n(A;R)} \rar["(i_A)_* - (i_B)_*"] 
		& H_n(X;R) \rar["\partial_n"] & H_{n-1}(A\cap B;R) \rar & \cdots
	\end{tikzcd}
\]
...
\[
	\begin{tikzcd}[column sep=3em]
		H_n(X;R) \rar["\partial_n^{X=A \cup B}"] \dar["f_*"]  & H_{n-1}(A \cap B;R) \dar["\enbrace*{f|_{A \cap B}}_*"] \\
		H_n(X';R) \rar["\partial_n^{X'=A' \cup B'}"] & H_n(A' \cap B';R)
	\end{tikzcd}
\]
\minisec{Beweis}
...
\]
Betrachte das folgende kommutative Leiterdiagramm. \todo{$\cong$ schöner platzieren}
\[
	\begin{tikzcd}[font=\scriptsize, column sep=1.8em, row sep=2.5em]
		\cdots \rar & H_{n+1}(B,A \cap B;R) \rar["\partial_{n+1}^{B,A \cap B}"] \dar["{(i_B,j_A)_* \cong}"] & H_n(A \cap B;R) \rar["(j_B)_*"] \dar["(j_A)_*"] 
		& H_n(B;R) \rar["(l_B)_*"] \dar["(i_B)_*"] & H_n(B,A \cap B;R) \rar["\partial_n"] \dar["{(i_B,j_A)_* \cong}"]
		& H_{n-1}(A \cap B;R) \rar \dar["(j_A)_*"] & \cdots \\
		\cdots \rar & H_{n+1}(X,A;R) \rar["\partial_{n+1}^{X,A}"] & H_n(A;R) \rar["(i_A)_*"]  & H_n(X;R) \rar["(l_X)_*"] & H_n(X,A;R) \rar["\partial_n^{X,A}"] & H_{n-1}(A;R) \rar & \cdots
	\end{tikzcd}
\]
...
\[
	\partial_n := \partial_n^{B, A\cap B} \circ  \enbrace*{(i_B,i_A)_*} ^{-1} \circ (l_X)_* 
\]
Die Natürlichkeit von $\partial_{n}^{B,A \cap B}$ impliziert die Natürlichkeit von $\partial_n$.

Zu zeigen: $\im \begin{psmallmatrix}
	(j_A)_* \\ (j_B)_*
\end{psmallmatrix} = \ker \enbrace*{(i_A)_* - (i_B)_*} $: 
\begin{description}
	\item["$\supseteq $":] Sei $(a,b) \in \ker \enbrace*{(i_A)_* - (i_B)_*}$. Also $(i_A)_*(a) = (i_B)_*(b)$. Es ist 
	\[
		(i_B,j_A)_* \enbrace*{(l_B)_*(b)} = (l_X)_* \circ (j_B)_* (b) = \underbrace{(l_X)_* \circ (i_A)_*}_{=0}(a)= 0 
	\]
	...
	\[
		(i_A)_* \enbrace*{a - (j_A)-*(x)} = (i_A)_*(a) - i_B \enbrace*{(j_B)_* (x)} = (i_A)_*(a) - (i_B)_* (b) = 0  
	\]
	Mit $\ker (i_A)_*  =\im \partial_n^{X,A}$ und $(i_B,j_A)_*$ ist Isomorphismus, folgt $\exists y \in H_{n+1}(B,A\cap B;R)$ mit 
	\[
		\partial_n^{X,A} \circ  \enbrace*{i_B,j_A}_* (y) = a - (j_A)_*(x) 
	\]
	Es folgt für $z := x + \partial_n^{B,A \cap B}(y)$.
	\begin{align*}
		(j_B)_*(z) & =(j_B)_*(x) = b \\
		(j_A)_*(z) &= (j_A)_*(x) + (j_A)_* \enbrace*{\partial_n^{B,A \cap B}(y)} = (j_A)_*(x) + \partial_n^{X,A} \circ  \enbrace*{i_B, j_A}_*(y) = (j_A)_*(x) + a -
		(j_A)_*(x) = a  
	\end{align*}
\end{description}
% subsection 92 (end)
