...
\subsection{Satz} % (fold)
\label{sub:72}
Seien $f,g : X \to Y$ stetige Abbildungen. Sind $f$ und $g$ homotop, so gilt $H_n(f;R) = H_n(g;R)$.
% subsection 72 (end)

\subsection{Korollar} % (fold)
\label{sub:73}
Ist $f : X \to Y$ eine Homotopieäquivalenz \todo{Def als Fußnote}, so ist $f_* : H_n(X;R) \to H_n(Y;R)$ ein Isomorphismus.
\minisec{Beweis}
