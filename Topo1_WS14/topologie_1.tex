...
\end{equation*}
heißt ein \Index{Pushout}, falls es folgende universelle Eigenschaft hat: $\forall \hat{f} \colon X \to \hat{Z}$, $\hat{g}: Y \to \hat{Z}$ mit 
$\hat{f} \circ f = \hat{g} \circ g$ gilt: $\exists ! \varphi \colon Z \to \hat{Z}$ mit $\hat{f}= \varphi \circ \overline{g} $, $\hat{g} = \varphi \circ \overline{f}$.
\[
	\begin{tikzcd}
...
\begin{enumerate}[(i)]
	...
	\]
	Wir erhalten einen Homöomorphismus $f^{(n)} \colon \nicefrac{D^{2n+1}}{z \sim \lambda z, z \in S^{2n}} \to \mathds{C}P^n$ mit
	\[
