...
\minisec{Beweis}
Da die Randabbildung in der Paarfolge eine natürliche Transformation ist, erhalten wir ein kommutatives Leiterdiagramm:\marginnote{"$R$" aus Platzgründen weggelassen}
\[
...
\]
Betrachte das folgende kommutative Leiterdiagramm: \marginnote{"$R$" aus Platzgründen weggelassen}
\[
...
\begin{itemize}
	...
	\begin{description}
		\item["$\supseteq$":] Sei $v \in \ker \partial_n$. \todo{Rest hinzufügen}
...
\subsection[Bemerkung zur Mayer-Vietoris-Folge]{Bemerkung} % (fold)
\label{sub:93}
...
\begin{description}
	...
	\]
	Betrachte nun die Mayer-Vietoris-Folge zu $S^n = D_n^+ \cup D_n^-$: \marginnote{"$R$" aus Platzgründen weggelassen}
	\[
		\begin{tikzcd}
...
\label{sub:101}
Sind $\mathds{R}^n$ und $\mathds{R}^m$ homöomorph, so gilt $n=m$. \todo{RevChap10}
\minisec{Beweis}
...
\section*{Vorbereitung: Abelisierung von Gruppen} % (fold)
...
\begin{align*}
	h [g_0,g_1] \cdot \ldots \cdot [g_{n-1},g_n] \cdot h ^{-1} = [h g h ^{-1}, h g_1 h ^{-1}] \cdot \ldots \cdot [h g_{n-1} h ^{-1}, h g_1 h ^{-1}] \bewende
\end{align*}
% subsection 152 (end)
