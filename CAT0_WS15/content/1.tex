...
\begin{beweis}
	Wir brauchen folgendes Resultat (Übungsaufgabe):
...
\label{lemma:1.26}
	...
	Dann ist $\Fix_\Phi(G)$ abgeschlossen. \marginnote{Beweis: ÜA!}
	Wenn $\Fix_\Phi(G) \neq \emptyset$, dann ist $\Fix_\Phi(G)$ konvex. \index{konvex}
\end{lemma}
...
\begin{lemma}
\label{lemma:1.35}
	Ein vollständiger metrischer Raum, der Mittelpunkte hat, ist geodätisch. \marginnote{Beweis: Übung.}
\end{lemma}
...
\label{def:1.36}
	...
	\begin{enumerate}[(i)]
		...
		\[
			...
			d(x_1,x_2) = d_2(\ol{x_1},\ol{x_2}), \quad
			d(y_1,y_2) = d_2(\ol{y_1},\ol{y_2}).
		\]
	\end{enumerate}
\end{definition}
...
\begin{lemma}
\label{lemma:1.37}
	Ein geodätischer Raum ist genau dann $\CAT$, wenn für jedes geodätische Dreieck $\Delta(x,y,z,\alpha,\beta,\gamma)$ mit Vergleichsdreieck \marginnote{Beweis: Übung.} $\ol{\Delta}(\ol{x},\ol{y},\ol{z},\ol{\alpha},\ol{\beta},\ol{\gamma})$ in $\EE^2$ gilt: $d(x,\beta(s)) \leq d_2(\ol{x},\ol{\beta}(s))$ für alle $s \in [0,d(y,z)]$.
\end{lemma}
...
\begin{theorem}
\label{thm:1.38}
	Ein vollständiger metrischer Raum ist $\CAT$ genau dann, wenn er er ungefähre Mittelpunkte hat und die $\CAT$-Vierpunktbedingung erfüllt.
\end{theorem}	
...
\begin{beweis}
	...
	\begin{description}
		...
		\begin{figure}[h]
			...
			\draw (1.25,1.5) node[fill,circle,inner sep=1pt,color=red]{};
			\draw (1.25,1.5) node[right,color=red]{$\ol{m}$};
			\end{tikzpicture}
		\end{figure}
	...
	\begin{equation}
		\left. \begin{array}{r}
			...
			d(u,m_s) \\
			d(v,m_s)		
		\end{array} \right\} \leq r+ \delta \quad \forall r,s \neq N \label{eq:1.38.1}
	\end{equation}
