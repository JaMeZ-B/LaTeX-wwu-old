...
\begin{description}
	\item[\Index{lokal homöomorph} zum $\mathds{R}^n$:]  $\forall p \in M :\, \exists$ offene Umgebung $U \subset M$ um $p$ und einen Homöomorphismus 
	$h : U \to U' \subset \mathds{R}^n$ auf eine offene Teilmenge $U'$ des $\mathds{R}^n$. 
...
In der Tat: \marginnote{In Aufgabe 2 von Blatt 5 wurden diese Schritte detailliert ausgeführt} Sei $u^0(t,x) := x$ und definiere iterativ 
$u^{\nu +1} := T u^\nu$ $\forall \nu \in \mathds{N}$. Es gilt dann die Kontraktionsabschätzung 
\[
	\norm{u^{\nu +1} - u^\nu} \le \frac{K \cdot L^\nu \cdot \varepsilon^{\nu +1} }{(\nu +1)!}, 
\]
...
\[
	T u = \lim_{\nu \to \infty} T u^\nu = \lim_{ \nu \to \infty} u^\nu = u \marginnote{$T$ ist (Lipschitz)-stetig}
\]
$u$ ein Fixpunkt von $T$.
Alternativ führt man den Beweis auch mit der Norm
\[
...
\]
ist $t \mapsto u(t,x)$ beliebig oft differenzierbar für alle $x$. Ist $u$ auch bezüglich $x$ beliebig oft differenzierbar, dann gilt
\[
...
$u \in C^3$ usw. und somit $u \in C^\infty$. \bewende\todo{RevChap2}
...
\label{ssub:242}
Da wo definiert, gilt
\[
	\Phi_t \circ \Phi_s(q) = \Phi_{t+s}(q).
\]
\minisec{Beweis}
$t \mapsto \gamma_q(t+s)$ und $t \mapsto \gamma_{\Phi_s(q)}(t)$ sind Integralkurven von $X$ mit identischem Anfangspunkt, da für $t=0$ gilt
\[
	\gamma_q(s) = \Phi_s(q). 
\]
...
\]
also muss $\Phi_{t+s}(q) = \Phi_t \circ \Phi_s (q)$ gelten. \bewende \bigskip\\
...
\[
	(\mathcal{L}_X Y)_p := \lim_{ t \to 0} \frac{1}{t} \enbrace[\big]{T \Phi_{-t} \enbrace*{Y_{\Phi_t(p)}} - Y_p} = \diffd{}{t}\Big|_{t=0} \enbrace[\Big]{T \Phi_{-t} 
	\enbrace*{Y_{\Phi _t(p)}} }     
\]
...
\begin{description}
	...
	\[
		\mathcal{L} _X f = \diffd{}{t}\Big|_{t=0} (f \circ  \Phi_t) = X(f). 
	\] 
	\item[zu b):] In lokalen Koordinaten: $X = X^i \diff{}{x^i}$, $Y = Y^j \diff{}{x^j}$. Betrachte zunächst nur einen Basisvektor
	\[
		\mathcal{L} _X \enbrace*{\diff{}{x^j} } = \diffd{}{t}\Big|_{t=0} T \Phi_{-t} \enbrace*{\diff{}{x^j} } \stackrel{\text{Ana II. 7.7}}{=} \diffd{}{t}\Big|_{t=0} \diff{\Phi_{-t}^i}{x^j} \diff{}{x^i} 
		= - \diff{X^i}{x^j} \diff{}{x^i}  \marginnote{$\Phi_{-t}$ ist der Fluss zu $-X$} 
	\]
	In Analogie zur Produktregel für die gewöhnliche Ableitung für reellwertige Funktionen zeigt\marginnote{siehe Blatt 6, Aufgabe 1} man:
	\begin{align*}
		\mathcal{L}_X(f  Y) = (\mathcal{L}_X f) Y + f \cdot \mathcal{L}_X Y
	\end{align*}
	Daraus folgt
	\begin{align*}
		\mathcal{L} _X\enbrace*{Y^j \diff{}{x^j}} = \mathcal{L}_X Y^j \cdot \diff{}{x^j} + Y^j \cdot \mathcal{L}_X \diff{}{x^j} \stackrel{\text{a)}}{=} X^i \diff{Y^j}{x^i} \diff{}{x^j} 
...
\subsubsection[Notation für Pushforward]{Notation für Diffeomorphismen (Pushforward\footnote{"[\ldots] \emph{push} tangent vectors on $M$ \emph{forward} to tangent vectors on $N$", \url{https://en.wikipedia.org/wiki/Pushforward_(differential)}})} % (fold)
\label{ssub:245}
Sei $f : M \xrightarrow{\cong} N $ ein Diffeomorphismus und $X$ ein Vektorfeld auf $M$.
Dann erhält man ein neues Vektorfeld auf $N$ in $q \in N$ durch 
\[
	 \enbrace*{f_* X}_q := T _{f ^{-1}(q)} f \enbrace*{X_{f ^{-1}(q)}}
\]
Damit können wir die Lie-Ableitung schreiben als
\[
...
\label{ssub:246}
Sei $M=N=\mathds{R}^2$. $f(x,y)= (x, x^2+y) \Rightarrow f^{-1} (x,y) = (x, -x^2 +y)$ ist ein Diffeomorphismus zwischen $M$ und $N$. Sei weiter ein Vektorfeld definiert durch
$X_{x,y} = y \diff{}{x} = \binom{y}{0}$.
\begin{figure}[h]
	\centering{
...
Es gilt $T_{(x,y)}(f) = J_f (x,y)= \begin{pmatrix}
	1 & 0 \\
	2x & 1
...
Es gilt
\begin{align*}
	\diffd{}{t} \Big|_{t=0} \enbrace*{\Phi_t \circ f ^{-1} (q)} &= X_{f ^{-1}(q)}  \\
	\intertext{Damit folgt}
	\diffd{}{t} \Big|_{t=0} \enbrace*{f \circ \Phi_t \circ f ^{-1}(q)} &= T_{f ^{-1}(q)} f \enbrace*{X_{f ^{-1}(q)}} = \enbrace*{f_*X}_q \bewende    
\end{align*}
...
\[
	[X,Y] = 0 \iff \forall s,t : \Phi_t \circ \Psi_s = \Psi_s \circ \Phi_t
\]
\minisec{Beweis}
\begin{description}
	\item["$\Leftarrow$":] Es gelte $\Phi_t \circ \Psi_s = \Psi_s \circ \Phi_t$. Dann gilt auch 
	\begin{align*}
		\diffd{}{s} \Big|_{s=0} \Phi_t \circ \Psi_s (p) = \diffd{}{s} \Big|_{s=0} \Psi_s \circ  \Phi_t(p) 
	\end{align*} 
	d.h. $T_p \Phi_t (Y_p) = Y_{\Phi_t(p)}$ für alle $p,t$ und $(\Phi_{-t})_* Y = Y$ für alle $t$. Dann erhalten wir
	\begin{align*}
		[X,Y] =  \mathcal{L}_X Y = \diffd{}{t}\Big|_{t=0} (\Phi_{-t})_* Y = 0 
	\end{align*}
	\item["$\Rightarrow $":] Es gelte $[X,Y] =0$. Nach Lemma 9  (\ref{ssub:247}) ist zu zeigen, dass $(\Phi_t)_* Y = Y$ für alle $t$. Es gilt 
	\begin{align*}
		\diffd{}{t}\Big|_{t=t_0} \enbrace*{\Phi_{-t}}_* Y = \diffd{}{t}\Big|_{t=0} \enbrace*{\Phi_{-t_0-t}}_* Y  = \diffd{}{t}\Big|_{t=0}
		\enbrace*{\Phi_{-t_0}}_* \enbrace*{\Phi_{-t}}_* Y = \enbrace*{\Phi_{-t_0}}_* \underbrace{\mathcal{L}_X Y}_{= [X,Y]=0} = 0   \bewende
	\end{align*}
\end{description}
...
\]
Für $A= \id$ entspricht dies einer Skalierung um $e^{t}$, da $e^{A \cdot t} = \begin{psmallmatrix*}[l]
	e^t & 0 \\
	0 & e^t
\end{psmallmatrix*}$:
\[
...
Für $A = \begin{psmallmatrix*}[r]
	1 & 0 \\
	0 & -1
\end{psmallmatrix*}$ ergibt sich $e^{A \cdot t} = \begin{psmallmatrix*}[l] e^t & 0 \\ 0 & e^{-t} \end{psmallmatrix*}$.
Startpunkte $\widehat{=}\,{\tikz[baseline=-0.5ex]{\draw[fill, RoyalBlue2] (0,0) circle[radius=0.04];}}$ und $t \in [-1, 1]$ 
\[
...
Für $A = \begin{psmallmatrix*}[r]
	0 & 1 \\
	-1 & 0
\end{psmallmatrix*}$ ergibt sich $e^{A \cdot t} = \begin{psmallmatrix*}[l]
	1 & e^t \\
	e^{-t} & 1
\end{psmallmatrix*}$.
Startpunkte $\widehat{=}\,{\tikz[baseline=-0.5ex]{\draw[fill, RoyalBlue2] (0,0) circle[radius=0.04];}}$ und $t \in [-1/2, 1/2]$
\[
	\begin{tikzpicture}[scale=1.2]
		%\draw[help lines] (-3,-3) grid (3,3);
	...
	\end{tikzpicture}
\]
Betrachte den 2-Torus $S^1 \times S^1 \subset \mathds{C} \times \mathds{C}$ mit dem Fluss 
\[
	\Phi\enbrace[\big]{t, (z_1, z_2)} = \enbrace*{e^{it} z_1, e^{a i t} z_2}, \quad a \in \mathds{R} 
\]
Für $a=0$ ergibt sich:
\[
	\tikzsetnextfilename{fluss_3}
	\vcenter{\hbox{\includegraphics[width=0.4\textwidth, keepaspectratio]{Mathematica/torus.png}}}
	\hspace{2cm} \tikzsetnextfilename{fundamentalgebiet_a_0}
...
Betrachte nun die 2-Sphäre $S^2 \subset \mathds{C} \times \mathds{R}$ mit dem Fluss $\Phi\enbrace[\big]{t, (z,r)} = \enbrace*{e^{it} z, r}$
\missingfigure{Rotation um die $r$-Achse}

"Schwingen" einer Ebene um den Nordpol des $S^2$
\missingfigure{"Schwingen" einer Ebene um den Nordpol des $S^2$}
...
\[
	...
	\end{tikzpicture}}} 
	\quad \xrightarrow{\makebox[1cm]{$\gamma$}} \quad 
...
\[
	\omega : \underbrace{V \times \ldots \times V}_{\text{$k$-mal}} \longrightarrow \mathds{R},
\]
...
\[
	\underbrace{\Gamma(T M) \times \ldots \times \Gamma(T M)}_{\text{$k$-mal}} \longrightarrow C^\infty(M)
\]
\minisec{Beweis}
\begin{description}
...
\begin{description}
	...
	\]
	Dies ist wohldefiniert: Sei also $(V,k)$ eine zweite Karte mit $p \in V$. Dann gilt
	\begin{align*}
...
\begin{enumerate}[(i)]
	...
	\[
		\begin{tikzcd}
			S^m \rar["\tau"] \dar[swap,"\text{2:1}"] & S^m \dlar["\text{2:1}"] \\
			\mathds{R}P^m & \mbox{ }
		\end{tikzcd}
	\]
