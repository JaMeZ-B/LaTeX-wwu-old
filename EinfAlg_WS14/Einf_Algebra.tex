...
Den Isomorphismus $\overline{\varphi}:\nicefrac{G}{\ker(\varphi)}  \xrightarrow{\cong} K$ nennt man \Index{kanonisch} oder \Index{natürlich}.
\hfill $\square$
...
\]
Gleiches gilt für $Q$ (mit ÜA %\hyperref[sub:a_4_3]{4.3 einfügen} 
...
\begin{enumerate}[(i)]
	...
	\item Es gibt ein Ring $S$ und einen Homomorphismus $R \xrightarrow{\varphi} S$ mit $\ker(\varphi)=I$.
\end{enumerate}
...
\begin{enumerate}[(a)]
	...
	Es folgt $\Char(K)=\Char(L)$ $\big($denn: $\Z\stackrel{c}{\to}K\stackrel{\varphi}{\hookrightarrow}L$, also $\ker(c:\Z\to K)=\ker(\varphi\circ c=c':\Z\to L)\big)$.
	\item Ist $K$ ein Körper, $X\subseteq K$ eine Teilmenge, so ist
	\[
	\bigcap \penbrace{L\subseteq K~|~L\text{ Teilkörper, }X\subseteq L}\subseteq K,
	\]
...
\label{sub:def_koerpererweiterung}
...
\]
Man nennt $\mu=r_0\dt{+}T^n$ das \Index{Minimalpolynom} von $u$ über $K$ und nennt $u$ \Index{algebraisch} über $K$.
...
Ein Punkt $q\in\R^2$ heißt \Index{elementar konstruierbar} aus $S$, wenn $q$ von den folgenden Typen ist
\begin{enumerate}[(a)]
...
\begin{enumerate}[(a)]
	...
	\psi|_K=\varphi
	\]
	so heißt $\psi$ \Index{Homomorphismus} über $\varphi$.
	\begin{center}
		\begin{tikzcd}[column sep=small]
			 L \ar{r}{\psi} & L'\\
			 K \ar{r}{\varphi} \ar[u,hook] & K' \ar[u,hook] 
		\end{tikzcd}
		\captionof{figure}{Homomorphismus}
	\end{center}
\end{enumerate}
%sub end
...
\begin{center}
	...
	\end{tikzcd}
	\captionof{figure}{Beweis Lemma 16}
\end{center}
...
\begin{center}
	\begin{tikzcd}[column sep=small]
		...
		K \ar{r}{\varphi} \ar[u,hook] & K' \ar[u,hook]
	\end{tikzcd}
	\captionof{figure}{Beweis Satz 27}
\end{center}
...
Das ist (unter anderem) Inhalt der \Index{Galois-Theorie}.
...
\subsection{Definition algebraisch abgeschlossen}
\label{sub:def_alg_abgeschlossen}
Ein Körper $K$ heißt \Index{algebraisch abgeschlossen}, wenn er eine der folgenden äquivalenten Bedingungen erfüllt
\begin{enumerate}[(i)]
...
Man nennt $L\supseteq K$ einen (den) \Index{algebraischen Abschluss} von $K$.
%sub end

\subsection{Satz 30, Eindeutigkeit des algebraischen Abschlusses}
\label{sub:satz_30}
