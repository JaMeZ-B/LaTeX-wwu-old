...
\vspace*{\fill}
\begin{center}
	Hierbei handelt es sich um eine Aufarbeitung der Vorlesungsmitschriften von \textbf{Prof. Dr. Kramer}, WWU Münster, aus der Vorlesung \textbf{Einführung in die Algebra} im Wintersemester 2014/15. Dies ist kein Skript der Vorlesung und keine eigene Arbeit des Autors.\\
	\vspace{2cm}
...
\minisec{Algebra}: 
historisch das aüflösen von Gleichungen. Moderne Algebra untersucht sog. algebraische Strukturen wie Gruppen, Ringe, Körper, Varietäten,$\dots$\\
\minisec{Literatur:}
\begin{itemize}
	\item Cohn   Basic Algebra
	\item Jacobsen    Basic Algebra I
	\item Herstein    Topics in Algebra
	\item Laug   Algebra
	\item Bosch   Algebra
	\item Lorenz   Einführung in die Algebra
\end{itemize}
(die beiden letzten Bücher gehen vor allem auf die Körpertheorie ein)

\minisec{Zur Vorlesung:}
Regelmäßige Teilnahme + \uline{Mitschreiben}. Meine eigenen Notizen gibt es dann immer im www eingescannt (\uline{kein} Skript)

\minisec{Übungen}
Regelmäßige Teilnahme, vorrechnen. Zwei Namen auf Hausaufgaben, wenn \uline{beide} alles vorrechnen können.\\
Regelmäßige Abgabe + min. eine Aufgabe erfolgreich vorrechnen + $50+x\%$ richtig $\Rightarrow$ Klausurzulassung.
%sub end
