...
\begin{enumerate}[(1)]
	...
	\item Auch $X := \mathds{R}^2 \setminus \set[(x,y)]{x \in (0,\infty) , y \in [0,1]} $ ist kontrahierbar.  \todo[noline]{Beschriftungen hinzufügen}
	\[
		\begin{tikzpicture}
...
\begin{enumerate}[a)]
	...
	$C^\infty$-Abbildungen\todo{Hyphenation!!!} sind.
\end{enumerate}
% subsection 152 (end)
...
\uline{Behauptung:} $\ind_n$ ist regulärer Wert von $f$. Dann folgt: $O(n)$ ist eine Untermannigfaltigkeit von $\mathds{R}^{n \times n} \cong \mathds{R}^{n^2}$.
...
\[
	...
	\end{cases}
\]
Dies ist eine $C^\infty$-Funktion (Analysis II.) \todo[noline]{Zeichnung vervollständigen}
\[
	\begin{tikzpicture}
		\draw[->] (0,-0.5) -- (0,1.5);
		\draw[->] (-3,0) -- (3,0);
		\draw[DarkGray, dashed] (-3,1)--(3,1);
	\end{tikzpicture}
\]
...
\missingfigure{Graph von $\varphi_\varepsilon$}
...
\begin{enumerate}[(i)]
	\item Jede (kompakte) $C^\infty$-Mannigfaltigkeit lässt sich in den $\mathds{R}^N$ für $N >> \dim M$ einbetten.
