...
Sei $\psi : \mathds{R} \to \mathds{R}$ gegeben durch 
\[
	\psi(t) := \begin{cases}
		0, &\text{ falls }t \le 0\\
		e^{-t^{-2}} ,&\text{ falls } t >0
	\end{cases}
\]
Dies ist eine $C^\infty$-Funktion (Analysis II.)
\begin{figure}[H]
	\centering{
	\begin{tikzpicture}[xscale=0.8, yscale=1]
		\draw[->, thick] (0,-0.3) -- (0,1.5);
		\draw[->, thick] (-1,0) -- (4.1,0);
		\draw[gray, dashed] (0,1) node[left]{$1$} --(4,1);
		\draw[variable=\t, domain=0.01:4, smooth, very thick, Sienna3] (-0.9,0) -- (0,0) -- plot({\t},{exp(- \t^(-2))}) node[below left]{$\psi$};
	\end{tikzpicture}
	\caption{Erster Schritt zu einer Glockenfunktion, die Funktion $\psi$}}
\end{figure}
Für $\varepsilon>0$ sei $\varphi_\varepsilon : \mathds{R} \to \mathds{R}$ gegeben durch $\varphi_\varepsilon(t) = \frac{\psi(t)}{\psi(t) + \psi(\varepsilon-t)}$
\todo[noline]{Zeichnung vervollständigen und für einheitliches Aussehen sorgen (\texttt{pgfplots})}
\begin{figure}[H]
	\centering{
	\begin{tikzpicture}[xscale=1.6]
		\draw[gray, dashed] (0.5,1.3) node[left]{$1$} --(2.5,1.3);
		\begin{axis}[hide x axis, hide y axis, x=1cm, y=1cm, ymin=-0.3, ymax=1.5, xmin=-0.5, xmax=2.2]
			% \draw[help lines] (axis cs: -2,-2) grid (axis cs: 2,2);
			\draw[->, thick] (axis cs: -0.5,0) -- (axis cs: 2.2,0);
			\draw[->, thick] (axis cs: 0,-0.3) -- (axis cs: 0,1.5);
			\addplot[smooth, thick, DodgerBlue2, domain=-0.45:2] table {Mathematica/data2.csv};
		\end{axis}
		\node[below left, DodgerBlue2] at (2.5,1.3) {$\varphi_\varepsilon$};
	\end{tikzpicture}}
\end{figure}
Für $r>0$ sei $f_{\varepsilon,r} : \mathds{R}^n \to \mathds{R}$ gegeben durch $f_{\varepsilon,r}(x) = 1- \varphi_\varepsilon(\norm{x}-r)$
\missingfigure{Graph von $f_{\varepsilon,r}$}
$f_{\varepsilon,r} \in C^\infty(\mathds{R}^n)$, Werte von $f_{\varepsilon,r} \subseteq [0,1]$.
$f_{\varepsilon,r}(x) = 1$ für $\abs*{x} \le r $, $f_{\varepsilon,r}(x)=0$ für $\abs*{x} \ge r +\varepsilon$.
\minisec{Beweis}
...
\label{sub:182}
$S^n$ heißt \Index{parallelisierbar}, falls es einen Homöomorphismus  $\Theta : T^n S^n\to S^n\times \mathds{R}^n$ gibt, so dass für jedes $x \in S^n$ die Einschränkung auf
$\Theta\big|_{T_x^n S^n} : T^n_x S^n \to \set{x}\times \mathds{R}^n$ ein $\mathds{R}$-Vektorraumisomorphismus ist. Unter den Sphären sind genau $S^1$, $S^3$ und $S^7$
...
\[
	T^n M := \set[(x,v)]{x \in M, v \in T_x^n M} \subseteq M \times \mathds{R}^{n+k} 
\]
\minisec{Bemerkung}
...
\begin{description}
	...
	\end{align*}
	Da $h \circ om : (-\varepsilon,\varepsilon) \to \mathds{R}^n \times \set{0}$ folgt $\D(h \circ \omega)_0 (1) \in \mathds{R}^k \times \set{0}$. \bewende
\end{description}
% subsection 184 (end)
