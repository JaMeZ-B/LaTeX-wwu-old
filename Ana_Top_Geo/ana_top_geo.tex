...
\begin{enumerate}[(1)]
	...
	\]
	für $(n,m) \not= (n',m')$ und $\varepsilon < \frac{1}{2}$. Der Quotient $G\backslash \mathds{R}^1 =: K$ heißt die \Index{Kleinsche Flasche}. Wir erhalten eine Wirkung
	$\mathds{R}^2 \to K$. 
	\begin{figure}[h]
		\centering{
...
Decktransformationen von $p$ bilden eine Gruppe $\triangle(p)$. Diese Gruppe wirkt in kanonischer Wiese auf $\hat{X}$.
% subsection 12.6 (end)

\subsection{Lemma} % (fold)
\label{sub:12.7}
Sei $p : \hat{X} \to X$ eine Überdeckung wobei $\hat{X}$ wegzusammenhängend ist. Dann ist die Wirkung der Decktransformationsgruppe $\triangle(p)$ auf $\hat{X}$ eigentlich
diskontinuierlich.
\minisec{Beweis}
Wir zeigen zunächst, dass die Wirkung frei ist. Sei $f \in \triangle(p)$ und $x \in \hat{X}$ mit $f(x) = x$. Sei $y \in \hat{X}$ und $\hat{\omega} : [0,1] \to \hat{X}$ ein
...
\begin{wrapfigure}{R}{3cm}
	\begin{tikzcd}
		\hat{X} \rar{p} \dar{q}& X \\
		H \backslash \hat{X} \urar{q'}&
	\end{tikzcd}
\end{wrapfigure}
Sei $p : \hat{X} \to X$ eine Überlagerung wobei $\hat{X}$ wegzusammenhängend ist. Sei $H \le \triangle(p)$ eine Untergruppe. Dann ist auch die Wirkung $H \wirkung \hat{X}$
...
Insgesamt haben wir also jeder Untergruppe von $\triangle (p)$ eine Überlagerung $H \backslash \hat{X}$ zugeordnet, die zwischen $\hat{X}$ und $X$ liegt.
% subsection 12.8 (end)

\subsection{Definition} % (fold)
\label{sub:12.9}
Sei $p : \hat{X} \to X$ eine Überlagerung. Für $x \in X$ wirkt dann $\triangle(p)$ auf der Faser $F_x := p ^{-1}(x)$. Die Überlagerung heißt 
\bet{normal}\index{Überlagerung!normal}, falls diese Wirkung transitiv ist, d.h. falls es zu $\hat{x}, \hat{y} \in F_x$ immer $f \in \triangle(p)$ gibt mit 
$f(\hat{x})= \hat{y}$.
% subsection 12.9 (end)

\subsection{Proposition} % (fold)
\label{sub:1210}
Sei $\hat{X} \xrightarrow{p} X $ eine normale Überdeckung wobei $\hat{X}$ wegzusammenhängend ist. Dann ist die Abbildung $q' : \triangle(p)\backslash X \to X$,
$q' \enbrace*{\triangle(p) x} = p(x)$ ein Homöomorphismus.
\minisec{Beweis}
