...
\begin{enumerate}[(i)]
	\item Sei $\varphi : [0,1] \to [0,1]$ mit \marginnote{Zeichnung einfügen}.
	Dann gilt 
	\begin{align*}
...
\begin{enumerate}[(1)]
	\item Eine Wirkung $G \cap X$ von $G$ auf $X$ ist eine Abbildung $G \times X \to X$, $(g,x) \mapsto g \cdot x$ so dass \marginnote{hier ist noch ein Zeichen falsch}
	\begin{itemize}
...
\subsection{Lemma} % (fold)
\label{sub:12.2}
Sei $G \cap X$ eine e.d.k Wirkung. Dann ist $p : X \to G \backslash X$, $x \mapsto Gx$ eine Überlagerung.
\minisec{Beweis}
...
\label{sub:12.3}
\begin{enumerate}[(1)]
	\item $\mathds{Z}^n \cap \mathds{R}^n$ mit $z \cdot x := x + z$ ist eine e.d.k. Wirkung: Ist $\varepsilon < \frac{1}{2}$ so gilt 
	\[
...
\label{sub:12.4}
Sei $X$ wegzusammenhängend und einfach zusammenhängend. Sei $G \cap X$ eine e.d.k. Wirkung. Für jedes $\overline{x}_0 \in G \backslash X$ ist dann 
\[
