...
\begin{enumerate}[a)]
	\item Eine \Index{Karte} für $M$ ist ein Homöomorphismus $h : U \xrightarrow{\cong}  V$ mit $U \subseteq M$ offen, $V\subseteq\mathds{R}^n$ offen. $U$ heißt das 
	\Index{Kartengebiet} von $h$. Ist $x \in U$, so heißt $h$ eine \bet{Karte um} $x$.
	\item Sind $h_i : U_i \xrightarrow{\cong} V_i$, $i=0,1$ zwei Karten, so heißt 
	\[
