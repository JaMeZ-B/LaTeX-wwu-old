...
\begin{enumerate}[(i)]
	...
	\[
		...
		\end{cases}
	\]
	wird auf $X$ eine Metrik (die \Index{diskrete Metrik}) definiert.
...
Das Urbild von $\mathds{R}^n \times \set{0}$ unter $\D h_x$ ist der \bet{Tangentialraum}\index{Tangentialraum!Untermannigfaltigkeit} $T^u_x M$ von $M$ im Punkt
...
\begin{description}
	...
	$I^{n-1} \times [0,1] \to X$: \todo{noch nicht fertig}
	\[
		\begin{tikzcd}[column sep=4.5em]
			I^{n-1} \times \set{0} \dar[hook] \rar["\omega\big|_{I^{n-1} \times \set{0}}"] & \overline{X} \dar["p"]\\
