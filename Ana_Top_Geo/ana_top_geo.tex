...
\label{sub:103}
\begin{wrapfigure}{r}[2cm]{0.25\textwidth}
	\vspace{-20pt}
	\centering{
	\begin{tikzpicture}[scale=0.65, font=\Large]
		\begin{axis}[
			z post scale=3,
			xmin=-1.5, xmax=2,
			view={15}{18},
		...
		\draw[dashed] (axis cs: 1,0,-18) -- (axis cs: 1,0,27);
		\addplot3[
			domain=0:2*pi,
			smooth,
			samples=50,
			...
			({cos(deg(x))}, {sin(deg(x))}, -17) node[pos=0, pin=0:$1 \in S^1$, minimum size=0.09cm, draw=black, thick, fill=Tomato3, inner sep=0.008cm, circle]{};
		\addplot3[
			domain=-1*pi:9*pi,
			smooth,
			samples=100,
		...
		\foreach \t in {0,1,2,3,4} {\edef\temp{\noexpand
			\node[pin=0:$\t$, minimum size=0.09cm,draw, thick,circle, fill=Tomato3, inner sep=0.008cm] at (axis cs: 1,0,\t*2*pi){}; 
		} \temp}
		\node at (axis cs: 0,0,33) {$\mathds{R}$};
		% \foreach \t in {0,1,2} {\draw (axis cs: 0,0,0)-- (axis cs: 0,0,\t); };
		\end{axis}
	\end{tikzpicture}
	\caption{Die Vorstellung bei der Windungszahl}}
\end{wrapfigure}
