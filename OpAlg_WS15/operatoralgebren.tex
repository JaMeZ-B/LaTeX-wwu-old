...
\begin{definition}[{name=[Charakter]}]
	...
	Es gilt dann $\chi(\ind)=1$. Wir setzen $\Omega(A) := \set{\chi \given \chi \text{ Charakter auf }A}$ (\Index{Charakterspektrum} oder einfach nur \Index{Spektrum} von $A$).
	\todo{Revision}
\end{definition}
...
\begin{definition}[{name=[{selbstadj, normal, unitär, Proj., Isometrie}]}]
...
\begin{beweis}
	...
	\begin{align}
		...
		\le \norm*{\iota(a) + \lambda \cdot \ind_{\mathcal{B}(A)}}_{\mathcal{B}(A)} 
	\end{align}
	Damit folgt, dass \eqref{eq:26:1} und \eqref{eq:26:2} Gleichungen sind und $\norm*{\cdot }_{\mathcal{B}(A)}$ eine $C^*$-Norm auf $A^\simm$ ist. Da $A$ unital ist, gilt
...
\begin{definition}[{name=[Spektrum in nichtunitalen $C^*$-Algebren]}]
	...
	\]
	Wieder schreiben wir $\Omega(A) = \set*{\chi \colon A \to \mathbb{C} \text{ Charaktere}}$ und \marginnote{Für $A$ unital sind maximale Ideale automatisch abgeschlossen}
	\[
