...
\begin{satz}[{name={GNS-Konstruktion}},label=gns]
	...
	Wir zeigen nun, dass $\pi_\varphi \colon A \to \mathcal{B}(\mathcal{H}_\varphi)$ ein kontraktiver $^*$-Homomorphismus ist.
	Dazu zeigen zunächst $\dot{\pi}(a^*)=\dot{\pi}(a)^*$. Für $b +N_\varphi,c+N_\varphi \in \sfrac{A}{N_\varphi}$ gilt
	\[
		\skal*{c+N_\varphi}{\dot{\pi}(a) b+N_\varphi} = \varphi(c^*ab) = \varphi \enbrace*{(a^*c)^*b} = \skal*{a^*c +N_\varphi}{b+N_\varphi}= \skal*{\dot{\pi}(a^*)c+N_\varphi}{b+N_\varphi}
	\]
	Da $\dot{\pi}$ stetig fortgesetzt wird, gilt dann auch $\pi(a^*)=\pi(a)^*$.\todo{Kontraktivität klären}
...
\begin{beweis}
	...
	\end{align}
	Damit ist $\pi_u$ injektiv also treu.\todo{Revision 5}
	...
	Für $a \in A^1$ existiert dann ein $i \in \mathbb{N}$ mit $\varphi_i(a^*a) \ge \frac{1}{2} \norm*{a}^2$.
	Dann ist 
	\[
		\norm*{\pi_{\varphi_i}(a)} \ge \norm[\big]{\pi_{\varphi_i}(a) \lim\nolimits_\lambda \enbrace*{e_\lambda + N_{\varphi_i}}}_{\mathcal{H}_{\varphi_i}}\hspace{-1ex}
		= \varphi_i \enbrace*{a^*a}^{\sfrac{1}{2}} \ge \frac{1}{\sqrt{2}}\cdot \norm*{a}
	\]
	...
	\]
	Mit obiger Ungleichung folgt $\norm*{\pi_{\sep}(a)} \ge \sfrac{1}{\sqrt{2}} \norm*{a}$ und damit ist $\pi_{\sep}$ treu.\todo{Hier verstehe ich nicht warum}
\end{beweis}
...
\begin{satz}[label=satz:62,{name=[monotone Netze selbstadjungierter Operatoren]}]
	...
	Falls $(x_\lambda)_\Lambda$ ein absteigendes Netz aus Projektionen ist, so ist $x$ die Projektion auf $\bigcap_\Lambda\overline{x_\lambda \mathcal{H}}$.\todo{Revision 6}
\end{satz}
\begin{beweis}
...
\begin{beweis}
	...
	\begin{enumerate}[(i)]
		...
		Für Projektionen gilt\todo{nachgucken, ob wir das schonmal gezeigt haben}
		\[
		...
		Wähle eine Folge $(x_n)_\mathbb{N} \to \xi_1$.\todo{fertig machen}
		\begin{align}
