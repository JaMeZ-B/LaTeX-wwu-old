...
\begin{korollar}[label=korr:220]
	...
	\]
	\begin{enumerate}[1)]
		\item Wenn $A$ und $B$ unital sind und $\ind_A=\ind_B$ ist, dann gilt sogar $\sigma_A(a) = \sigma_B(a)$.
...
\begin{definition}[{name=[Positivität]}]
	Sei $A$ eine $C^*$-Algebra. Ein Element $a \in A$ heißt \Index{positiv} ($a \ge 0$), wenn $a=a^*$ und $\sigma(a) \subseteq [0,\infty)$. Mit $A_+$ bezeichnen wir die Menge der 
	positiven Elemente. Wenn $b,c \in A_{\sa}$ sind, dann  \todo{Revision 3}
	\[
		b \le c :\iff c-b \ge 0
	\]
\end{definition}
...
\begin{bemerkung}
	\begin{enumerate}[(i)]
		...
		\item Für $a \in A = C_=(X)$ ist $a \ge 0$ genau dann, wenn $a(x) \ge 0$ für alle $x \in X$ gilt (nach \autoref{bem217}).
	\end{enumerate}
\end{bemerkung}
