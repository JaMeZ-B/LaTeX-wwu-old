...
\end{korollar}
\begin{beweis}
	Die Menge $F:= \set*{\varphi \in S(A^\simm) \given \varphi|_{B}=\rho}$ ist nichtleer nach \autoref{satz:57} und es ist $F \subset S(A^\simm)$ $\w^*$-abgeschlossen, also $\w^*$-kompakt.
	Weiter ist $F$ auch wieder konvex.
	$F$ ist eine Seite von $S = \set*{\varphi \colon A^\simm \to \mathbb{C}\given \text{ positiv, linear, } \norm*{\varphi}\le 1}$ (Übung).
