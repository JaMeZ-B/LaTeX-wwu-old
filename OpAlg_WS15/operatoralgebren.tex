...
\begin{beispiel}[{name=[Toeplitz-Algebra]}]
	...
	\[
		...
		\end{tikzcd}
	\]
	Für jede unitale $C^*$-Algebra $A$ mit einer Isometrie $w$, das heißt $w^*w=\ind$, existiert ein $^*$"=Homomorphismus $\varphi\colon\mathcal{T}\to A$ mit $\varphi(v)=w$, dies besagt der \emph{Satz von Coburn}.\todo{Revision 5}
\end{beispiel}
...
\begin{beispiel}
	...
	Es ist $v_i^* v_i=\ind$ und $v_1v_1^* + v_2v_2^*=\ind$.
	\[
		\Oc_2 := C^*(v_1,v_2) \subset \mathcal{B}(\mathcal{H}) 
	\]
	ist eine \href{https://de.wikipedia.org/wiki/Cuntz-Algebra}{\Index{Cuntz-Algebra}}. Ähnlich lassen sich $\Oc_n$ und $\Oc_\infty$ definieren.
