...
\begin{satz}[name={Gelfand},label=satz16:gelfand]
	Sei $A \neq 0$ eine abelsche $C^*$-Algebra. Dann ist $\widehat{\hspace{5pt}} \colon A \to C_0 \enbrace*{\Omega(A)}$\marginnote{$C_0 \enbrace*{\Omega(A)} = C \enbrace*{\Omega(A)}$, 
...
\begin{beweis}
	...
	\]
	Damit ist $a \mapsto \widehat{a}$ isometrisch. Damit folgt, dass $C := \set*{\widehat{a} \given a \in A} \subset C_0 \enbrace*{\Omega(A)}$ abgeschlossen ist. Außerdem ist 
	$C$ eine
...
\begin{beweis}
	...
	\begin{figure}[t]
		...
		\begin{tikzpicture}[scale=0.5]
			\draw[->,thick] (-3,0) -- (3,0);
			\draw[->,thick] (0,-3) -- (0,3);
			\draw[Firebrick2,very thick,dashed] (-3,0) -- (0,0) -- (3,3) node[below]{$f$};
			\draw[SeaGreen4,very thick,dashed] (-3,-3) -- (0,0) -- (2.8,0) node[below left]{$g$};
		\end{tikzpicture}
