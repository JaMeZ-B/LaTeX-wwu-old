...
\begin{erinnerungA}[{name=[schwache Operatortopologie]}]
...
\begin{beweis}
	...
	Sei also $\varphi$ $\so$-stetig. Die Menge $S_\varphi := \set*{x \in \mathcal{B}(\mathcal{H}) \given \abs*{\varphi(x)}<1} \subset \mathcal{B}(\mathcal{H})$ ist dann $\so$-offen und es existieren $\xi_1, \ldots ,\xi_n \in \mathcal{H}$ und ein $\alpha>0$, sodass 
	\[
		U_{0,\xi_i,\alpha}= \set[\big]{y \in \mathcal{B}(\mathcal{H}) \given \norm*{(y-0)\xi_i}< \alpha}  \subset S_\varphi
	\]
	Das heißt für $x \in \mathcal{B}(\mathcal{H})$ gilt: Falls $\norm*{x \xi_i}<\alpha$ ist für alle $i$, so ist $\abs*{\varphi(x)}<1$.\todo{Das stimmt so noch nicht, hier muss noch der Schnitt der $U_{0,\xi_i,\alpha}$ vorkommen}
	Nach Skalierung der $\xi_i$ gilt für $x \in \mathcal{B}(\mathcal{H})$:
	Ist $\norm*{x \xi_i} < 1$ für alle $i$, so ist auch $\abs*{\varphi(x)}<1$.
	Für $x \in \mathcal{B}(\mathcal{H})$ gilt also\todo{die erste Ungleichung sehe ich nicht ein} 
	\begin{equation}
		\abs*{\varphi(x)}  \le \enbrace*{\sum_{i=1}^{n} \norm*{x \xi_i}^2}^{\sfrac{1}{2}} \label{eq:619}\tag{\dag}
	\end{equation}
	Setze nun 
	\[
	...
	Dann ist $\psi \colon \mathcal{H}_0 \to \mathbb{C}$, $\begin{psmallmatrix}
		x \xi_1 \\ \cdot \\ x \xi_n
	\end{psmallmatrix} \mapsto \varphi(x)$ wohldefiniert, stetig und linear (benutze \eqref{eq:619}).
	\todo{wie die Stetigkeit folgt ist klar, Wohldefiniertheit nicht unbedingt}
	$\psi$ setzt sich also fort zu $\overline{\psi} \colon \overline{\mathcal{H}_0} \to \mathbb{C}$.
	Mit Riesz folg die Existenz eines eindeutigen $\eta \in \overline{\mathcal{H}_0}$ mit $\overline{\psi}(.) = \skal*{\eta}{.}$, also
	\[
		\varphi(x) = \overline{\psi} \begin{pmatrix}
			x \xi_1 \\ \vdots \\ x \xi_n
		\end{pmatrix} = \sum_{i=1}^{n} \skal*{\eta_i}{x \xi_i}
	\]
	wobei $\eta_i$ die Komponenten von $\eta$ sind.
\end{beweis}
...
\begin{proposition}[label=prop:623,{name=[jede beschränkte stetige Funktion ist stark stetig]}]
	Jedes $h \in C_b(\mathbb{R})$ ist stark stetig.\todo{Revision 6}
\end{proposition}
\begin{beweis}
...
\begin{satz}[label=satz:624,name={Kaplansky's Dichtesatz}]
	Sei $\mathcal{H}$ ein Hilbertraum, $A \subset \mathcal{B}(\mathcal{H})$ eine $C^*$-Algebra und $B= \overline{A}^\so=\overline{A}^\wo$ die von $A$ in $\mathcal{B}(\mathcal{H})$ erzeugte von-Neumann-Algebra.\marginnote{Dass $B$ vN-Algebra ist in Aufgabe 1 von Blatt 10 gezeigt}
	Dann gilt
	\begin{enumerate}[(i),itemsep=2pt]
...
\begin{beweis}
	...
	\begin{enumerate}[(i)]
		\item Falls $a \in B_\sa$, so existiert $(a_\lambda)_\Lambda \subset A$ mit $a_\lambda \grenzw{\wo}a$.
		 ...
		 Wir haben also $B^1_\sa \subseteq \overline{A^1_\sa}^\so$ gezeigt. Die andere Inklusion ist klar.
		 \item Wie (ii) mit $g$ anstatt $f$.
		 \item Sei $a \in B^1$.
		 Dann ist $\begin{psmallmatrix*}[l] 0 & a \\ a^* & 0 \end{psmallmatrix*} \in M_2(B)_\sa \subset \mathcal{B} \enbrace*{\mathcal{H}^{\oplus 2}}$. 
