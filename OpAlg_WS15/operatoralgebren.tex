...
\begin{beispiel}[{name=[{Zustände,Spurzustände}]}]
	...
	\begin{enumerate}[(i)]
		...
		\[
			...
			\end{tikzcd}
		\]
		Die Menge rechts unten ist eine abelsche, unitale $C^*$-Algebra. Wähle einen Charakter $\chi$. Man sieht auch, dass die freien Ultrafilter in 1-zu-1-Zusammenhang zu $\beta \mathbb{N} \setminus \mathbb{N}$ stehen.\todo{Revision 5}
	\end{enumerate}
\end{beispiel}
...
\begin{erinnerungA}[{name=[starke Operatortopologie]},label=erinnerung:61]
	...
	Für ein Netz $(x_\lambda)_\Lambda \subset \mathcal{B}(\mathcal{H})$ und $x \in \mathcal{B}(\mathcal{H})$ gilt\marginnote{Damit ist $\mathcal{T}_\so$ die Topologie der punktweisen Konvergenz für Operatoren}
	\begin{align}
...
\subsection{GNS-Konstruktion ist zyklisch} % (fold)
\label{sub:gns__zyklisch}
\todo[inline]{Aus Notizen übernehmen}
