...
\begin{satz}[label=satz:1.14,{name=[Spektrum entspricht Menge der $\chi(a)$]}]
	Sei $A$ eine abelsche unitale Banachalgebra. Für $a \in A$ gilt $\sigma(a)=\set[\big]{\chi(a) \given \chi \in \Omega(A)}$. \todo{Revision 1}
\end{satz}
\begin{beweis}
...
\begin{beweis}
	...
	ist. Sei $\chi_\gamma \xrightarrow{\raisebox{-3pt}{\scriptsize$\w^*$}}\chi$ ein Netz. $\chi_\gamma$ ist multiplikativ für jedes $\gamma$ und $\chi_\gamma(\ind)=1$. Damit gelten diese Eigenschaften auch für 
	$\chi$.
\end{beweis}
...
\end{satz}
\begin{beweis}
	$\widehat{a} \colon \Omega(A) \to \mathbb{C}$ ist stetig nach Definition der $\w^*$-Topologie: Für $U \subset \mathbb{C}$ offen ist
	\[
	...
	\]
	und somit offen in $A^*$. Damit ist $\widehat{a}^{-1}(U)$ offen in der Relativtopologie. Wir haben
	\begin{align}
	...
	Also 
	\[
		\norm{a} \StackText{\ref{spec_kompakt}}{\le} r(a) = \sup_{\lambda \in \sigma(a)} \abs*{\lambda} = \sup_{\chi \in \Omega(A)} \abs{\widehat{a}(\chi)} 
...
\begin{definition}[{name=[erzeuge C*-Algebra]}]
	...
	Dann bezeichnet $C^*(S) \subset A$ die kleinste $C^*$-Unteralgebra von $A$, die $S$ enthält; also \todo{Revision 2}
	\[
...
\begin{beweis}
	...
	\mapdef{\alpha \colon\Omega \enbrace[\big]{C^*(a,\ind)}}{\widehat{a} \enbrace[\big]{\Omega \enbrace*{C^*(a,\ind)}}\StackText{\ref{satz:117}}{=} \sigma_{C^*(A,\ind)}\subset \mathbb{C}}%
	...
	$\chi_1(a)= \widehat{a}(\chi_1) = \widehat{a}(\chi_2) = \chi_2(a)$ ist, so ist $\chi_1|_{^*\Alg(a,\ind)} = \chi_2|_{^*\Alg(a,\ind)}$ und mit der Stetigkeit von $\chi_1$ und $\chi_2$ 
	folgt, dass $\chi_1=\chi_2$ sein muss.\todo{Abstand * und Alg fixen} Die Inverse ist stetig, da $\Omega \enbrace*{C^*(a,\ind)}$ kompakt ist.
	
	Wir erhalten nach \autoref{satz16:gelfand} $C^*(a,\ind) \cong C \enbrace*{\Omega(C^*(a,\ind))}$ mittels $a \mapsto \widehat{a}$. und 
	$C \enbrace*{\Omega(C^*(a,\ind))} \cong C \enbrace*{\sigma_{C^*(a,\ind)}(a)}$ mittels $\widehat{a} \mapsto \widehat{a} \circ \alpha^{-1}$. Für 
	$\alpha(\chi)=t \in \sigma_{C^*(a,\ind)}(a)$ haben wir 
	\[
		\enbrace*{\widehat{a} \circ \alpha^{-1}}(t) = \enbrace*{\widehat{a} \circ \alpha^{-1}} \enbrace*{\alpha(\chi)} = \widehat{a}(\chi) = \alpha(\chi)=t
	\]
	Also ist $\widehat{a} \circ \alpha^{-1}=\id_{\sigma_{C^*(a,\ind)}(a)}$. Wir nenen die Komposition der beiden Isomorphismen $\beta$.\todo{schöner machen}
	...
	\[
		\begin{tikzcd}
			...
			a \rar[mapsto] & \widehat{a} \rar[mapsto] & \id_{\sigma_{C^*(a,\ind_{A^\simm})}(a)}
		\end{tikzcd}
	\]
...
\begin{satz}[{name=[Existenz von Zuständen zu gegebenen normalen Elementen]}]
...
\begin{beispiel}
	...
	\begin{enumerate}[(i)]
		...
		\[
			A \hookrightarrow \Faktor{\prod_\mathbb{N} A_i}{\bigoplus_\mathbb{N}(A_i)}
		\]
