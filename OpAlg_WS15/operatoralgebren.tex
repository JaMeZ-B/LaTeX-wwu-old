...
\begin{satz}[{name={GNS-Konstruktion}},label=gns]
	...
	Dazu zeigen zunächst $\dot{\pi}(a^*)=\dot{\pi}(a)^*$. Für $[b],[c] \in \sfrac{A}{N_\varphi}$ gilt
	\[
		\skal*{[c]}{\dot{\pi}(a) [b]} = \varphi(c^*ab) = \varphi \enbrace*{(a^*c)^*b} = \skal*{\benbrace*{a^*c}}{[b]}= \skal*{\dot{\pi}(a^*)[c]}{[b]}
	\]
	Da $\dot{\pi}$ stetig fortgesetzt wird, gilt dann auch $\pi(a^*)=\pi(a)^*$.
...
\begin{satz}
	...
	Dann ist $(\mathcal{H},\pi)$ eine direkte Summe von irreduziblen Darstellungen.
\end{satz}
% section 7 (end)
