...
\begin{satz}
	...
	Dann existiert eine (bis auf Isomorphie) eindeutig bestimmte $C^*$-Algebra $\underrightarrow{\lim}(A_n,\varphi_n)$ mit folgender universeller Eigenschaft:
	\begin{itemize}
		\item Es existieren $^*$-Homomorphismen $\varphi_{n,\infty} \colon A_n \to \underrightarrow{\lim}(A_n,\varphi_n)$ mit $\varphi_{n,\infty} = \varphi_{n+1,\infty} \circ \varphi_{n}$.
		\item Falls $\psi_n \colon A_n \to B$ für $n \in \mathbb{N}$ $^*$-Homomorphismen sind mit $\psi_n = \psi_{n+1} \circ \varphi_n$, so existiert ein eindeutig bestimmter $^*$-Homomorphismus $\psi \colon \underrightarrow{\lim}(A_n,\varphi_n) \to B$ mit $\psi_n = \psi \circ \varphi_{n,\infty}$ für $n \in \mathbb{N}$.
	\end{itemize}
	Kürzer:
	\[
		\begin{tikzcd}[column sep=large,row sep=huge,arrows={sloped,pos=.37}]
			& & & & & \underrightarrow{\lim}(A_n,\varphi_n) \ar[dd,"\exists!",dashed,sloped=false,pos=.5] \\
			A_0 \rar \ar[urrrrr,"\varphi_{0,\infty}",gray,end anchor={[yshift=.7ex]}] \ar[drrrrr,"\psi_0"',gray,end anchor={[yshift=.7ex,xshift=-.1ex]south west}] & \ldots  \rar 
			& A_n \rar \ar[urrr,"\varphi_{n,\infty}",end anchor={[yshift=.4ex]}] \ar[drrr,"\psi_n"',end anchor={[yshift=-.2ex,xshift=.5ex]}] 
			& A_{n+1} \rar \ar[urr,"\varphi_{n+1,\infty}"',end anchor={[yshift=-.1ex,xshift=-.2ex]}] \ar[drr,"\psi_{n+1}",end anchor={[yshift=0.5ex]}] 
			& \ldots \\
			& & & & & B
		\end{tikzcd}
	\]
	\todo[inline]{Pfeile hübscher machen}
\end{satz}
% section 4 (end)
