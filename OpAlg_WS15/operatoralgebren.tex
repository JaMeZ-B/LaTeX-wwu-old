...
\begin{lemma}[label=lem:311]
	Sei $A$ eine $C^*$-Algebra. Die Menge $\Lambda := \set*{a \in A_+ \given \norm*{a} < 1}$ ist gerichtet bezüglich \enquote{$\le$}.
\end{lemma}
\begin{beweis}
...
\end{satz}
\begin{beweis}
	Sei $\Lambda := \set*{a \in A_+ \given \norm*{a} < 1}$ wie \autoref{lem:311} und setze $e_\lambda := \lambda$ für $\lambda \in \Lambda$. 
...
\begin{satz}
	...
	\[
		B \subher \iff bac \in B \text{ für alle } b,c \in B, a \in A 
	\]
\end{satz}
\begin{beweis}
...
\begin{beweis}
	Die Menge
	\[
		\prod_\mathbb{N} A_n := \set*{(a_n)_\mathbb{N} \given a_n \in A_n, \sup_n \norm*{a_n} < \infty}
	\]
...
\begin{definition}[{name=[positives Funktional]}]
