...
\begin{beweis}
	...
	\begin{enumerate}[(i)]
		...
		Falls $-1 \notin \sigma(u) \subset \mathbb{T}$, so ist $h=f(u) \in A_\sa^1$ und $u=e^{i \pi h}$.\todo{Revision 6}
		...
		\]
		\todo{bei dem $e$-Term kommt wieder $u(\ind-p)$ raus meine ich \ldots}\qedhere
	\end{enumerate}
\end{beweis}
...
\begin{bemerkung}[{name=[Folgerungen aus \autoref{korr:613}]}]
	...
	\begin{enumerate}[a),itemsep=0pt]
		\item Aus (i) folgt insbesondere, dass $A= \overline{\Span \set*{p \in A \given p\text{ ist Projektion}}}$.
...
\begin{satz}[{name=[Einheitskugel der beschränkten Operatoren ist wo-kompakt]}]
...
\begin{satz}[{name={Kadisons Transitivitätssatz}}]
...
\begin{beweis}
	\begin{enumerate}[1.]
		...
		Exkurs:\todo{vlt in den Anhang auslagern} Es ist $\pi(A)$ $\so$-dicht in $\mathcal{B}(\mathcal{H})$: 
		...
		\[
			\begin{tikzcd}[sep=small]
				...
				\mbox{   }\hspace{1em}\pi(A) \uar[phantom,sloped,"\subset","\text{\footnotesize dicht}" yshift=-1em] \rar[phantom,sloped,description,"\subset"] & \pi(A)'' \uar[phantom,sloped,description,"\subset"] \urar[equal]
			\end{tikzcd}
		\]
