...
\[
A\in \Gamma(\underbrace{TM\otimes T^* M\otimes T^* M}_{\End(TM\otimes T^* M)}) \hat{=} A:\Gamma(TM)\to\Gamma(\End T^*M), X\mapsto A_X
\]
$\Rightarrow \nabla^A=\nabla+A$ definiert durch 
\[
\nabla_x^A Y = \nabla_x Y+A_x Y \in \Gamma(TM)\ \text{wobei}\ A_X = C_2^1(X\otimes A
\]
