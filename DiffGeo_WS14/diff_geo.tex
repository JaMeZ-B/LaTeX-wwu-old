...
\centering{
\begin{tikzpicture}[scale = 0.6]
	%\draw [help lines] (0,0) grid (18,5);
	\draw (1,1) .. controls +(180:0.5) and +(270:0.5) .. (0,1.5)
	 .. controls +(90:1) and +(140:0.5) .. (2,2.8)
	 .. controls +(320:1.5) and +(180:1) .. (5,2.5)
	 .. controls +(0:0.5) and +(90:1) .. (6,1.5)
	 .. controls +(270:0.5) and +(0:0.5) .. (5,0.5)
	 .. controls +(180:2) and +(0:2) .. (1,1);

	\draw (0.5,1.5) .. controls +(90:0.5) and +(160:0.5) .. (1.5,2.2);
	\draw (0.5,1.5) .. controls +(0:0.25) and +(240:0.25) .. (1.5,2.2);	 
	 
	\draw (4.5,1.2) .. controls +(80:0.25) and +(160:0.25) .. (5,1.7);
	\draw (4.5,1.2) .. controls +(350:0.4) and +(320:0.25) .. (5,1.7);	
	
	\draw (11,1) .. controls +(90:1.5) and +(180:1.5) .. (15,3)
	 .. controls +(0:1) and +(160:0.5) .. (17,2.5)
	 .. controls +(340:1) and +(0:1) .. (16,0.2)
	 .. controls +(180:0.5) and +(0:1) .. (13.5,1.2)
	 .. controls +(180:0.5) and +(0:0.5) .. (11.5,0.5)
	 .. controls +(180:0.35) and +(270:0.35) .. (11,1); 

	\draw (15.5,1.1) .. controls +(20:0.4) and +(160:0.4) .. (16.5,1);
	\draw (15.5,1.1) .. controls +(320:0.4) and +(220:0.4) .. (16.5,1);
	
	\draw (15.4,2.5) .. controls +(0:0.4) and +(140:0.4) .. (16.2,2.2);
	\draw (15.4,2.5) .. controls +(290:0.4) and +(200:0.4) .. (16.2,2.2);	
	
	\draw[->] (6.5,1.7) .. controls +(30:0.5) and +(180:0.5) .. (8.5,2.1) node[above]{$\phi$ Diffeo}
	 .. controls +(0:0.5) and +(150:0.5) .. (10.5,1.7);
	 
	\draw[->] (6.5,1) -- (8.5,1) node[below]{$\mathd_x \phi$} -- (10.5,1);
	
	\draw[fill=OrangeRed2] (5.4,0.9) node[below]{$x$} circle[radius=0.05cm];
	\draw[->] (5.4,0.9) -- (5.9,1.3);
	
	\draw[fill=OrangeRed2] (12,1.7) node[below]{$f(x)$} circle[radius=0.05cm];
	\draw[->] (12,1.7) -- (12.7,1.8);
	
	\draw[dashed] (4.5,0.9) -- (5.5,1.7) -- (6.3,0.7) -- (5.3,-0.1)  node[below]{$T_x M$} -- (4.5,0.9);
	
	\draw[dashed] (11.5,1.9) -- (12.5,2.7) -- (13.3,1.7) node[right]{$T_x N$} -- (12.3,0.9) -- (11.5,1.9);
\end{tikzpicture}
\caption{Abbildung zwischen Tangentialräumen}}
\end{figure}
...
\item $\underbrace{C^\infty (M)}_{\text{Ring}}$-Modul: $f\in C^\infty (M), S\in \Gamma(\mathcal{T}_q^p M)$, so gilt $\enbrace{fS}_x = f(x)\cdot S_x$
\end{enumerate}
\end{minipage}}
\end{figure}
...
\label{ssub:183}
\begin{enumerate}[(i)]
\item Ist $f:M\to N$ ein Diffeomorphismus, so kann man den \Index{Pullback} auf kontravarianten Tensoren via dem \Index{Push-Forward} definieren, nämlich ist zum Beispiel $X\in \Gamma(TN)$, so ist: $f^*X:= (f^{-1})_* X \in \Gamma(TM)$. Also $f^*:\Gamma(\mathcal{T}_q^p N)\to \Gamma(\mathcal{T}_q^p M)$.
...
\label{ssub:24}
\begin{enumerate}[(i)]
\item Eine Abbildung $p:M\to N$ heißt \Index{Überlagerung} von $N$, falls für alle $y\in N$ eine Umgebung $y\in V\subset N$ existiert, sodass $p^{-1}(V) = \dot{\bigcup}_j U_j$ disjunkte Vereinigung offener $U_j \subset M$ mit $p\vert_{U_j}$ ist Diffeomorphismus auf sein Bild.
...
\subsubsection{Definition}
\label{ssub:27}
Eine \Index{kovariante Ableitung} auf $M$ ist eine $\mathds{R}$-lineare Abbildung
\[
...
\begin{align*}
\enbrace{\nabla_X Y}(x) &= \enbrace{\nabla_X Y\vert_U}(x)\\
&=\enbrace*{\sum_i \enbrace{\mathd Y^i(X)}\partial_i + Y^i\nabla_X \partial_i}(x)\\
&=\enbrace*{\sum_{i,j}\enbrace{X^j \mathd  Y^i(\partial_j)}\partial_i+ X^j Y^i\underbrace{\nabla_{\partial_j}\partial_i}_{=\sum \Gamma_{ij}^k \partial_k \in \Gamma(TM\vert_U)}}(x)\\
&=\enbrace*{\sum_k \enbrace*{\sum_j\enbrace*{X^j \mathd Y^k(\partial_j)+\sum_i X^j Y^i \Gamma_{i,j}^k}}\partial_k}(x)\\
...
&= \sum_k \enbrace*{  \mathd_x Y^k(X(x)) + \sum_{j,i} X^j(x)Y^i(x)\Gamma_{i,j}^k(x)}\partial_k(x)
\end{align*}
...
\begin{itemize}[(i)]
\item $\frac{\nabla}{\mathd t}(f Y)(t) = \dot{f}(t)Y(t) + f(t)\frac{\nabla}{\mathd t} Y(t)$ für $f\in C^\infty(I)$
...
\begin{align*}
	...
	& \overset{(ii)}{=} \sum_i \dot{Y}^i(t)\partial_i(\gamma(t)) + Y^i(t) \enbrace*{\nabla_{\dot{\gamma}(t)} \partial_i}(\gamma(t))
\end{align*}
...
\subsubsection{Beispiel}
\label{ssub:212}
Sei $\mathds{R}^n$ mit flacher kovarianter Ableitung gegeben, so folgt aus dem Beweis $\ref{ssub:211}$ direkt, dass für ein Vektorfeld $X:I\to \mathds{R}^n$ entlang $\gamma$ gilt:
\[
\frac{\nabla}{\mathd t} X(t) = \frac{\mathd}{\mathd s} X(s)\vert_t = \dot{X}(t)
\]
...
So wähle lok. Koordinaten $x^1,\ldots,x^n$ auf $U$ mit $x \in U$. So können wir die Metrik $g$ darstellen $g\vert_U = \sum g_{ij} \mathd x^i \otimes \mathd x^j$. Definiere auf $U$ zweite Metrik: $g_{\text{eukl}}\vert_U = \sum \mathd x^i \otimes \mathd x^i$. Betrachte einen Ball um $x$ $B_r(x) = \set[p]{\mathd^{g_{\text{eukl}}}(x,p) < r}$, sodass $y \not\in B_r(x)$ (hier wird die Hausdorff-Eigenschaft von $M$ genutzt). Da $(g_{ij})$ eine symmetrische pos. def. Matrix für alle $x\in U$ existiert ein kleinster Eigenwert $\lambda > 0$ von $(g_{ij})$ und es folgt $\abs{v}_{g}^2 = g(v,v) \geq \lambda^2 g_{\text{eukl}}(v,v) =\lambda^2 \abs{v}_{g_{\text{eukl}}}^2$. Sei $\gamma$ nun eine Kurve von $x$ nach $y$. So verlässt diese Kurve zwingend $B_r(x)$ und es folgt somit
\[
...
\[
F:Q:=(-\epsilon,\epsilon)\times I \to M
...
\item $\nabla_X^F V = \nabla_{\bar{X}}^g \tilde{V}$, falls $V(q) = \tilde{V}(F(q))$, $\tilde{V}\in \Gamma(TM)$
\end{itemize}
...
\[
(\nabla_X^F V)(q) = \sum \mathd_q V^i(X(q))\partial_i(F(q))+V^i(q)\enbrace*{\nabla_{X(q)}\partial_i(F(q))}
\]
...
\]
\minisec{Beweis}
Seien $V = \bar{\partial}_s, T = \bar{\partial}_t$. (Bemerkung: $V,T \in \Gamma(F^* TM),\ V(0,t) = V_\gamma(t), T(s,t) = \mathd_{(s,t)} F(s,t) = \dot{\gamma}_s(t)$) Mit $\ref{ssub:235}$ folgt, dass $\partial_s g(\dot{\gamma}_s,\dot{\gamma}_s) = 2g(\nabla_{\partial_s}^F T,T)$. Dann folgt
\begin{align*}
...
\label{ssub:238}
Eine glatte, nach Bogenlänge parametrisierte Kurve $\gamma:I\to M$ heißt \Index{Geodäte}, falls
\[
...
Also heißt dies, dass $\gamma$ Geodäte genau dann, wenn die Beschleunigung $\ddot{\gamma}$ normal zu $M$ ist.
...
\label{sub:25}
Im folgenden Teil stellen wir uns die Frage, wann Geodäten längenminimierend sind. Wir haben beobachtet, sei $v\in T_x M$ und $\gamma_v$ eindeutig bestimmte Geodäte mit $\gamma_v(0) = x$ und $\dot{\gamma}_v(0) = v$. Seien $s\in \mathds{R}$ und $\tilde{\gamma}(t) = \gamma_v (st)$ mit $\dot{\tilde{\gamma}} = sv$, so folgt aus der Eindeutigkeit $\gamma_v(st) = \tilde{\gamma}(t) = \gamma_{sv}(t)$. Insbesondere für $t=1$: $\gamma_v(s) = \gamma_{sv}(1)$.\\
...
\subsubsection{Korollar}
\label{ssub:246}
Sei $x\in M$. Dann existiert $\epsilon>0$ und eine Umgebung $U$ von $x$ mit: Je zwei Punkte in $U$ können durch eine Geodäte der Länge kleiner $\epsilon$ verbunden werden. Außerdem ist die Einschränkung von $\exp_y$ auf $B_\epsilon (0_y)$ ein Diffeomorphismus auf das Bild.
\minisec{Beweis}
Nach $\ref{ssub:245}$ existiert eine Umgebung $\mathcal{W}$ von $0_y$ in $TM$ mit $\widetilde{\exp}\vert_\mathcal{W}$ Diffeomorphismus auf das Bild. Außerdem existiert eine Umgebung $W$ um $x$ in $M$ und $\epsilon>0$, sodass $\mathcal{W}_\epsilon := \bigcup_{y\in W} B_\epsilon(0_y) \subset \mathcal{W}$. Da $(x,x) = \exp(0_x)$ gilt $(x,x) \in \widetilde{\exp}(\mathcal{W}_\epsilon)$ und somit existiert eine offene Menge $U\subset M$ um $x$, sodass $U\times U\subset \widetilde{\exp}(\mathcal{W}_\epsilon)$. Sind also $p,q\in U$, dann existiert ein (eindeutiges) $v\in \mathcal{W_\epsilon}$, sodass $(p,q) = \enbrace{\pi(v), \exp_{\pi(v)} v}$.
Schließlich folgt
\[
L(\gamma_v \vert_{\benbrace{0,1}}) = \int_0^1 \abs{\dot{\gamma}_v}\mathd t = \abs{v} < \epsilon,\quad \text{da}\ v\in B_\epsilon (0_p)
\]
\\
Wir können uns nach $\ref{ssub:245}$ $\exp$ auch als Familie von Karten abhängig von $x$ vorstellen. So können wir annehmen, dass $\exp_x: \mathcal{V}_x \subset T_x M \to U_x \subset M$ ein Diffeomorphismus auf das Bild ist. Wählen wir eine Orthonormalbasis $e_1,\ldots,e_n$ auf $T_x M$ so gibt und $\exp^{-1}_x: U_x \to \mathds{R}^n$ eine Karte um $x$. Die resultierenden Koordinaten nennen wir \Index{Normalkoordinaten}.
...
\[
\sum_{i,j} \Gamma_{ij}^k (x) a^i a^j = 0\quad \forall a^i \in \mathds{R}
\]
Damit folgt $\Gamma_{ij}^k = 0$.
\end{enumerate}
...
\item "$(GV_1) \Rightarrow (HB)$": Sei $K\subset M$ abgeschlossen und beschränkt, so folgt aus Theorem $\ref{ssub:253}$, dass ein $R$ existiert mit $K\subset B_R(x) := \exp_x(B_R(0_x))$. Sei nun $\tilde{K} := \exp^{-1}(\underbrace{K}_{\text{abg.}}) \subset B_R (0_x) \subset T_x M$ abgeschlossen und beschränkt. Mit Heine-Borel folgt nun, dass $\tilde{K}$ kompakt ist. ($T_x M$ ist ein endlich dimensionaler $\mathds{R}$-VR). Da $\exp_x$ stetig ist folgt somit, dass $\exp_x(\tilde{K}) = K$ kompakt ist.
...
\subsubsection{Satz}
\label{ssub:256}
Sei $p:(M,g)\to (N,h)$ lokale Isometrie. $(M,g)$ ist geodätisch vollständig genau dann, wenn $p$ ist Riemannsche Überlagerung und $(N,h)$ geodätisch vollständig.
