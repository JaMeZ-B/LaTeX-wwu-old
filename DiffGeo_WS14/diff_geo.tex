	...
	\item 
		...
		\centering
			\begin{tikzpicture}[yscale = 0.4, xscale= 0.6]
				...
				\draw[SteelBlue2,->] (0.1,4.9) node[above]{$\mathd_0 g = 0$} -- (0.9,4.1);
			\end{tikzpicture}
		\end{figure}
...
Dann gilt zum Beispiel $U \overset{offen}{\subset} \mathds{R}^n$ ist Untermannigfaltigkeit im "abstrakten" Sinne. Untermannigfaltigkeit $N\subset \mathds{R}^n$ im Sinne von $1.1$ ist Untermannigfaltigkeit im abstrakten Sinne.
	...
	\end{figure}
		...
		\item Der $n$-Torus $T^n$. Betrachte $f:\mathds{R}^n \to \mathds{C}^n,\ \enbrace{x_1, \ldots, x_n} \mapsto \enbrace{\exp(ix_1), \ldots, \exp(ix_n)}\in \underbrace{S^1\times \ldots \times S^1}_{n-\text{mal}} \subset \mathds{C}\times \ldots \times \mathds{C}$, so folgt $f(\mathds{R}^n) = T^n,\ f(a_1,\cdot,a_n) = (e^{ia_1},\cdot,e^{ia_n})\in T^n$.
\begin{figure}[H]
	\centering{
