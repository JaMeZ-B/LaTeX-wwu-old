...
\[
\benbrace*{\mathcal{L}_X,\mathcal{L}_Y}:= \mathcal{L}_X \cdot \mathcal{L}_Y - \mathcal{L}_Y \cdot \mathcal{L}_X 
\]
...
\]
wobei $I_X$ max. Intervall von $c_x$. Die glatte Abbildung $\psi_X:\mathcal{D}_X \to M,\ (x,t)\mapsto \psi_X(x,t) = c_x(t)$ heißt \Index{globaler Fluss}. $X$ heißt vollständig, falls $\mathcal{D} = M\times \mathds{R}$
