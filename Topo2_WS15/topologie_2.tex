...
\begin{beispiel}[{name=[$0$-te Kohomologie]}]
...
\begin{definition}[label=sec3:def_cup,{name=[Cup-Produkt]}]
	...
	\]
	durch $(\alpha \cupp \beta)(\sigma) \coloneqq  (-1)^{p \cdot q} \cdot \alpha\enbrace*{\sigma|_{\benbrace{0,\ldots ,p}}} \cdot \beta \enbrace*{\sigma|_{\benbrace*{p, \ldots , p+q}}}$ für $\sigma \colon \abs{\Delta^{p+q}} \to X$. \todo{Revision 3}
\end{definition}
...
\begin{lemma}[{name=[Eigenschaften des Cup-Produktes]}]
	...
	\begin{enumerate}[1)]
		...
		\item Für $f \colon X \to X'$ mit $f(A) \subseteq A'$, $f(B) \subseteq B'$ gilt $f^*(\alpha \cupp \beta)= f^*(\alpha) \cupp f^*(\beta)$ für $\alpha \in C^p_\sing(X',A';R)$, 
...
\begin{definition}[{name=[Cup-Produkt in Kohomologie]}]
	...
	\[
		\cupp \colon H^p(X,A;R) \otimes H^q(X,B;R) \longrightarrow H^{p+q}(X,A \cup B;R)
	\]
	ist das $\cupp$-Produkt in Kohomologie $\benbrace*{\alpha} \cupp \benbrace*{\beta} := \benbrace*{\alpha \cupp \beta}$. Mit 1) folgt, dass das Cup-Produkt in Kohomologie 
...
\begin{definition}[{name=[Kreuz-Produkt]}]
	Seien $(X,A)$ und $(Y,B)$ zwei Paare von topologischen Räumen. Seien $p_X \colon X \times Y \to X$ und $p_Y$ die Projektionen. 
	Das \bet{Kreuz-Produkt} ($\times$"~Produkt)\index{Kreuz-Produkt, $\times$-Produkt}
	\[
		\times \colon H^p(X,A;R) \otimes H^q(Y,B;R) \longrightarrow H^{p+q}(X \times Y,A \times Y \cup X \times B ;R)
	\]
...
\begin{bemerkung}[{name=[Cup-Produkt auf $C^*$ nicht graduiert kommutativ]}]
	Das $\cup$-Produkt auf $C^*_\sing(X;R)$ ist \emph{nicht} graduiert kommutativ, was man direkt aus der \autoref{sec3:def_cup} ablesen kann. \todo{Revision 4}
\end{bemerkung}

\begin{lemma}
	Für $\sigma \colon \abs{\Delta^p} \to X$ sei $\overline{\sigma} \colon \abs{\Delta^n} \to X$ definiert durch 
	\[
		\overline{\sigma}(t_0, \ldots , t_p ) := (-1)^{\frac{p(p+1)}{2}} \sigma(t_p, \ldots ,t_0)
	\]
	Sei $\Sp \colon C_p(X,A;R) \to C_p(X,A;R)$ definiert durch $\Sp(\sigma) := \overline{\sigma}$. Sei $\Sp^* \colon C^p(X,A;R) \to C^p(X,A;R)$ definiert durch 
	$\Sp^*(\alpha) := \alpha \circ \Sp$. Es gilt
	\begin{enumerate}[a)]
		\item $\Sp$ ist eine Kettenabbildung, $\Sp^*$ ist eine Kokettenabbildung
		\item Ist $R$ kommutativ, so gilt $\beta \cup \alpha = (-1)^{p \cdot q}\Sp^*\enbrace*{\Sp^*(\alpha) \cup \Sp^*(\beta)}$
	\end{enumerate}
\end{lemma}
\begin{beweis}
	\begin{enumerate}[a)]
		...
		\begin{align}
			\Sp(d_p \sigma) = \Sp\enbrace*{\sum_{i=0}^{p} (-1)^i \sigma|_{\benbrace*{0, \ldots , i\hspace{-3.5pt}/, \ldots ,p}}} 
			&= \sum_{i=0}^{p} (-1)^i \Sp \enbrace*{\sigma|_{\benbrace*{0, \ldots , i\hspace{-3.5pt}/, \ldots , p}}} \\
			&= \sum_{i=0}^{p} (-1)^i (-1)^{\frac{p(p-1)}{2}} \enbrace*{\sigma|_{\benbrace*{p, \ldots , i\hspace{-3.5pt}/, \ldots , 0}}}
		\end{align}
		wobei $i\hspace{-3.5pt}/$ an der $p-i$-ten Stellen von vorn steht. Weiter ist
		\begin{align}
			d_p \enbrace[\big]{\Sp(\sigma)} = d_p \enbrace*{(-1)^{\frac{p(p+1)}{2}} \sigma|_{\benbrace*{p, \ldots ,0}}} &= (-1)^{\frac{p(p+1)}{2}} \sum_{i=0}^{p} (-1)^i 
			\sigma|_{\benbrace*{p, \ldots , \cancel{p-i}, \ldots ,0}} \\ 
			&= (-1)^{\frac{p(p+1)}{2}} \sum_{i=0}^{p} (-1)^{p-i} \sigma|_{\benbrace*{p, \ldots ,i\hspace{-3.5pt}/, \ldots ,0}}
		\end{align}
		Nun ist $\frac{p(p-1)}{2} \equiv \frac{p(p+1)}{2} + p =   $ \todo{fertig machen} und es folgt die Gleichheit.
		\item Für $\sigma \colon \abs{\Delta^n} \to X$ gilt 
		\begin{align}
			\enbrace*{\Sp^* \enbrace*{\Sp^*(\alpha) \cup \Sp^*(\beta)}}(\sigma) &= (-1)^{\frac{n(n+1)}{2}} \cdot \enbrace*{\Sp^*(\alpha) \cup \Sp^*(\beta)} 
			\enbrace*{\sigma|_{\benbrace*{n, \ldots ,0}}} \\
			&= (-1)^{\frac{n(n+1)}{2}} (-1)^{p \cdot q} \Sp^*(\alpha) \enbrace*{\sigma_{\benbrace*{n, \ldots , n-p}}} \cdot \Sp^*(\beta) 
			\enbrace*{\sigma|_{\benbrace*{n-p, \ldots ,0}}} \\
			&= (-1)^{\frac{n(n+1)}{2}} (-1)^{p \cdot q} (-1)^{\frac{p(p+1)}{2}} (-1)^{\frac{q(q+1)}{2}} \alpha \enbrace*{\sigma|_{\benbrace*{q, \ldots ,n}}}
			\beta \enbrace*{\sigma|_{0, \ldots ,q}} \\
			&= (-1)^{\frac{n(n+1)}{2}} (-1)^{p \cdot q} (-1)^{\frac{p(p+1)}{2}} (-1)^{\frac{q(q+1)}{2}} \beta \enbrace*{\sigma|_{0, \ldots ,q}} 
			\alpha \enbrace*{\sigma|_{\benbrace*{q, \ldots ,n}}} \\
			&\stackrel{!}{=} (-1)^{p \cdot q} \enbrace*{\beta \cup \alpha}(\sigma) = \cancel{(-1)^{p \cdot q}} \cancel{(-1)^{p \cdot q}} \beta \enbrace*{\sigma|_{0, \ldots ,q}} 
			\alpha \enbrace*{\sigma|_{\benbrace*{q, \ldots ,n}}}
		\end{align}
		Weiter gilt nun
		\begin{align}
			\frac{n(n+1)}{2} + \frac{p(p+1)}{2} + \frac{q(q+1)}{2} &= (1+ \ldots + n) + (1+ \ldots + p) + (1+ \ldots + q)\\ 
			&\equiv \enbrace[\big]{(p+1) + \ldots + n} + (1+ \ldots + q)\\
			&= (n+1) q = (p+q+1) q = pq + q^2 + q \equiv p \cdot q
		\end{align}
		da $q^2 \equiv q$. \qedhere
	\end{enumerate}
\end{beweis}
...
\begin{satz}[{name=[Das $\cup$-Produkt in Kohomologie ist graduiert kommutativ]}]
	Das $\cup$-Produkt in Kohomologie ist graduiert kommutativ.
\end{satz}
\begin{beweis}
	Seien $\xi \in H^p(X,A;R)$, $\eta \in H^q(X,B;R)$. Wähle $\alpha \in C^p(X,A;R)$, $\beta \in C^q(X,B;R)$ mit $\xi= [\alpha]$ und $\eta= [\beta]$. Dann ist 
	\begin{align}
		\xi \cup \eta = \benbrace*{\alpha \cup \beta} = (-1)^{pq} \cdot \benbrace*{\Sp^* \enbrace[\big]{\Sp^*(\beta) \cup \Sp^*(\alpha)}} &= (-1)^{pq} \cdot 
		\benbrace[\big]{\Sp^*(\beta) \cup \Sp^*(\alpha)} \\
		&= (-1)^{pq} \cdot \benbrace[\big]{\Sp^*(\beta)} \cup \benbrace[\big]{\Sp^*(\alpha)}\\ 
		&= (-1)^{pq} \cdot [\beta] \cup [\alpha] = (-1)^{pq} \cdot \eta \cup \xi \qedhere
	\end{align}
\end{beweis}
...
\begin{lemma}
	Sei $A \subseteq X$. Das $\capp$-Produkt auf $C^*$ induziert folgende $\cap$-Produkte in $H^*$
	\[
		\begin{tikzcd}[row sep=0em]
...
\begin{lemma}
	Für $f \colon X \to Y$ stetig gilt $\xi \capp f_*(x) = f_* \enbrace*{f^* \xi \capp x}$ für $\xi \in H^p(X;R)$ und $x \in H_{p+q}(X;R)$.
\end{lemma}
...
\begin{satz}[{name={Dualitätssatz von Poincaré für geschlossene Mannigfaltigkeiten}},label=poincare]
	...
	Dann ist $\capp \mu \colon H^p \to H_{n-p}(M;R)$ ein Isomorphismus.
\end{satz}
...
\begin{satz}
	...
	\begin{enumerate}[(i)]
		...
		\item $H^p \enbrace*{\mathbb{C}P^n,\mathbb{Z}} \cong \Faktor{\mathbb{Z}[n]}{\eta^{n+1}=0}$ mit $\abs{\eta}=2$.
	\end{enumerate}
\end{satz}
\begin{beweis}
...
\begin{satz}
	...
	Sei $M$ weiter homotopieäquivalent zu einen endlichen CW-Komplex.\footnote{Dies ist tatsächlich immer der Fall, wie Aufgabe 4 von Blatt 5 zeigt} Dann ist $\chi_M =0$.
\end{satz}
\begin{beweis}
...
\begin{bemerkung}
	Am Beweis der Poincaré-Dualtität hindern uns momentan noch folgende Probleme, die wir in diesem Kapitel \enquote{umschiffen} wollen
	\begin{itemize}
		\item Es ist für $n>0$ $H^{n-*}(\mathbb{R}^n) \cong H_*(\mathbb{R}^n)$
		\item Es gibt keine Fundamentalklasse in $H_n(\mathbb{R}^n)$.
	\end{itemize}
\end{bemerkung}

\begin{erinnerungA}
	Sei $M$ eine (nicht notwendig kompakte) Mannigfaltigkeit und $K \subseteq M$ kompakt. Sei $n := \dim M$. Es gilt
	\begin{enumerate}[(i)]
		\item Ist $(\mu_x)_{x \in M}$ eine $R$-Orientierung von $M$, so gibt es eine eindeutige Klasse $\mu_K \in H_n(M,M \setminus K;R)$ mit 
		\[
...
\begin{beispiel}
	Sei $B=B_R(x_0)$ ein kompakter Ball in $\mathbb{R}^n$. 
	Dann ist $H_n(\mathbb{R}^n,\mathbb{R}^n \setminus B;R) \cong R$ und jeder Erzeuger $\mu_B \in H_n(\mathbb{R}^n,\mathbb{R}^n \setminus B;R)$ ist eine Fundamentalklasse über $B$.
	$\mu_B|_x \in H_n(\mathbb{R}^n,\mathbb{R}^n \setminus \set*{x};R)$ ist ein Erzeuger für $x \in B$.
\end{beispiel}
...
\begin{beweis}
	...
	\[
		H^n(\mathbb{R}^n,\mathbb{R}^n \setminus B;R) \longrightarrow \Hom_R \enbrace*{H_n(\mathbb{R}^n,\mathbb{R}^n \setminus B;R),R}
	\]
	immer surjektiv ist, gibt es $\xi \in H^n \enbrace*{\mathbb{R}^n,\mathbb{R}^n \setminus B;R}$ mit $\xi(\mu_B)=1$. 
	Also $\epsilon(\xi \capp \mu_B)=1$. Folglich ist $\xi \capp \mu_B$ ein Erzeuger von $H^0(\mathbb{R}^n;R)$ und $\capp \mu_B$ ein Isomorphismus.
\end{beweis}

\begin{definition}[{name=[gerichtete Menge]}]
	Eine \Index{gerichtete Menge} ist eine Menge $\Lambda$ zusammen mit einer partiellen Ordnung \enquote{$\le$}, sodass es für alle $\lambda,\lambda' \in \Lambda$ ein $\mu \in \Lambda$ mit $\lambda \le \mu $ und $\lambda' \le \mu$.
\end{definition}
...
\begin{beispiel}
	...
	\begin{itemize}[itemsep=0pt] 
		...
		\item $\mathcal{K}(X)= \set*{K \subseteq X \given K \text{ kompakt}}$ mit \enquote{$\subseteq$}
	\end{itemize}
\end{beispiel}
...
\begin{definition}[{name=[Kolimes]}]
	Sei $(\Lambda,\le)$ eine gerichtete Menge und $F \colon (\Lambda,\le) \to R\text{-}\MOD$ ein Funktor. Der \Index{Kolimes} von $F$ ist definiert als 
	\[
		\colim_\Lambda F := \Faktor{\coprod_{\lambda \in \Lambda}F(\lambda)}{v \sim F \enbrace{i_\lambda^{\lambda'}}(v), v \in F(\lambda), \lambda \le \lambda'}
	\]
	Für $\lambda \in \Lambda$ sei $s_{\lambda_0} \colon F(\lambda) \to \colim_\Lambda F$ die Komposition von der Inklusion $F(\lambda_0) \to \coprod_{\lambda \in \Lambda}F(\lambda)$ mit der Projektion $\coprod_{\lambda \in \Lambda}F(\lambda) \twoheadrightarrow \colim_\Lambda F$. Durch
	\begin{align}
...
\begin{bemerkung}
	...
	\begin{itemize}
		...
		\begin{enumerate}[(i)]
			...
			\item \todo{Kategorie mit zwei Objekten und keinen nichttrivialen Morphismen}
		\end{enumerate}
...
\begin{beweis}
	...
	\]
	Injektivität: Sei $v \in \ker c$. Sei also $\lambda_0 \in \Lambda_0$, $v_{\lambda_0} \in F(\lambda_0)$ mit $v=s_{\lambda_0}(v_{\lambda_0})$. 
...
\end{korollar}
\begin{beweis}
	$\set*{\lambda_\infty} \subseteq \Lambda$ ist kofinal.
\end{beweis}
...
\begin{beispiel}\leavevmode
	\begin{itemize}
		...
		Dann ist $\colim_{\mathcal{P}\mkern-2.5mu_{\mathrm{endl}}} F \cong R[S]$ mit $s_{s_0} \colon R[S_0]\to R(S)$ die von $S_0 \subseteq S$ induzierte Abbildung.
		...
		Dann ist
		\[
			\colim F = \Faktor{\coprod_{n \in \mathbb{N}} \mathbb{Z}}{\sim} = \Faktor{\mathbb{N}\times \mathbb{Z}}{\scriptstyle(n,z) \sim (n',2^{n'-n} z), n < n'} \longrightarrow \mathbb{Z}\benbrace*{\sfrac{1}{2}} = \set*{\sfrac{a}{2^n}\given a \in \mathbb{Z}, n \in \mathbb{N} }
		\]
...
\begin{beweis}
	...
	Wir zeigen nur $\colim_\Lambda \tau$ ist injektiv. 
	Sei $v \in \ker (\colim_\Lambda \tau)$.
	Wähle $v_\lambda \in F(\lambda)$ mit $s_\lambda(v_\lambda)=v$.
	Dann ist $\tau_*(v)=s_\lambda'\enbrace[\big]{\tau_\lambda \enbrace*{v_\lambda}}$.
	Da $\tau_*(v)=0$ ist, gibt es $\mu \ge \lambda$ mit 
	\[
		F'(i_\lambda^\mu) \enbrace*{s_\lambda'\enbrace[\big]{\tau_\lambda \enbrace*{v_\lambda}}}=0
	\]
	...
	\end{align}
	Da $\tau_\mu$ injektiv ist, ist $F(i_\lambda^\mu)(v_\lambda)=0$. Damit ist $v=s_\lambda(v_\lambda)=0$.
\end{beweis}
% section 7 (end)
