...
\end{lemma}
\begin{beweis}
	Folgt offensichtlich aus \autoref{prop:H0_kettenhomotop}.\todo[inline]{muss eventuell noch angepasst werden}
\end{beweis}
...
\begin{beweis}
	...
	\[
		\eta \longmapsto \xi \enbrace*{\eta \capp \mu} = \epsilon \enbrace[\big]{\xi \capp (\eta \capp \mu)} = \epsilon \enbrace[\big]{(\xi \cupp \eta) \capp \mu} = (\xi \cupp \eta)(\mu) = \sprod*{\xi}{\eta}_K
	\]
...
\begin{beweis}
	\begin{enumerate}[a)]
		...
		\item Folgt aus folgender Proposition.\todo{ref}\qedhere
	\end{enumerate}
\end{beweis}
...
\begin{beweis}
	...
	\[
		\begin{tikzcd}[row sep=large, column sep=3.15em]
			H_0(P_*) \dar["0"] & P_0 \lar \dar["f_0-g_0"] \drar[sloped,pos=.4,"s_0"] & P_1 \lar \dar["f_1-g_1"] \drar[sloped,pos=.4,"s_1"] & P_2 \lar \dar["f_2 - g_2"] & \ldots \lar & P_{n-1} \lar\dar["f_{n-1}- g_{n-1}"'] \drar[sloped,pos=.3,"s_{n-1}"] & P_n \lar \dar["f_n - g_n"]  \drar[dashed,sloped,pos=.4,"s_n"] & P_{n+1} \lar \\
...
\begin{definition}[{name=[projektive Dimension]}]
	...
	\[
		...
		\end{tikzcd}
	\] 
	Gibt es kein solches $n$, so sagen wir die projektive Dimension von $R$ ist $\infty$.
\end{definition}
...
\begin{lemma}
	...
	\begin{enumerate}[a)]
		...
		\[
			\begin{tikzcd}[sep=large]
				H_=(P_*) \rar["H_0(F)"] \dar["\epsilon","\cong"'] & H_0(P_*') \dar["\epsilon'","\cong"'] \rar["H_0(F')"] & H_0(P_*'') \dar["\epsilon''","\cong"'] \\
				M \rar & M' \rar & M''
			\end{tikzcd}
		\]
	\end{enumerate}
...
\begin{frage}
	...
	\[
		H* \enbrace*{V' \otimes_R C_*(X;R)}= H_*(X;V') \cong V' \otimes_R H_*(X;R)
	\]
...
\begin{bemerkung}
	...
	\[
		\begin{array}{ll}
			H_* \enbrace*{F(C_*)} = F_* \enbrace*{H_*(C_*)} & \text{ falls $F$ kovariant ist} \\
			H^* \enbrace*{F(C_*)} = F \enbrace*{H_*(C_*)} & \text{ falls $F$ kontravariant ist} 
		\end{array}
	\]
\end{bemerkung}
...
\begin{lemma}
	...
	\begin{enumerate}[(i)]
		\item \( \begin{tikzcd}[cramped,sep=2.3em]
				...
				\end{tikzcd} \)
		\item $\begin{tikzcd}[cramped,sep=small]
