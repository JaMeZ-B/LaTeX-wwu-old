...
\section{Kommutativität von $\cup$} % (fold)
\label{sec:4}
...
\begin{beweis}
	\begin{enumerate}[a)]
		...
		\begin{align}
			...
			&\equiv \enbrace*{(p+1) + \ldots + n} + (1+ \ldots + q)\\
...
\begin{beweis}
	...
	\begin{align}
		\xi \cup \eta = \benbrace*{\alpha \cup \beta} = (-1)^{pq} \cdot \benbrace*{\Sp^* \enbrace*{\Sp^*(\beta) \cup \Sp^*(\alpha)}} &= (-1)^{pq} \cdot 
		\enbrace*{\Sp^*(\beta) \cup \Sp^*(\alpha)} \\
		&= (-1)^{pq} \cdot \benbrace*{\Sp^*(\beta)} \cup \benbrace*{\Sp^*(\alpha)}\\ 
		&= (-1)^{pq} \cdot [\beta] \cup [\alpha] = (-1)^{pq} \cdot \eta \cup \xi \qedhere
	\end{align}
\end{beweis}

\begin{proposition}
	Sei $\tau_* \colon C_*^\sing \to C_*^\sing$ eine natürliche Transformation durch Kettenabbildungen. Induziert $\tau$ die Identität auf $H_0(\set*{\pt})$, so ist $\tau_*$
	kettenhomotop zur Identität, das heißt es gibt eine natürliche Transformation $\eta_* \colon C_*^\sing \to C_*^\sing$ mit
	\[
