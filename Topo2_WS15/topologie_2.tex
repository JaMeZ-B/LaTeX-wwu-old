...
\begin{beweis} Wir betrachten folgendes Diagramm
	...
	Betrachte dazu folgenden Ausschnitt aus der Paarsequenz des Paares $(D^1, \partial D^1)$:
	\[
	...
	\[
		...
		\end{tikzcd}
	\]
	zerfällt: Sei dazu $s \colon D^1 \times X \to \partial D^1 \times X$ gegeben durch $(t,x) \mapsto (0,x)$ und $i \colon \partial D^1 \times X \hookrightarrow D^1 \times X$ die Inklusion.
	Dann ist $i \circ s$ homotop zu $\id_{D^1 \times X}$, also $s^* \circ i^*=\id$ in Kohomologie und mit der Exaktheit folgt $j^*=0$. Die gesuchte Spaltung ist $s^*$.\todo{Revision 3}
	...
	\]
	Für das zweite $\times$-Produkt betrachte \todo{Diagramm vervollständigen}
	\[
		\begin{tikzcd}
...
\begin{beweis}
	...
	Die Behauptung folgt mittels Ausschneidung und Homotopieinvarianz:
	\[
...
\end{korollar}
\begin{beweis}
	Betrachte $\tau-\tau'$ und wende \autoref{prop:H0_kettenhomotop} an. \todo{Revision 4}
\end{beweis}
...
\end{satz}
\begin{beweis}
	Per Induktion nach $n$. Für $n=0$ ist nichts zu zeigen. 
...
\begin{beispiel}[{name=[Freie Moduln sind projektiv]}]
	Freie Moduln sind projektiv, wie man durch Anwendung der universellen Eigenschaft eines freien Moduls sehr einfach sieht (wurde auch in Topologie \RM{1} bewiesen). 
\end{beispiel}
...
\begin{beweis}
	...
	Dann gilt $f \circ \hat{\varphi} = f \circ \hat{\psi} \circ i = \varphi \circ p \circ i = \varphi$ und $P$ ist projektiv. \todo{Revision 5}
\end{beweis}
...
\begin{definition}[{name=[Schnittform]}]
	...
	heißt die \Index{Schnittform} von $M$. \todo{Revision 10}
\end{definition}
...
\begin{definition}[{name=[projektive Auflösung]}]
	...
	Wir schreiben oft $(P_*,\epsilon)$ für eine solche Auflösung, wobei $P_*$ ein Kettenkomplex projektiver Moduln ist, $H_k(P_*)=0$ für $k>0$ und $H_0(P_*)\cong M$ via $\epsilon$ ist.\todo{Revision 11}
\end{definition}
...
\begin{satz}
	...
	\)
	eine kurze exakte Folge von $R$-Moduln.
	Dann gibt es lange exakte Folgen \todo{Verbindungsho\-momorphismen hinzufügen}
	\begin{enumerate}[(i)]
		\item \[
			\begin{tikzcd}
...
\begin{lemma}
	...
	\begin{enumerate}[(i)]
		...
		\[
			\begin{tikzcd}
				0 \rar & E^2_{1,n-1} \rar & A_n \rar & E^2_{0,n} \rar & 0
			\end{tikzcd}
		\]
...
\begin{satz}[{name={Zerlegungssatz}}]
	Sei $C_*$ ein Kettenkomplex über einem Hauptidealring.
	Seien die Kettenmodule von $C_*$ frei.
	Dann gibt es Kettenkomplexe $C_*^{[i]}$, $i \in \mathbb{N}$ mit 
	\begin{itemize}
		\item $C_*^{[i]}=0$ für $* \in \set*{i,i+1}$
		\item $H_i \enbrace*{C_*^{[i]}} = H_i(C_*)$
		\item $H_{i+1} \enbrace*{C_*^{[i]}}=0$ (insbesondere ist $\partial_{i+1}^{[i]} \colon C^{[i]}_{i+1} \to C_i^{[i]}$ injektiv)
	\end{itemize}
	und einen Isomorphismus von Kettenkomplexen $C_* \cong \bigoplus_{i \in \mathbb{N}} C_*^{[i]}$.
\end{satz}
% section 16 (end)
