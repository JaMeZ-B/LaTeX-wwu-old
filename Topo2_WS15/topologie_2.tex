...
\begin{bemerkung}[{name=[Grundlegende Eigenschaften der Paarung]}]
	...
	\begin{itemize}
		...
		\begin{align}
			\enbrace*{\alpha + d^{n-1}(\beta)}(a + d_{n+1}(b)) = \alpha(a) \pm \Underbrace{\alpha \enbrace[\big]{d_{n+1}(b)}}{=0 } 
...
\begin{satz}[label=eig_kohomo_to_hom_homo]
	...
	\begin{enumerate}[(i),itemsep=0pt]
		...
		\item Ist $V$ ein $\mathbb{Q}$-Vektorraum, so ist $f$ auch injektiv. \todo{Revision 2}
	\end{enumerate}
\end{satz}
...
\begin{beweis}
	...
	\[
		\begin{tikzcd}
			\mathbb{Z}[R] \cap C \rar[hook] \dar[hook,"i_0"]& \mathbb{Z}\benbrace[\big]{R \cup \set{s}} \cap C \rar[two heads] \dar[hook,"i_1"] 
			& \sfrac{\mathbb{Z}\benbrace[\big]{R \cup \set{s}} \cap C}{\mathbb{Z}[R] \cap C} \dar["i_2"] \\
			\mathbb{Z}[R] \rar[hook] & \mathbb{Z}\benbrace[\big]{R \cup \set{s}} \rar[two heads,"p"] & \mathbb{Z}\benbrace[\big]{\set{s}}
		\end{tikzcd}
	\]
