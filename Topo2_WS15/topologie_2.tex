...
\begin{definition}[{name=[Kolimes]}]
	Sei $(\Lambda,\le)$ eine gerichtete Menge und $F \colon (\Lambda,\le) \to R\text{-}\MOD$ ein Funktor. Der \Index{Kolimes} von $F$ ist dann definiert als 
	\[
...
\begin{bemerkung}
	...
	\begin{itemize}
		...
		\begin{enumerate}[(i)]
			...
			\item Die Kategorie bestehen aus zwei Objekten und keinem Morphismus, der nicht eine Identität ist.
		\end{enumerate}
		...
		\).
		\item Sei $\FUN \enbrace*{(\Lambda,\le),R\text{-}\MOD}$ die Kategorie der Funktoren und natürlichen Transformationen von $(\Lambda,\le)$ nach.
		Die Definition des Kolimes ist natürlich und definiert damit einen Funktor $\colim_\Lambda \colon \FUN \enbrace*{(\Lambda,\le),R\text{-}\MOD} \to R\text{-}\MOD$.
	\end{itemize}
\end{bemerkung}

\begin{definition}[{name=[kofinal]}]
	Sei $(\Lambda,\le)$ eine gerichtete Menge. $\Lambda_0 \subseteq \Lambda$ heißt \Index{kofinal}, falls es zu jedem $\lambda \in \Lambda$ ein $\lambda_0 \in \Lambda_0$ gibt mit $\lambda\le \lambda_0$.\todo{Revision 7}
\end{definition}
