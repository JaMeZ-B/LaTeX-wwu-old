...
\begin{beweis}
	...
	Nun ist $\mathbb{Z}[R] \cap C  = \mathbb{Z}[R] \cap \enbrace[\big]{\mathbb{Z}[R \cup \set{s}] \cap C}$ und damit muss auch der von $i_0$ und $i_1$ induzierte Homomorphismus 
...
\begin{definition}[{name=[Einschränkung von singulären Simplizes]}]
	Sei $\sigma \colon \abs{\Delta^n}  \to X$ ein singulärer Simplex in $X$. Für $0 \le p \le n$ definieren wir \todo{Revision 3}
	\[
	...
	\]
	durch $\sigma|_{\benbrace*{0,\ldots ,p}}(t_0, \ldots ,t_p) = \sigma \enbrace*{t_0, \ldots , t_p, 0, \ldots ,0}$ und 
...
\begin{definition}[label=sec3:def_cup,{name=[$\cup$-Produkt]}]
	...
	\[
		\cup \colon C^p_\sing \enbrace*{X,A;R} \otimes C^q_\sing(X;R) \longrightarrow C^{p+q}_\sing \enbrace*{X,A;R}
	\]
...
\begin{beweis}[von \autoref{satz:5:kettenhomotopie} zweite Implikation,label=bew:satz5]
...
\begin{definition}[{name=[Cap-Produkt]}]
	Sei $X$ ein topologischer Raum und $R$ ein kommutativer Ring. Wir definieren das \bet{Cap-Produkt} ($\cap$-Produkt)\index{Cap-Produkt, $\cap$-Produkt}
	\[
