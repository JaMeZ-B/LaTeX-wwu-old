...
\end{satz}
\begin{beweis}
	Die Implikation (i) $\Rightarrow$ (ii) ist klar (siehe Topologie \RM{1}.) Die zweite Implikation besprechen wir später.\todo{Verweis hinzufügen, wenn da}
\end{beweis}
...
\end{definition}
 ...
 ist eine kurze exakte Sequenz von Kettenkomplexen. Das Schlangenlemma liefert dann eine lange exakte Sequenz, deren Verbindungshomomorphismus $f_*$ entspricht.
...
\begin{beweis}
	...
	\begin{align}
		f_n \gamma_n + d^D_{n+1} \alpha_n + \alpha_{n-1} d_n^D &= \id_{D_n} \\
		d^D_{n+1} \beta_{n-1} + f_n \delta_{n-1} + d_{n-1} f_{n-1} - \beta_{n-2} d_{n-1}^C &= 0 \\
		- d^C_n \gamma_n +  + \gamma_n d_n^D &= 0 \\
		- d_n^C \delta_{n-1} + \gamma_{n-1} f_{n-1} - \delta_{n-2} d^C_{n-1} &= \id_{C_{n-1}} 
	\end{align}
	\todo[inline]{besser lesbar machen, Nummern einfügen}
	...
	Gelten umgekehrt (1'), (3') und (4'), so setzt man $\hat{\delta} := \delta + \gamma \circ f - \delta \gamma f$ und $\beta := \alpha f \gamma + \alpha^2 f + f \delta^2$ und 
	\[
		S := \begin{pmatrix}
			\alpha & \beta \\
			\gamma & \hat{\delta}
...
\begin{beweis}[von \autoref{satz:5:kettenhomotopie} zweite Implikation]
