...
\label{sub:11}
Ein \Index{metrischer Raum} $(X,d)$ ist eine Menge $X$ mit einer Abbildung, \Index{Metrik} genannt,  $d: X \times X \to [0,\infty)$ mit folgenden Eigenschaften:
\begin{enumerate}[(i)]
...
\begin{enumerate}[(i)]
	...
	\item Ist $X$ ein metrischer Raum und $A$ eine Teilmenge von $X$, so wird $A$ durch die Einschränkung der Metrik auf $A$ zu einem metrischen Raum. Wir sagen dann $A$ ist ein Unterraum von $X$.
...
\end{figure}
$T^n := \underbrace{S^1 \times \ldots \times S^1}_{n} = \prod_{i=1}^n S^1$ heißt der $n$-Torus. Der $n$-Torus ist eine (glatte) $n$-Mannigfaltigkeit.
...
\begin{enumerate}[(i)]
	...
	\begin{minipage}{\textwidth}
		...
		\[
			...
			\end{tikzpicture}}}
		\]
		\caption[Möbius-Band]{Möbius-Band, \hrefsym{http://tex.stackexchange.com/questions/118563/moebius-strip-using-tikz}{Quelle}}
		}
	\end{minipage}
	...
	\item\label{29:enum:5} Sei $f : X \to X$ eine stetige Abbildung. Betrachte auf $X \times [0,1]$ die Äquivalenzrelation 
	\[
...
Dann ist die Menge $\Lambda := \set[U]{U \text{ ist offene Umgebung von }x} $ gerichtet bezüglich
\[
...
Sei $\varphi : [0,1] \to [0,1]$ gegeben wie in Abbildung \ref{fig:98} gezeichnet. \marginnote{die anderen Beweise funktionieren ähnlich}
Dann gilt 
\begin{align*}
...
\subsection[Proposition: Für normale Überlagerungen ist \protect{$q' :\Delta(p)\text{\textbackslash{}} \hat{X} \to X$} Homöomorphismus]{Proposition} % (fold)
\label{sub:1210}
...
Sei $x_0 \in X$. Sei $P = \set[{\omega : [0,1] \to X \text{ Weg}}]{\omega(0)=x_0} $. Sei 
\[
	\tilde{X}:= \nicefrac{P}{\text{Homotopie mit festen Endpunkten}}
\] 
Dann induziert $\omega \mapsto \omega(1)$ eine wohldefinierte Abbildung $p : \tilde{X} \to X$. Sei $\omega \in P$ und $V$ eine wegzusammenhängende, einfach zusammenhängende Umgebung von $\omega(1)$ in $X$. Setze 
\[
...
\subsection{Lemma (Eckmann-Hilton-Argument)} % (fold)
\label{sub:145}
...
\subsection[Korollar: Die Homotopiegruppen punktiert homöomorpher Räume stimmen überein]{Korollar} % (fold)
\label{sub:147}
Es gilt 
\[
...
\subsection[Satz: Existenz einer langen exakten Sequenz von Homotopiegruppen für Faserung]{Satz} % (fold)
\label{sub:1411}
...
\begin{enumerate}[a)]
	...
	\)
	\item Ein Atlas $\mathcal{A}$ heißt $C^\infty$ (oder \bet{glatt}\index{Atlas!glatter}), wenn alle Kartenwechsel zwischen Karten aus $\mathcal{A}$ 
...
\label{sub:1510}
Eine $C^\infty$"=Abbildung $f : N \to M$ heißt eine \Index{Einbettung}, wenn $f(N) \subseteq M$ eine $C^\infty$"=Untermannigfaltigkeit ist und $f : N \to f(N)$ ein
...
$\D \enbrace[\big]{\hat{h}_1 \circ  h_1 ^{-1}}_{h_1\enbrace*{f(x)}}$ und $\D \enbrace[\big]{h_0 \circ \hat{h}_0 ^{-1}}_{\hat{h}_0(x)}$ invertierbar. Es folgt die Behauptung. \bewende
% subsection 164 (end)
...
\label{sub:171}
Sei $M$ eine $C^\infty$"=Mannigfaltigkeit. Dann ist $C^\infty_0 (M,\mathds{R}) := C^\infty(M,\mathds{R}) \cap C_0(M,\mathds{R})$\marginnote{siehe \ref{sub:61} für 
...
\subsection[Korollar 1: Approximation von $\varphi : M \to \mathds{R}^n$ stetig durch $C^\infty$"=Funktionen]{Korollar 1} % (fold)
\label{sub:172}
...
\subsection[Bemerkung: Jede stetige Abbildung zwischen $C^\infty$"=Mfkt. ist approximierbar]{Bemerkung} % (fold)
\label{sub:174}
...
Das Urbild von $\mathds{R}^n \times \set{0}$ unter $\D h_x$ ist der \bet{Tangentialraum}\index{Tangentialraum!einer Untermannigfaltigkeit} $T^u_x M$ von $M$ im Punkt
...
\subsection[Lemma 2: Äquivalenz, die allgemeine Definition von Tangentialvektoren ermöglicht]{Lemma 2} % (fold)
\label{sub:185}
...
\]
erhalten wir eine Äquivalenzrelation auf $\mathcal{T}_xM$. Der \Index{Tangentialraum} zu $M$ im Punkt $x$ ist definiert als die Menge der Äquivalenzklassen 
$T_x M := \nicefrac{\mathcal{T}_x M}{\sim}$.
% subsection 186 (end)

\subsection[Bemerkung: Die Definition von $T_x^u M$ ist verträglich mit der allgemeinen Definition]{Bemerkung} % (fold)
\label{sub:187}
...
\subsection[Bemerkung: Tangentialraum von $V \subseteq \mathds{R}^n$ offen]{Bemerkung} % (fold)
\label{sub:188}
...
\subsection[Bemerkung: Für $U \subseteq M$ offen gilt $T_x U = T_x M$]{Bemerkung} % (fold)
\label{sub:189}
...
die wiederum einen kanonischen Isomorphismus $T_x U \xrightarrow{\cong} T_x M$ induziert.
% subsection 189 (end)

\subsection[Lemma 3: Wohldefiniertheit der Tangentialabbildung]{Lemma 3} % (fold)
\label{sub:1810}
...
\subsection[Lemma 4: Die Tangentialabbildung ist eine Verallgemeinerung des Differentials]{Lemma 4} % (fold)
\label{sub:1812}
...
\subsection[Proposition: Vektorraumstruktur von $T_x M$, Linearität der Tangentialabbildung]{Proposition} % (fold)
\label{sub:1813}
\begin{enumerate}[a)]
...
\subsection[Bemerkung: $x \in N$ regulär $\iff$ $T_x f : T_x N \to T_{f(x)} M$ surjektiv]{Bemerkung} % (fold)
\label{sub:1814}
...
\label{sub:191}
Sei $X$ ein topologischer Raum. Ein $n$-dimensionales \Index{Vektorraumbündel} über $X$ ist eine stetige, surjektive Abbildung $\pi  : E \to X$,\marginnote{Oft bezeichnet 
man das Vektorbündel einfach nur mit $E$} wobei für jedes 
...
\end{itemize}
Oft sagt man für $(\star)$ auch, dass $E$ \Index{lokal trivial} ist. Das Paar $(f,U)$ heißt dann \Index{Bündelkarte} für $E$.  
% subsection 191 (end)

\subsection[Beispiel: Das Möbiusband ist ein nichttriviales Vektorraumbündel]{Beispiel} % (fold)
\label{sub:192}
...
\subsection[Definition: Schnitt eines Vektorbündels]{Definition} % (fold)
\label{sub:194}
...
\subsection[Definition: Induzierte Bündelkarte]{Definition} % (fold)
\label{sub:196}
...
\]
die von $h$ \bet{induzierte Bündelkarte}\index{Bündelkarte!induzierte Bündelkarte} von $T M$. 
% subsection 196 (end)

\subsection[Bemerkung: Bündelkartenwechsel]{Bemerkung} % (fold)
\label{sub:197}
Seien $M \supseteq U_i \xrightarrow{\, h_i \,} V_i \subseteq \mathds{R}^n $, $i=0,1$ zwei $C^\infty$"=Karten von $M$. Dann ist der \Index{Bündelkartenwechsel} zwischen den 
...
\subsection[Proposition: Topologie auf $T M$, die $T M$ zu einem Vektorraumbündel macht]{Proposition} % (fold)
\label{sub:198}
...
\subsection[Bemerkung: $T \varphi : T M  \to T N$ ist linear, Vektorfelder als Schnitte von $T M$]{Bemerkung} % (fold)
\label{sub:199}
\begin{itemize}
	...
	über $\varphi$.
	\item Schnitte des Tangentialbündels heißen \Index{Vektorfelder}. Ein $C^\infty$"=Vektorfeld ist ein Vektorfeld $s : M \to T M$, so dass 
