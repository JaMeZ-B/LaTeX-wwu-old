...
\((e_1,\dots,e_n)\) ist eine Basis von $\mathbb{R}^n$ und heißt \Index{ Standardbasis}. 
...
Das \Index{ Skalarprodukt} auf $\mathbb{R}^n$ ist die Abbildung \[ \langle\, .\,,\,.\,\rangle : \mathbb{R}^n \times \, \mathbb{R}^n \to \mathbb{R}\] mit 
...
\label{ssub:definition}
Sei $ V$  ein $\mathbb{R}$-Vektorraum. Ein \Index{ Skalarprodukt} auf $ V$  ist eine Abbildung $\skal{.}{.}:V \times V \to \mathbb{R}$, welche die Axiome S1,S2,S3 erfüllt. \\
Ein \Index{ euklidscher Vektorraum} ist ein Paar $(V,\skal{}{})$. $ V$  ist ein $\mathbb{R}$-Vektorraum und $\skal{}{}$ ist Skalarprodukt auf $ V$. \\
...
\label{ssub:definition}
Sei $ V$ ein $\mathbb{R} $-Vektorraum. Eine \Index{ Norm} auf $ V$ ist eine Abbildung $\norm{.}:V \to \mathbb{R} , v \mapsto \norm{v}$ mit
\begin{description}
...
\begin{enumerate}
	...
	\item $\forall\, v,w \in V$ gilt die \Index{ Cauchy-Schwarz-Ungleichung}:
	\[
		|\skal{v}{w}| \leq \norm{v} \cdot \norm{w}
	\]  
\end{enumerate}
...
\label{ssub:definition}
$ V$ $\mathbb{R}$-VR , $\norm{.}_0 , \norm{.}_1$ Normen auf $ V$. Die Normen $\norm{.}_0 , \norm{.}_1$ heißen \Index{ äquivalent}, falls $C,c>0$ existieren, so dass für alle $v \in  V$ gilt:
\[
	c \norm{v}_0 \leq \norm{v}_1 \leq C \norm{v}_0 
\] 
...
Der \Index{ Abstand} von $v$ und $w$ ist \[
d(v,w):= \norm{v-w}	
\] 
% subsubsection definition (end)
 ...
 \label{ssub:definition}
Sei $X$ eine Menge \\
Eine \Index{ Metrik} (oder Abstandsfunktion) ist eine Abbildung $d: X \times X \to \mathbb{R} , (x,y) \mapsto d(x,y)$, sodass gilt:
\begin{description}
	...
	\item[(M4)] $\forall\, x,y,z \in X: \hspace{1cm}  d(x,z) \leq d(x,y)+d(y,z) \hspace{1cm}$ (\Index{ Dreiecksungleichung}) 
\end{description}
Ein \Index{ Metrischer Raum} ist ein Paar $(X,d)$, $X$ Menge, $d$ Metrik auf $X$
\minisec{Beispiele:}
\begin{itemize}
...
\label{ssub:definition}
Sei $(X,d)$ metrischer Raum und $(x_n)_n $ Folge in $X$. Dann heißt $(x_n)_n$ \Index{ Cauchyfolge}, falls gilt:
\[
...
\label{ssub:definition}
$(X,d)$ metrischer Raum. Dann heißt $X$ \Index{ vollständig}, falls jede Cauchy-Folge $(x_n)_n $ in $X$ gegen ein $x \in X$ konvergiert (Ana I)
...
\begin{enumerate}[(i)]
	\item $f$ heißt \Index{ $\varepsilon$-$\delta$-stetig} (oder \Index{ stetig}) bei $x$, wenn gilt
	\[
		\forall\, \varepsilon >0 \, \exists \, \delta>0 \, \forall\, x' \in X \text{ mit } d_X(x,x')< \delta \text{ gilt: } d_Y(f(x),f(x'))<\varepsilon 
	\]
	\item $f$ heißt \Index{ folgenstetig} in $x$ $:\Leftrightarrow$ Ist $(x_n)_n $ eine Folge in $X$ mit $\lim_{n \to \infty}x_n=x$, so gilt
	\[
...
\label{ssub:definition}
$(X,d_X),(Y,d_y)$ metrische Räume $f:X \to Y$ heißt \Index{ Lipschitz-stetig} 
...
\label{ssub:definition}
Die Zahl $\norm{A}$ heißt \Index{ Operatornorm} von $A$ \\
...
\label{ssub:definition}
 $(X,d)$ sei metrischer Raum, $\varepsilon >0$ und $x \in X$. Der \Index{ $\varepsilon $-Ball} in $X$ um $x$ ist
 \[
...
\begin{enumerate}
	\item $x$ heißt \Index{ innerer Punkt} von $Y$  \[
		\exists\, \delta>0 \text{ mit } B_{\delta }(x) \subseteq Y
	\]
	In diesem Fall heißt eine \Index{ Umgebung} von $x$ in $X$
	\item $x \in X$ heißt \Index{ Berührpunkt} von $Y$
	\[
...
\label{ssub:definition}
\begin{enumerate}
	\item $(X,d)$ metrischer Raum, $Y \subseteq X$. Dann heißt $Y$ \Index{ offen} in $X$ $\Leftrightarrow $ jedes $x \in Y$ ist innerer Punkt von $Y$ \\
	( $\Leftrightarrow \forall\, x \in Y \, \exists \, \delta >0: \, B_{\delta } \subseteq Y$)
	\item $Y$ heißt \Index{ abgeschlossen} in $X$ $:\Leftrightarrow $ Das Komplement $X-Y$ ist offen in $X$
\end{enumerate}
 \minisec{Bsp:}
 $X = \mathbb{R}$
 \begin{enumerate}
...
\label{ssub:definition}
Ein metrischer Raum $X$ heißt \Index{ folgenkompakt}, falls für jede Folge $(x_n)_n$ in $X$ eine konvergente Teilfolge hat. \\
...
$X$ metrischer Raum, $f:X \to \mathbb{R}$ heißt \Index{ gleichmäßig stetig}, wenn gilt:
\[
...
\end{align*}
 \subsubsection{Definition} % (fold)
 \label{ssub:definition}
 $X$ metrischer Raum, Eine \Index{ offene Überdeckung} von $X$ ist eine Familie $U=(U_i)_{i \in I}$, $U_i \subseteq X$ offen für alle $i \in I$ und es gilt:
 \[
 ...
 \]
 Ein solches $\delta$ heißt \Index{ Lebesgue Zahl} der Überdeckung von $U$
...
\subsubsection{Definition} % (fold)
\label{ssub:definition}
$X$ metrischer Raum, $U=(U_i)_{i \in I}$ eine offene Überdeckung. Eine \Index{ Teilüberdeckung von U} ist eine offene Überdeckung $V=(V_j)_{j \in J}$ von $X$, so dass gilt: 
\[
...
\subsubsection{Definition} % (fold)
\label{ssub:definition}
Ein metrischer Raum heißt \Index{ überdeckungskompakt} (oder \Index{ Kompakt}) falls { \bfseries jede} offene Überdeckung $U=(U_i)_{i \in I}$ eine endliche Teilüberdeckung $V=(V_j)_{j \in J}$ besitzt. (d.h die Indexmenge J ist endlich)
% subsubsection definition (end)
\subsubsection{Satz von Heine-Borel} % (fold)
...
Dann ist $X$ \Index{ total beschränkt}, d.h. $\forall\, \varepsilon>0$ gibt es endlich viele $x_1,\dots,x_r \in X$, so dass:
\[
...
\subsubsection{Definition} % (fold)
\label{ssub:definition}
$U \subseteq \mathbb{R}^n$ offen, $f:U \to \mathbb{R}^m$ $x \in U$. f heißt \Index{ differenzierbar} bei $x$, falls eine Funktion $A: -x+U \to \Mat_{m,n}(\mathbb{R})$ existiert, so dass gilt:
\begin{enumerate}
	\item $A$ stetig bei 0
	...
	\]
\end{enumerate}
Die Matrix $A(0) \in \Mat_{m,n}(\mathbb{R})$ heißt \Index{ Ableitung} von $f$ in $x$ und wird mit
\[
...
\subsubsection{Definition} % (fold)
\label{ssub:definition}
$U,x,f,v$ wie in Satz II.1.3. Dann heißt $Df(x)v$ die \Index{ Richtungsableitung} von $f$ in Richtung $v$.
\[
	D_vf(x)=Df(x)v
\] 
\minisec{Bemerkung:}
...
\subsubsection{Definition} % (fold)
\label{ssub:definition}
$U \subseteq \mathbb{R}^n$ offen, $x \in U$, $f:  \to \mathbb{R}^m$. Die \Index{ $i$-te partielle Ableitung von $f$} bei $x$ ist
\[
...
\subsubsection{Definition} % (fold)
\label{ssub:definition}
$f: U \to \mathbb{R}^m$ heißt \Index{ stetig differenzierbar}, falls $f$ differenzierbar ist und falls $Df: U \to \text{Mat}_{n,m}(\mathbb{R})$ stetig ist.
% subsubsection definition (end)
...
\end{align*}
Analog erhält man durch Betrachtung der Funktion $\mu : \mathbb{R}^2 \to  \mathbb{R}$ mit $\mu(y,z)=yz$, die \Index{ Produktregel}
...
\subsubsection{Definition} % (fold)
\label{ssub:definition}
$X,Y$ endlich-dimensionale $\mathbb{R}$-VR, $U \subseteq X$ offen, $x \in U$,$f: U \to Y$ heißt \Index{ differenzierbar} bei $x$, falls gilt: \\
...
\subsubsection{Definition} % (fold)
\label{ssub:definition}
$U \subseteq \mathbb{R}^n$ offen, $f: U \to \mathbb{R}^m$. Dann heißt $f$ \Index{ r-mal stetig differenzierbar} (oder $C^r$-Funktion), falls gilt: \\
 für alle $s=0,\dots,r$ und alle $v_1,\dots,v_s \in \mathbb{R}^m$ existiert die \Index{ iterierte Richtungsableitung}
 \[
 ...
 \minisec{Definition:} 
 $U \subseteq \mathbb{R}^n$ offen, $f: U \to \mathbb{R}$ eine $C^2-Funktion$. Die \Index{ Hessematrix} von $f$ bei $x$ ist
 \[
 	Hf(x):= \begin{pmatrix}
...
\begin{enumerate}[(i)]
	...
	\]
\end{enumerate}
Dieses Polynom heißt $m$-tes \Index{ Taylorpolynom} von $f$ bei $p$.
...
\]
\Index{ Taylorreihe} von $f$ bei p und ist eine Potenzreihe.
\begin{itemize}
...
\subsubsection{Definition} % (fold)
\label{ssub:definition}
$U \subseteq \mathbb{R}^n$, $x \in U$. Dann heißt $U$ \Index{ sternförmig }bezüglich $x$, falls gilt:
\[
...
\subsubsection{Definition} % (fold)
\label{ssub:definition}
$ f: U \to \mathbb{R} $  differenzierbar und $x \in U$. $x$ heißt \Index{ kritischer Punkt} von $f$ genau dann, wenn \[
	Df(x)=0
\] 
...
\subsubsection{Definition} % (fold)
\label{ssub:definition}
$A \in \Mat_{n,n}(\mathbb{R})$, $\lambda \in \mathbb{R}$ heißt \Index{ Eigenwert} von $A$ genau dann, wenn
\[
	\exists\,x \in \mathbb{R}^n, x \neq 0 \text{ und } Ax=\lambda x
\]
Die \Index{ Eigenvektoren} zum Eigenwert $\lambda$ sind die $x \in \mathbb{R}^n$ mit $Ax=\lambda x$
\[
	\Eig(A,\lambda)= \set[x \in \mathbb{R}^n]{Ax=\lambda x}
\]
Das \Index{ Charakteristische Polynom} ist definiert durch
\[
...
\]
\begin{itemize}
	\item $A \in S(n)$ heißt \Index{ positiv definit}, falls für alle $v \in \mathbb{R}^n$,$v \neq 0$ gilt:
	\[
		\skal{Av}{v}>0
	\]
	\item $A \in S(n)$ heißt \Index{ negativ definit}, falls für alle $v \in \mathbb{R}^n$,$v \neq 0$ gilt:
	\[
		\skal{Av}{v}<0
	\]
\end{itemize}
...
\subsubsection{Beweis des Umkehrsatzes, Teil $1$ (Spezialfall und Hauptarbeit)} % (fold)
...
\minisec{Beweis:} Wir müssen die Gleichung $f(x)=y$ für kleine $y$ lösen.
...
\begin{align*}
	...
	\leq r,
\end{align*}
wobei sich $(*)$ mit dem Schrankensatz ($D(x-f(x))\leq \frac{1}{2}$, $\norm{x}\leq r$) \\
...
% subsubsection beweis_des_umkehrsatzes_teil_1_spezialfall_und_hauptarbeit (end)
\subsubsection{Beweis des Umkehrsatzes, Teil $2$} % (fold)
...
\hphantom{k}
\minisec{Satz über implizite Funktionen:}
\hphantom{k}
\begin{description}
	...
	\item[Notation:] \begin{itemize}
		...
		\end{itemize}
\end{description}
\subsubsection{Satz über implizite Funktionen} % (fold)
\label{implfunkt}
...
\minisec{Bemerkung:}Die Gleichung $f(x,y)=z_0$ lässt sich lokal nach $x$ auflösen. \\
\\
...
\minisec{Beweis von \ref{implfunkt}:} 
...
\subsubsection{Beispiel} % (fold)
\label{ssub:beispiel}
\minisec{Nullstellen von Polynomen:}
...
% subsubsection beispiel (end)
\hphantom{p}
\minisec{Geometrische Version des Satzes über implizite Funktionen}
\hphantom{p}
\subsubsection{Definition} % (fold)
\label{ssub:definition}
$ U \subseteq \mathbb{R}^m$ offen, $f: U \to \mathbb{R}^k$ in $C^1$. \\
\begin{itemize}
	\item $x \in U$ heißt \Index{regulärer Punkt}, von f, falls $Df(x) \in \Mat_{k,m}(\mathbb{R})$ den Rang $k$ hat (d.h $Df(x)$ stellt eine surjektive lineare Abbildung von $\mathbb{R}^m \to \mathbb{R}^k$ dar.)
	\item $x \in U$ heißt \Index{singulärer} (oder \Index{kritischer Punkt}) von $f$, falls $x$ kein regulärer Punkt von $f$ ist. (d.h Rang $Df(x) < k$)
	\item $y \in \mathbb{R}^k$ heißt \Index{regulärer Wert} von $f$, falls jedes $x \in U$ mit $f(x)=y$ ein regulärer Punkt ist.
\end{itemize} 
\minisec{Bemerkung:}
\begin{enumerate}
...
\]
\minisec{Beweis von \ref{satz210}:}
...
\underline{Behauptung}: $Dh(x_0) \in \Mat_{m,m}(\mathbb{R})$ ist invertierbar. \\
Dies gilt denn 
\[
...
\subsubsection{Definition} % (fold)
\label{ssub:definition}
Eine Teilmenge $M \subseteq \mathbb{R}^{n+k}$ heißt \Index{$n$-dimensionale Untermannigfaltigkeit} von $\mathbb{R}^{n+1}$, falls gilt: \\
...
Ein solches Paar $(U,h)$ heißt \Index{Karte} von M. \\
Eine Familie $(U_i,h_i)_{i \in I}$ von Karten heißt \Index{Atlas} von M, falls $\bigcup\limits_{i \in I}U_i \cap M=M$
% subsubsection definition (end)
\subsubsection{Beispiele} % (fold)
\label{ssub:beispiele}
\begin{enumerate}
...
\section{Anfangsgründe der Theorie gewöhnlicher Differentialgleichungen} % (fold)
...
% subsubsection definition (end)
Eine \Index{gewöhnliche Differentialgleichung} $m$-ter Ordnung ist eine Gleichung der Form
\[
...
Eine \Index{Lösung} der Gleichung ist eine Funktion $x$, welche die Gleichung erfüllt. \\
...
% subsubsection beispiel (end)
% section anfangsgrunde_der_theorie_gewohnlicher_differentialgleichungen (end)
\subsubsection{Beispiel} % (fold)
\label{ssub:beispiel}
\[
...
\\
% subsubsection beispiel (end)
\underline{Erwartung}: Das \Index{Anfangswertproblem} $x^{(m)}(t)= F(x(t), \dots , x^{m-1}(t),t)$ hat zu gegebenen $x_0,\dots, x_{m-1} \in \mathbb{R}^n$ genau eine Lösung $x:(t_0 - \delta , t_0 + \delta ) \to \mathbb{R}^n$ mit $\delta >0$ und Anfangsdaten $x^{(k)}(t_0)=x_k$ für $k = 0, \dots, m-1$ unter 'vernünftigen' Voraussetzungen an $F$.
\subsection{Lineare Differentialgleichungen mit Konstanten Koeffizienten} % (fold)
...
\begin{enumerate}[(i)]
	...
	\]
\end{enumerate}
\minisec{Bemerkung:}Man kann zeigen, dass die Funktion $\exp: \Mat_{n,n}(\mathbb{C}) \to \Mat_{n,n}(\mathbb{C})$ eine $C^{\infty}$-Funktion ist. Die naheliegende Formel
\[
	D_H \exp(A) = \exp(A) H
\] ist \underline{Falsch!}
% subsubsection satz (end)
\minisec{Beweis:} \begin{enumerate}[(i)]
	...
	\[
		G(t)=G(0)
	\]
	\item Setze $y(t)= \exp ( -tA)X(t)$ mit $y(0)= \exp(0)X(0)= x_0$. \\
	Dann gilt
	\begin{align*}
		\dot y(t) &\stackrel{1)}{=}-A\exp(-tA)X(t)+\exp(-tA)AX(t) \\
		&\stackrel{2)}{=}-A \exp(-tA)X(t)+ A \exp(-tA)X(t) \\
		&\stackrel{\hphantom{3)}}{=}0 
	\end{align*}
	Dann folgt
	\[
		y(t)= x_0 \qquad \forall\, t
	\]
	\[
		\stackrel{3)}{\Rightarrow} X(t)=\exp(tA)x_0 \qquad \forall\, t
	\]
	\item \begin{align*}
		\diffd{}{t}(\exp(tA)\exp(tB)) &\stackrel{1)}{=} A \exp(tA)\exp(tB)+ \exp(tA)B\exp(tB) \\
		& \stackrel{2)}{=}(A+B) \exp(tA)\exp(tB)
	\end{align*}
	$\Rightarrow$ Die Funktion $X:\mathbb{R} \to \Mat_{n,n}(\mathbb{C})$ mit $X(t)=\exp(tA)\exp(tB)$ erfüllt die Differentialgleichung
	\[
		\dot X(t)=(A+B)X(t) \qquad und \qquad X(0)=\ind_n
	\]
	\[
		\stackrel{4)}{\Rightarrow} X(t)=\exp(t(A+B)) \qquad t=1
	\]\bewende
	\item Setze $X(t)=B \exp(tA)B^{-1}$ und $X(0)=B \ind_n B^{-1}= \ind_n$. \\
	Dann gilt
	\begin{align*}
		BA \exp(tA)B^{-1} &= BAB^{-1} \underset{X(t)}{\underbrace{B \exp(tA)B^{-1}}} \\
		&= (BAB^{-1})X(t)
	\end{align*}
	\[
		\Rightarrow X(t)=\exp(tBAB^{-1})
	\] \bewende
\end{enumerate}
...
\begin{align*}
	...
	\dot x(0)&=x_0
\end{align*}
$x$ ist eine Funktion $\mathbb{R} \xrightarrow{x} \mathbb{C}^n$ und $A \in \Mat_{n,n}(\mathbb{C})$

\subsubsection{Korollar} % (fold)
\label{ssub:korollar}
Zu jedem $x_0 \in \mathbb{C}^n$ gibt es genau eine Lösung von obigen Anfangswertproblem, nämlich
\[
	X(t)=\exp(tA)x_0
\]
\minisec{Beweis:} \[
	\diffd{}{t}(\exp(tA)x_0)= A\exp(tA)x_0
\]
\[
	\exp(0A)x_0 = \ind_n x_0 = x_0
\]
	$\Rightarrow$ $x(t):= \exp(tA)x_0$ löst das Anfangswertproblem. \\
\underline{Eindeutigkeit}: Sei $x \mathbb{R} \to \mathbb{C}^n$ eine Lösung des AWP. Dann gilt
\begin{align*}
	\diffd{}{t}\exp(-tA)x(t) &= -A \exp(-tA)x(t)+ \exp(-tA) \dot x(t) \\
	&= -A \exp(-tA)x(t)+ \exp(-tA)Ax(t) \\
	&= 0
\end{align*}
\[
	\Rightarrow \exp(-tA)x(t)= \underset{=\ind_n}{\underbrace{\exp(0A)}}\underset{x_0}{\underbrace{x(0)}}=x_0
\]
\[
	\Rightarrow x(t)= \exp(tA)x_0
	\]
\minisec{Bemerkung:}Sei $A \in \Mat_{n,n}(\mathbb{C})$ fest. Dann gilt

	 $x,y: \mathbb{R} \to \mathbb{C}^n$ \qquad $\dot x(t)=Ax(t)$, $\dot y(t)=Ay(t)$. Wenn $a \in \mathbb{C}$, dann gilt
	\[
		\diffd{}{t}(x(t)+ay(t))=A(x(t)+ay(t))
	\]
	$\Rightarrow$ Jede Linearkombination von Lösungen von $Ax(t)= \dot x(t)$ ist wieder eine Lösung. \\
	\[
		\set[x: \mathbb{R} \to \mathbb{C}^n]{\dot x(t)=Ax(t)}
	\] ist Vektorraum der Dimension $n$.
	\begin{align*}
		\set[x: \mathbb{R} \to \mathbb{C}]{\dot x(t)=Ax(t)} &\to \mathbb{C}^n \\
		x &\mapsto x(0)
	\end{align*}
	ist linear und bijektiv. \\
	Die Funktion
	\[
		x_i(t):= \exp(tA)e_i \qquad i=1,\dots,n
	\]
	bilden eine Basis des Lösungsraumes.
$\Rightarrow$ Die $i$-te Spalte von $\exp(tA)$ ist die Lösung des Anfangswertproblems
\[
	\dot = Ax, \qquad x(0)=e_i
\]
\subsubsection{Inhomogene Differentialgleichung} % (fold)
\label{ssub:inhomogene_differentialgleichung}

% subsubsection inhomogene_differentialgleichung (end)
$A \in \Mat_{n,n}(\mathbb{C})$, $x_0 \in \mathbb{C}^n$, $b: \mathbb{R} \to \mathbb{C}^n$ stetig. Dann heißt
\begin{align*}
	\dot x(t)&= Ax(t)+b(t) \\
	x(0)&=x_0
\end{align*}
\Index{inhomogene Differentialgleichung}
\underline{Ansatz}: Suche $x$ in der Form 
\[
	x(t)=\exp(tA)c(t)
\] mit $c: \mathbb{R} \to \Mat_{n,n}(\mathbb{C})$ in $C^1$.
\[
	\exp(0)c(0)=c(0)\stackrel{!}{=}x_0
\]
Dann gilt
\begin{align*}
	\diffd{}{t}(\exp(tA)c(t)) &= A(\exp(tA)c(t))+\underset{\stackrel{!}{=}b(t)}{\underbrace{\exp(tA)\dot c(t)}}
\end{align*}
\[
	\Rightarrow \dot c(t)= \exp(-tA)b(t)
\]
\[
	c(t)=x_0+ \int_{0}^{t}\exp(-sA)b(s) \,\mathrm{d}s
\]
Dann folgt
\[
	\Rightarrow x(t)= \exp(tA)\left( x_0 + \int_{0}^{t}\exp(-sA)b(s) \,\mathrm{d}s \right)
\]
ist eine Lösung von
	\begin{align*}
		\dot x(t)&= Ax(t)+b(t) \\
	...
	\end{align*}
\[
	x(t)= \underset{\substack{\text{löst die} \\ \text{inhomogene} \\ \text{Gleichung}}}{\underbrace{\exp(tA)x_0}} 
	+ \underset{\substack{\text{Variation} \\ \text{ der Konstanten}}}{\underbrace{\int_{0}^{t}\exp((t-s)A)b(s) \,\mathrm{d}s}}
\]
Wie berechnent man $\exp(tA)$?
\subsubsection{Beispiele} % (fold)
\label{ssub:beispiele}
% subsubsection beispiele (end)
\begin{enumerate}
...
% subsubsection korollar (end)
% subsection lineare_differentialgleichungen_mit_konstanten_koeffizienten (end)
\cleardoubleoddemptypage
\pagenumbering{Alph}
\setcounter{page}{1}
\printindex
% subsubsection beispiele (end)
 % subsubsection satz_uber_implizite_funktionen (end)
% subsubsection beweis_des_umkehrsatzes_teil_2 (end)
