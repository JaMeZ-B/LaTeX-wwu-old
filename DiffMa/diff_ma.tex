...
\begin{enumerate}[(i)]
	\item \begin{itemize}
		...
		\item abzählbare Basis der Topologie:\todo{Zeichnung unfertig \ldots umbauen mit \texttt{elipsis}}\\
		\begin{minipage}{\textwidth}
			\captionsetup{type=figure, skip=3pt}
			\centering{
			\begin{tikzpicture}
...
\begin{figure}[h]
	\begin{tikzpicture}[scale=1.1]
		...
		\coordinate (b) at (3,5);
		\draw (0,5) -- (a) -- (b) -- (4,5) node[below]{$\mathds{R}$};
		...
		.. controls +(170:4) and + (85:3) .. (6,4.5) node[above right]{$M$}
		...
		\draw (6.5,5) .. controls +(0:0.1) and +(185:0.1) .. (c)
...
\[
	\int_{V= \varphi(U)} \! f  \, \mathrm{d}x = \int_{U} (f \circ \varphi) \abs*{\det J_\varphi}  \, \mathrm{d}x  
\]
...
\begin{itemize}
	...
	\[
		\sum_{i=1}^{\infty} \int_{h_i(A_i)}\! \! \! \abs*{a_i(x)}  \, \mathrm{d}x < \infty 
	\]
\end{itemize} 
Das \Index{Integral} von $\omega$ ist dann:
\[
	\int_{M}\! \omega := \sum_{i=1}^{\infty} \int_{h_i(A_i)} \! \! \! a_i(x)  \, \mathrm{d}x  
\] 
...
\begin{itemize}
	...
	\item $\sum_{j=1}^{\infty}  \int_{k_j(B_j)} \! \!  \abs*{b_j}  \, \mathrm{d}x < \infty $ 
	\item $\sum_{i=1}^{\infty} \int_{h_i(A_i)}\! \!  a_i(x)  \, \mathrm{d}x  = \sum_{j=1}^{\infty} \int_{k_j(B_j)}\! \! \! b_j(x)  \, \mathrm{d}x  $
\end{itemize}
...
\[
	\sum_{j=1}^{\infty} \int_{h_i(A_i \cap B_j)} \! \! a_i  \, \mathrm{d}x = \lim_{ n \to \infty} \int f_n  \, \mathrm{d}x = \int_{h_i(A_i)} a_i  \, \mathrm{d}x 
\]
...
\[
	\int_{k_j(A_i \cap B_j)} \! \! b_j  \, \mathrm{d}x  = \int_{k_j(A_i \cap B_j)} (a_i \circ \varphi) \abs*{\det J_\varphi}   \, \mathrm{d}x  = \int_{h_i(A_i \cap B_j)} 
	a_i  \, \mathrm{d}x 
\]
(Analog mit "$\abs*{.}$"). Es folgt 
\[
	\infty > \sum_{i=1}^{\infty} \int_{h_i(A_i)} \! \! \abs*{a_i} = \sum_{i,j} \int_{h_i(A_i \cap B_j)} \abs*{a_i}  \, \mathrm{d}x  = 
	\sum_{i,j} \int_{k_j(A_i \cap B_j)} \abs*{b_j}  \, \mathrm{d}x   
\]
Mit dem Konvegenzsatz von Lebesgue folgt: $\abs*{b_j}$ ist über $k_j(B_j)$ integrierbar (wie oben) und 
\[
	\int_{k_j(B_j)} \! \! b_j  \, \mathrm{d}x = \sum_{i=1}^\infty \int_{k_j(A_i \cap B_j)} b_j  \, \mathrm{d}x \tag*{Analog für $\abs*{b_j}$}
\]
Also gilt
\[
	\sum_{j=1}^{\infty} \int_{k_j(B_j)} \! \!  \abs*{b_j}  \, \mathrm{d}x < \infty 
\]
...
\begin{align*}
	...
	& \sum_{i} \int_{h_{\alpha(i)}(U_{\alpha(i)})} \abs*{a_i}  \, \mathrm{d}x < \infty 
\end{align*}
Es gilt $\int_{M} \! \omega = \sum_{i=1}^{\infty} \int_{h_{\alpha(i)}(U_{\alpha(i)})} a_i  \, \mathrm{d}x $
...
\[
	\int_M f  \, \mathrm{d}\mu = \int_M f \Omega
\]
...
\[
	\int_{a} ^{b} \! f'(t)  \, \mathrm{d}t =\int_{[a,b]}\mathd f = \int_{\partial[a,b]} f = f(b)- f(a)
\]
...
\end{align*}
Beweis der Behauptung: $\alpha \in \Omega^k(I \times M)$, $K_\alpha = \int_{0} ^{1} \! \enbrace*{i_{\diff{}{t} } \alpha}  \, \mathrm{d}t $. \oE $M=\mathds{R}^n$
\begin{enumerate}[(i)]
	\item $\alpha = f \mathd x^{i_1} \wedge \ldots \wedge \mathd x^{i_k}$ $\Rightarrow \mathd K \alpha =0$. 
	\[
		K \mathd \alpha = K \enbrace*{\mathd f \wedge \mathd x^{i_1} \wedge \ldots } = \enbrace*{\int_{0}^{1} \! \diff{f}{t}  \, \mathrm{d}t} \cdot \mathd x^{i_1} \wedge 
	...
	\]
	\[
		\mathd K \alpha = \sum_{j} \enbrace*{ \int_{0} ^{1} \! \diff{f}{x^j}  \, \mathrm{d}t } \mathd x^j \wedge \mathd x^{i_1} \wedge \ldots  
	\]
	\[
