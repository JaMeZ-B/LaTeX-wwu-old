...
% 		node[right]{$h$}; 
		...
		\begin{axis}[hide x axis, hide y axis, x=1cm, y=1cm, ymin=-0.5, ymax=1.5]
			...
			\draw[fill] (axis cs: -2,0) circle[radius=1.5] node[below]{\footnotesize$-2$};
			\draw[fill](axis cs: 2,0) circle[radius=1.5] node[below]{\footnotesize $2$};
		\end{axis}
	\end{tikzpicture}
...
\[
	\Rightarrow \abs*{ g \enbrace*{\gamma(t)} - g \enbrace*{\gamma(0)}  } \le \int_{0} ^{\abs*{t} } \abs*{\dot{(g \circ \gamma)}} \le \abs*{t}    
\]
...
\subsubsection{Definition} % (fold)
\label{ssub:422}
Für $f \in C^\infty(M)$ sei $\mathrm{d} f(X) := X(f)$. Also $(\mathrm{d}f)_p (X_p) = X_p f$. Die ist das \bet{totale} oder \bet{äußere Differential}\index{Differential!totales/äußeres}. $\mathrm{d}f$ ist eine $1$-Form auf $M$. Seien $(x^1, \ldots , x^m)$ lokale Koordinaten, dann gilt
\[
...
\]
d.h. $\mathrm{d} f = \diff{f}{x^i} \mathrm{d}x^i $ ist das \Index{totales Differential}. 
