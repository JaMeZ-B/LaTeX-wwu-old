...
\subsubsection[Notation für Pushforward]{Notation für Diffeomorphismen (Pushforward\footnote{"\emph{push} tangent vectors on $M$ \emph{forward} to tangent vectors on $N$", \url{https://en.wikipedia.org/wiki/Pushforward_(differential)}})} % (fold)
\label{ssub:245}
...
\begin{description}
	...
	\end{align*}
	\item["$\Rightarrow $":] Gelte $[X,Y] =0$. Nach Lemma 9  (\ref{ssub:247}) ist zu zeigen, dass $(\Phi_t)_* Y = Y$ für alle $t$. Es gilt 
	\begin{align*}
