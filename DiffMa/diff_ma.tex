...
\todo[inline]{Hier fehlt noch ein ganzes Stück, wurde anscheinend in der Vorlesung übersprungen oder vergessen!}
...
\[
	...
	\end{tikzcd}
\]
 Bündel heißt \bet{trivial}\index{triviales Bündel}, falls es eine (globale) Bündelkarte $(B,\varphi)$ gibt, d.h. 
\[
...
\begin{itemize}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}
			...
			\draw (215:1.5) -- (-x) node[left]{$-x$} -- (x) node[right]{$x$}-- ++ (35:0.5);
			\foreach \b in {x,-x,zero}{
				\draw[fill=Tomato1] (\b) circle[radius=0.03];
			}
		\end{tikzpicture}
