...
%	\item Halbgruppe
%	\item Gruppe
%	\item Kommutative bzw. abelsche Gruppe
%	\item Ring $(R,+,\cdot)$.
%	\item Ein Ring heißt kommutativ, wenn die Multiplikation kommutativ ist. Soll der Nullring $R = \setnull$ ausgeschlossen werden, fordert man zusätzlich $0 \neq 1$.
%	\item Die in einem Ring $(R,+,\cdot)$ bezüglich $\cdot$ invertierbaren Elemente heißen Einheiten. Die Menge der Einheiten bezeichnen wir mit $R^*$. $(R^*,\cdot, 1)$ ist eine Gruppe -- die Einheitengruppe des Rings.
%	\item Ein Körper $(k,+,\cdot)$ ist ein kommutativer Ring mit $0 \neq 1$ mit $k^* = k \setminus \setnull$.
%\end{enumerate}
%
%Motivation ...
%
%\begin{satz}[Satz 8]
%	Sei $g \in \NN, g \geq 2$ und $n \in \NN$. Dann gibt es ein $k \in \NN_0$ und $c_k, c_{k-1}, \dots c_0 \in \{0,1,\dots,g-1\}$, sodass
%	\[ n = c_k g^k + c_{k-1} g^{k-1} + \dots + c_1 g + c_0 \]
%	Fordert man $c_k \neq 0$, so ist $k$ und die Folge $c_k, c_{k-1}, \dots, c_0$ eindeutig bestimmt.
%\end{satz}
%
%\minisec{Beweis}
%\begin{description}
%	\item[Existenz:] Sei $k \in \NN_0$ so, dass $g^k \leq n < g^{k+1}$, setze $k := \floor*{\frac{\log n}{\log g}}$. Zeige durch Induktion nach $k$ die Existenz: \\
%	$k = 0$: Setze $c_0 = n$. \\
%	$k \rightsquigarrow k+1$: Seu $g^{k+1} \leq n < g^{k+2}$. Setze $n' = n - \floor*{\frac{n}{g^{k+1}}} \cdot g^{k+1}$. Es folgt $0 \leq n' < g^{k+1}$, d.h. auf $n'$ ist die Induktionsvoraussetzung anwendbar. Nach dieser hat $n'$ eine Zifferndarstellung $n' = \sum\limits_{i=0}^{k} c_i g^i$. Wegen $1 \leq  \frac{n}{g^{k+1}} < g$ ist $1 \leq \underbrace{\floor*{\frac{n}{g^{k+1}}}}_{=:c_{k+1}} < g$. \\
%	$\Rightarrow  n = c_{k+1} g^{k+1} + n' = \sum\limits_{i=0}^{k+1} c_i g^i$.
%	\item[Eindeutigkeit:] Sind $\sum\limits_{i=0}^{k} a_i g^i = m = \sum\limits_{i=0}^{r} b_i g^i$ zwei verschiedene Darstellungen von $m$. Sei $l := \max \{ i \leq \max\{k,r\}, a_i \neq b_i \}$. \\
%	$\Rightarrow 0 = \sum\limits_{i=0}^l (a_i-b_i)g^i \Rightarrow \underbrace{|b_l - a_l|}_{\geq 1} g^l = \abs{\sum\limits_{i=0}^{l-1} (a_i - b_i)g^i}$ \\
%	$\Rightarrow g^l \leq |b_l - a_l| g^l \leq \sum \underbrace{|a_i - b_i|} g^i$
%	
%	
%\end{description}
%	Die Zahl $k+1$ heißt Stellenzahl oder Länge der $g$-adischen Darstellung. Der Beweis zeigt $k + 1 = \abs*{\frac{\log n}{\log g} + 1 = \mathcal{O}(\log n)$, d.h. es existiert eine Konstante $C > 0$, sodass $k+1 \leq C \cdot \log(n)$.
%	
%
%
%\minisec{Beispiel}
%	$163_{(10)} = 1 \cdot 10^2 + 6 \cdot 10^1 + 3 \cdot 10^0$ \\
%	$43_{(10)} = 101011_{(2)} = 2B_{(16)}$ 
